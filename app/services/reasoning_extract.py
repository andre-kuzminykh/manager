...
Твоя задача — один reasoning pass:
  ...
  3. Список задач (tasks) — ВСЕ actionable follow-ups, включая:

     (a) ЯВНЫЕ задачи — кто-то сказал «я сделаю X» / «отправлю Y» / «организую Z».
         raw_owner_mention = тот кто это сказал (или назначен).

     (b) НЕЯВНЫЕ follow-ups для НАШЕЙ стороны — после встречи с инвестором /
         кандидатом / партнёром / клиентом всегда есть стандартные post-meeting
         обязательства:
         * обновить data room / shared folder если показывали материалы
         * передать промис'нутые контакты / документы
         * отправить следующую версию deck / pitch
         * запросить дополнительную инфу / запланировать следующий call
         * отследить committee / committee follow-up (если invest meeting)
         * подготовить аргументы / memo для внутренней дискуссии
         * сделать intro обещанным людям
         Эти задачи извлекай ДАЖЕ если на встрече не было явного «я сделаю».
         raw_owner_mention для них — name тех на нашей стороне кто отвечает
         за такие follow-ups (если в транскрипте не названо — пиши «мы»).

     (c) Внешние commitment'ы — другая сторона сказала «вернусь к вам с X».
         raw_owner_mention = их имя из транскрипта.

ВАЖНО:
  - Excessive extraction лучше чем missed: 12 задач из 30-минутной встречи —
    норма. 2-3 задачи из такой встречи = ты пропустил implicit follow-ups.
  - НЕ пытайся резолвить кто такой "Дима" / "Schaeffler" — пиши raw mention
    как в транскрипте. Резолв сущностей идёт отдельным шагом потом.
  - Используй «мы»/«нам» когда follow-up подразумевает team action без
    явного namee.
...
Поля задачи:
  - raw_owner_mention (required): имя как в транскрипте, или «мы» если team-side follow-up
