...
| FR-CB2-3.39 | Bilingual transcript restoration (operator-pinned 2026-05-20, «при транскрипте из зума отдельно (ничего не трогая в основном пайплайне) сделать так, чтобы отдельный вызов ллм сказал нужно ли сделать ещё транскрипт на английском, если нужно — отдельный вызов STT делает на английском, далее берём оба текста и прогоняем через LLM чтобы восстановить смысл»). Полностью изолированный пост-процессор `bilingual_restorer.py`, ВКЛЮЧАЕТСЯ только под `CEO_BRAIN_BILINGUAL_RESTORATION_ENABLED=true` (default OFF — оператор сначала смотрит трейсы, потом включает). Шаги: (1) detector — `gpt-4o-mini` (модель настраивается через `CEO_BRAIN_BILINGUAL_DETECTOR_MODEL`), бинарный YES/NO «есть ли в этом транскрипте garbled English»; (2) re-STT — POST на `CEO_BRAIN_STT_ENGLISH_URL` с `{meeting_id, audio_url, lang:"en"}`, ожидаем `{text}`; (3) reconciler — `gpt-4o` (`CEO_BRAIN_BILINGUAL_RECONCILER_MODEL`) принимает PRIMARY+SECONDARY транскрипты и эмитит финальный микс — русские сегменты из PRIMARY, английские из SECONDARY. Любой шаг fail-soft → возвращаем оригинал. Trace `_bilingual` пишется в `claude_responder_runs.request_payload` для диагностики (stage, decision, primary_chars, secondary_chars, final_chars). Hook — в `parallel_gather.gather_via_direct_http` после всех HTTP-пассов: каждый bucket с `::get_zoom_transcript` в label прогоняется через restore. OpenAI key — `CEO_BRAIN_OPENAI_API_KEY` (fallback на `OPENAI_API_KEY`). | `…::test_bilingual_restorer_disabled_by_default` + `…::test_bilingual_detector_decides_yes_for_mixed` + `…::test_bilingual_reconcile_merges_via_llm` + `…::test_bilingual_restorer_falls_back_when_no_stt_url` |
...
| FR-CB2-3.39 | `…::test_bilingual_restorer_disabled_by_default` + `…::test_bilingual_detector_decides_yes_for_mixed` + `…::test_bilingual_reconcile_merges_via_llm` + `…::test_bilingual_restorer_falls_back_when_no_stt_url` |
