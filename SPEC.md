...
| FR-CR-05-118 | `test_zoom.py::test_zoom_pipeline_runs_every_step_and_creates_zoom_source_tasks` extended: asserts `t.source_conversation_id == row.zoom_id` (so `JOIN zoom_recordings ON z.zoom_id = t.source_conversation_id` finds the meeting), `t.source_message_ts == row.zoom_id`, `t.due_time == 18:00` (FR-CR-05-63 parity), and that the admin DM stream contains BOTH the short summary AND ≥1 task card (`len(admin_dms) >= 2`). Pre-fix the Zoom path skipped `source_conversation_id`, `source_message_ts`, `due_time`, status-history rows, Sheets sync, and per-task DM cards — the operator saw the short summary in TG but no task cards, and no `tasks` rows joined to the `zoom_recordings` row. `app/zoom/pipeline.py::_step_extract_tasks` now mirrors `app/fireflies/pipeline.py`: full Task() ctor with join keys + due_time, `session.flush()` between Task and TaskStatusHistory, `schedule_sync_task` for Sheets/Tasks API push, `post_initial_card` for the per-task DM card (FR-CR-05-58 parity). | `test_zoom.py::test_zoom_listener_poll_off_by_default` (no API call until `wire_zoom(enabled=True)`); `::test_zoom_listener_poll_pulls_when_enabled` (after wire + cutoff override → `process_one` called for each fresh recording); `::test_zoom_listener_poll_throttled_within_interval` (3 immediate ticks → 1 API call); `::test_zoom_listener_poll_swallows_pipeline_errors` (one bad recording doesn't abort the batch); `::test_zoom_listener_poll_skips_recordings_before_startup` (FR-CR-05-51 cutoff symmetric with Fireflies — old meetings dropped, new ones processed); `::test_settings_zoom_polling_defaults` (realtime OFF by default; 60s interval; 10-item batch; Fireflies poll interval also defaulted to 60s). Manual: alembic migration `0021_task_source_kind_zoom` adds `'zoom'` to the `task_source_kind` Postgres enum (the 0020 migration created the table but forgot the enum extension, causing «invalid input value for enum task_source_kind: \"zoom\"» on Task insert); `app/telegram_bot/listener.py::_maybe_poll_zoom` mirrors `_maybe_poll_fireflies` (same cutoff, throttle, error-swallow contract); `ops/telegram_listener.py` wires `ZoomPipeline` when `ZOOM_ACCOUNT_ID/CLIENT_ID/SECRET` are set; `ops/migrate_zoom.py --rerun` symmetric with Fireflies. |
| FR-CR-05-117 | `test_fireflies.py::test_looks_like_auto_stamp_title_detects_fireflies_defaults` («Apr 30, 03:32 PM», «May 5 at 5pm», «Dec 1», empty/None all flagged; real titles like «ADNOC partnership call», «Раунд Humanoid» pass through); `::test_short_summary_prompt_pins_operator_format` (canonical ADNOC layout pinned in `SHORT_SUMMARY_SYSTEM`: header `<Тема> — DD.MM.YYYY \| NN мин`, «Их сторона» / «Наша сторона» split, «Суть», «To-Do» numbered list, anti-regression on Fireflies auto-stamps). Manual: `ops/migrate_fireflies.py --rerun` resets all step flags + outputs (`audio_downloaded`, `transcribed`, `detailed_summarised`, `short_summary_sent`, `doc_exported`, `tasks_extracted`, `transcript_text`, `detailed_summary`, `short_summary`, `google_doc_id`, `google_doc_url`, `tasks_extracted_count`, `last_error`, `processed_at`) on already-processed recordings so the pipeline re-executes every step after prompt updates; `pipeline.py::_step_detailed_summary` calls `_derive_topic_title` (LLM, ≤60 chars Russian topic) when `_looks_like_auto_stamp_title(row.title)` to replace Fireflies' auto-timestamps with a real business topic before the doc / short-summary stages. |
