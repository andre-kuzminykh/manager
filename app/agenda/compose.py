...
- previous_recap: список 2-4 коротких предложений. Это будет отрисовано как сплошной абзац — renderer САМ добавит префикс «На прошлой встрече: » один раз. Каждый item — отдельное предложение, НЕ начинай с «На прошлой встрече» / «На предыдущей встрече» / «В прошлый раз» / любых аналогов (renderer уже это написал). Без маркеров, без нумерации, без эмодзи. Передавай суть последней встречи + ключевые договорённости. Если prior_recordings пусто — верни пустой список.
