...
| FR-CB2-3.17 | Synthesis recovery — когда финальное сообщение Claude НЕ заканчивается text-блоком с синтезом (наблюдаемые Sonnet-quirks: «нет text-блоков вообще» ИЛИ «есть промежуточный «Сейчас проверю…», но последний блок — tool_use без summary после него»), responder делает ОДИН follow-up `messages.stream` call с original messages + assistant turn (содержимое первого ответа) + user turn («Не вызывай больше tools. Напиши финальный ответ оператору на основе данных выше»). Recovery-запрос **принудительно очищается** от `mcp_servers` / `tools` / `betas`, чтобы модель не могла снова уйти в tool-вызовы вместо синтеза. Recovery детектится по индексам: last_text_block_index > last_tool_use_index → синтез есть; иначе → recovery. Если и recovery вернул пусто — рендерим явный fallback `_(модель не сформулировала ответ — см. Sources)_` вместо пустого тела. | `test_responder_synthesis_recovery_when_text_empty` + `test_responder_synthesis_recovery_when_last_block_is_tool_use` + `test_responder_synthesis_recovery_strips_tools_to_force_text` |
...
| FR-CB2-3.19 | Search-strategy rules в system prompt — operator-observed 2026-05-19 (last iter): для вопроса «что обсудили с Йоханом по фандрайзингу» бот вызвал `search_zoom_meetings('Strategic Investors')`, получил пустой transcript (792 символа — только участники), и сдался вместо того чтобы попробовать «Fundraising daily» (где 60К символов реального диалога). System prompt теперь содержит две явные инструкции: (1) search-query должен брать ТЕМУ из вопроса оператора (fundraising, due diligence, raise), а не только имя контрагента; (2) если `get_transcript` / `get_meeting` вернул <2000 символов — считать пустым, обязательно попробовать другой кандидат или новый search с другим query. | `test_responder_system_prompt_includes_search_strategy_rules` |
...
| FR-CB2-3.23 | MCP transient handshake retry (operator-pinned 2026-05-19): Anthropic делает parallel handshake ко всем `mcp_servers` в начале вызова. Если хотя бы один не отвечает за таймаут — весь запрос валится с `BadRequestError: Connection error while communicating with MCP server`. Каждый отдельный сервер работает (проверено индивидуально), но один из 6+ периодически тормозит на handshake. Фикс: responder детектит этот error pattern и ретраит до 5 раз с exp backoff 2/4/8/16/32 сек. | `test_responder_retries_on_mcp_handshake_connection_error` |
| FR-CB2-3.24 | Progressive MCP degradation on retry (operator-pinned 2026-05-19): чистый ретрай не помогает когда у Anthropic↔n8n cloud handshake стабильно flaky из-за parallel-load. Чем меньше MCP в запросе — тем выше шанс что все они синхронно ответят. Responder на каждом retry (начиная с 3-го) отбрасывает по одному MCP с конца списка `mcp_servers`, оставляя минимум 2. То есть attempt 1-2: все 6 серверов; attempt 3: 5; attempt 4: 4; attempt 5: 3. К концу серии retries бот работает уже с подмножеством но handshake скорее всего пройдёт. Оператор может отсортировать `MCP_SERVERS` env в порядке убывания критичности — самый важный наверху сохраняется до конца. | `test_responder_progressive_mcp_degradation_on_retry` |
| FR-CB2-3.25 | Smart MCP routing — operator-pinned 2026-05-19. Чтобы каждый ответ не тащил handshake ко всем 6 MCP (медленно + flaky), responder сначала делает короткий `claude-haiku-4-5` вызов с вопросом оператора + каталогом MCP описаний → модель возвращает JSON-список MCP-имён, которые реально нужны для ответа. Только эти MCP передаются в основной запрос. Каталог-описания захардкожены в `_MCP_DESCRIPTIONS` (можно вынести в env позже). Если классификатор упал или вернул мусор — fallback на все MCP (поведение FR-CB2-3.24 остаётся). Время ответа на типичный вопрос: 30-60 сек вместо 2-6 мин. | `test_smart_mcp_routing_picks_calendar_for_meeting_questions` + `test_smart_mcp_routing_fallback_on_classifier_failure` |
| FR-CB2-3.26 | Conversational continuity в треде (operator-pinned 2026-05-19): после фикса FR-CB2-3.20 все bot-сообщения дропались из thread context, и оператор терял continuity («ты же уже отвечал, бери из треда»). Теперь bot-сообщения сохраняются как `role=assistant` — но ТОЛЬКО реальные ответы. Статусные плейсхолдеры (`🤔 думаю`, `⏳ retry`, `⚠️ ошибка`, `🔍 проверил…`, `🔄 ищу…`) детектятся по emoji-префиксу и пропускаются — они засоряют контекст. Empty-text bot-сообщения тоже skip. Так оператор может задавать follow-up вопросы «а ещё?», «расскажи подробнее про X из того что выше», и модель видит свой предыдущий ответ. | `test_thread_history_includes_bot_real_answers_as_assistant` + `test_thread_history_skips_bot_status_placeholders` |
| FR-CB2-3.28 | Laconic output (operator-pinned 2026-05-19): «мне лаконично надо без всего мусора, чисто по сути». System prompt теперь жёстко требует: (1) прямой ответ на вопрос — без преамбул («Хорошо, нашёл…»), без описания процесса («Читаю транскрипт…»), без markdown-таблиц с колонками «Приоритет/Срок» если оператор не попросил; (2) maximum 5-8 строк на типичный вопрос; (3) bullet-points только когда реально перечисление; (4) Sources блок — однострочный список tool names, без описаний. | `test_system_prompt_enforces_laconic_output` |
| FR-CB2-3.29 | Inline source links, no IDs (operator-pinned 2026-05-19): «вот эти ссылки не надо, можешь только гиперссылкой источник указать, в самом ответе вставить». System prompt запрещает: (1) выводить technical metadata типа `Zoom ID: Es0xxBa8RHG4lPl9Z5ZMGQ==`, `60 мин`, `(meeting_id=…)` — это мусор для оператора; (2) отдельный блок `_Источники:_ tool_a, tool_b` в конце ответа; (3) markdown-список `tool1: что делает …`. Вместо этого: factual statements в ответе должны содержать inline-гиперссылки на источник (Google Doc, Slack permalink, Telegram link) в формате `<URL\|короткий якорь>` (Slack-native). Пример: «На <https://docs.google.com/.../edit\|Fundraising daily> обсудили…». | `test_system_prompt_enforces_inline_source_links_no_ids` |
| FR-CB2-3.30 | Per-MCP parallel gather architecture (operator-pinned 2026-05-19, «определи куда ходить и оттуда верни параллельно данные»): вместо ОДНОГО `messages.create` с `mcp_servers=[1..N]` и parallel handshake-race на стороне Anthropic, responder делает: (1) классификатор (FR-CB2-3.25) → словарь `{mcp_name: подробный sub-вопрос}` (haiku); (2) `concurrent.futures.ThreadPoolExecutor` фьючерсы — для каждого выбранного MCP отдельный `beta.messages.create` с ОДНИМ сервером в `mcp_servers` (handshake надёжен, latency предсказуем); (3) каждый future возвращает `{mcp_name: harvested_text}`; (4) финальный synthesis call — обычный `messages.create` БЕЗ MCP, с собранными `<DATA>` блоками и оригинальным вопросом → текст ответа в Slack. Гарантирует: max latency = max(per-MCP), не sum; handshake-failures изолированы (один упал — остальные дают данные); SLA <60 сек на типичный вопрос. | `test_per_mcp_parallel_gather_aggregates_in_threads` + `test_per_mcp_synthesis_skips_failed_mcp` |
| FR-CB2-3.31 | Direct-HTTP MCP architecture (operator-pinned 2026-05-20, «нам не должны выводить мы ничего не нашли»): Anthropic-MCP даже с per-MCP parallel gather (FR-CB2-3.30) висит на multi-step queries (model делает 3-5 tool-вызовов внутри одного call'а, каждый 5-15 сек overhead'а Anthropic-стороны → 60-300 сек). Direct test: HTTP к n8n MCP отдаёт 60К-char transcript за 1.97 сек. Решение: полностью обойти Anthropic-MCP слой — `mcp_client.py` напрямую говорит JSON-RPC over HTTP с n8n endpoints (init → notifications/initialized → tools/call), `planner.py` через haiku решает какие tool calls делать с какими args, gather в ThreadPoolExecutor дёргает HTTP параллельно, synthesis через Anthropic без MCP. Результат: 10-25 сек total на сложный multi-step вопрос. Anthropic-MCP путь остаётся как fallback для непланируемых сценариев. | `test_mcp_http_init_and_tool_call` + `test_planner_returns_tool_calls_per_mcp` + `test_direct_http_gather_aggregates` |
| FR-CB2-3.38 | Planner-output backfill (operator-pinned 2026-05-20): даже когда классификатор (FR-CB2-3.25) корректно выбирает MCP, haiku-планировщик периодически забывает положить tool-call для одного из выбранных серверов (например, отдаёт только `n8n_drive::search_messages`, но НЕ `n8n_calendar::search_zoom_meetings` хотя календарь в picked-списке). Из-за этого синтез отвечает «сообщений не нашёл» проигнорировав сегодняшнюю встречу. После основного prompt-цикла планнер сверяет `{c.mcp for c in out}` против set picked-MCP. Для каждого MISSING MCP ищет первый tool, имя которого начинается с `search` (или fallback на дефолтный из `_OPERATOR_TOOL_DESCRIPTIONS`), формирует args из `inputSchema.properties` — `query=question[:60]`, остальные required-стринги пустыми — и инжектит call. Гарантирует: ни один выбранный сервер не остаётся без хотя бы одного дёрнутого tool. | `…::test_planner_backfills_missing_mcps` |
| FR-CB2-3.39 | Bilingual transcript restoration (operator-pinned 2026-05-20, «при транскрипте из зума отдельно (ничего не трогая в основном пайплайне) сделать так, чтобы отдельный вызов ллм сказал нужно ли сделать ещё транскрипт на английском, если нужно — отдельный вызов STT делает на английском // тот же STT что и брал, но язык англ // далее берём оба текста и прогоняем через LLM чтобы восстановить смысл»). Полностью изолированный пост-процессор `bilingual_restorer.py`, ВКЛЮЧАЕТСЯ только под `CEO_BRAIN_BILINGUAL_RESTORATION_ENABLED=true` (default OFF — оператор сначала смотрит трейсы, потом включает). Шаги: (1) detector — `gpt-4o-mini` (`CEO_BRAIN_BILINGUAL_DETECTOR_MODEL`), бинарный YES/NO «есть ли в этом транскрипте garbled English»; (2) re-STT — тот же Whisper из `app.services.transcription.transcribe_bytes` с `language="en"`, аудиофайл резолвится по `ZoomRecording.audio_path` (lookup по `zoom_id`, который парсится из `Zoom ID: …` хедера транскрипта); (3) reconciler — `gpt-4o` (`CEO_BRAIN_BILINGUAL_RECONCILER_MODEL`) принимает PRIMARY+SECONDARY и эмитит финальный микс — русские сегменты из PRIMARY, английские из SECONDARY. Любой шаг fail-soft → возвращаем оригинал. Trace `_bilingual` пишется в `claude_responder_runs.request_payload` (stage, decision, primary_chars, secondary_chars, final_chars). Hook — в `parallel_gather.gather_via_direct_http` после всех HTTP-пассов: каждый bucket с `::get_zoom_transcript` в label прогоняется через restore. Smoke-test CLI: `python -m ops.bilingual_smoke --zoom-id ...`. OpenAI key — `CEO_BRAIN_OPENAI_API_KEY` (fallback на `OPENAI_API_KEY`). | `…::test_bilingual_restorer_disabled_by_default` + `…::test_bilingual_detector_decides_yes_for_mixed` + `…::test_bilingual_reconcile_merges_via_llm` + `…::test_bilingual_restorer_falls_back_when_audio_missing` |
...
| FR-CB2-3.17 | `…::test_responder_synthesis_recovery_when_text_empty` + `…::test_responder_synthesis_recovery_when_last_block_is_tool_use` + `…::test_responder_synthesis_recovery_strips_tools_to_force_text` |
| FR-CB2-3.18 | `…::test_final_render_retries_on_rate_limit` + `…::test_final_render_falls_back_to_new_message` |
| FR-CB2-3.19 | `…::test_responder_system_prompt_includes_search_strategy_rules` |
...
| FR-CB2-3.24 | `…::test_responder_progressive_mcp_degradation_on_retry` |
| FR-CB2-3.25 | `…::test_smart_mcp_routing_picks_calendar_for_meeting_questions` + `…::test_smart_mcp_routing_fallback_on_classifier_failure` |
| FR-CB2-3.26 | `…::test_thread_history_includes_bot_real_answers_as_assistant` + `…::test_thread_history_skips_bot_status_placeholders` |
| FR-CB2-3.28 | `…::test_system_prompt_enforces_laconic_output` |
| FR-CB2-3.29 | `…::test_system_prompt_enforces_inline_source_links_no_ids` |
| FR-CB2-3.38 | `…::test_planner_backfills_missing_mcps` |
| FR-CB2-3.39 | `…::test_bilingual_restorer_disabled_by_default` + `…::test_bilingual_detector_decides_yes_for_mixed` + `…::test_bilingual_reconcile_merges_via_llm` + `…::test_bilingual_restorer_falls_back_when_audio_missing` |
