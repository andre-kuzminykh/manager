...
| FR-CR-05-126 | Per-LLM-call transparency: every LLM round-trip in the meeting pipelines emits a `*_llm_returned` log line with the raw model output (titles + owners + matched ids) BEFORE filtering / persisting. Operator can `grep counterparty_match\\|task_extraction_llm_returned\\|task_verification_done` to see exactly what each pass extracted vs what landed in DB. **Counterparty match:** `app/services/counterparty_match.py::match_counterparties_in_transcript` emits `counterparty_match_call_started` (model, reasoning_effort, directory_size, directory_sample, transcript_chars, transcript_preview, prompt_chars), `counterparty_match_llm_returned` (raw_ids_count, valid_ids_count, invalid_ids, raw_ids_sample), `counterparty_match_done` (matched_names list). Empty directory case logs `counterparty_match_skipped_empty_directory`. **Task extraction (Fireflies + Zoom):** `*_task_extraction_llm_returned` carries `raw_count`, `raw_titles[:25]`, `raw_owners[:25]` so the operator sees what the model wrote before owner-routing / dedupe / status-history insertion. **Task verifier:** `*_task_verification_done` extended with `new_titles[:25]` + `new_owners[:25]` so the second-pass additions are auditable inline. Suite: 1536 passed (no test changes — the new logs sit alongside existing assertions). |
