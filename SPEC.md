...
| FR-CR-05-199 | **Slack publish — ALL tasks в thread reply, priority direction в parent message** (operator-pinned 2026-05-22: «все задачи в поток а те кто в дирекции приоритетной - в слак идут»). До этого FR `_build_todo_section` фильтровал tasks по `direction in DIRECTIONS_IMPORTANT` (beta/budget/design/investors/deliverables) — если у task'и direction='other', она в Slack публикацию НЕ попадала вообще. Это приемлемо для legacy mode (короткий важный To-Do для CEO), но **противоречит V2 интенту**: V2 находит больше задач, ВСЕ они нужны в thread для аудита, а в parent message — только важные как highlight. **Fix:** двойной рендер — `_build_todo_section(filter_by_direction=True)` для parent (как было) + `_build_todo_section(filter_by_direction=False)` для thread reply. `publish_zoom_recording_to_slack` принимает kwarg `all_tasks_in_thread: bool = False` (default backwards-compat false для legacy auto-publish); V2 publish (`ops/v2_publish_meeting.py`) вызывает с `all_tasks_in_thread=True`. Parent — short_summary + важные tasks в «TODO:» trailer. Thread reply — ПОЛНЫЙ список tasks (включая direction='other'). | `test_slack_publish_all_tasks.py::test_fr_cr_05_199_thread_contains_all_tasks` + `…parent_only_important` + `…backwards_compat_default_filter_only` |
