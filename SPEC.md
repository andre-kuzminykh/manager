...
| FR-CR-05-193a-2 | **Step 1 output JSON schema** `{summary_detailed: str, summary_short: str, tasks: [{raw_owner_mention: str, title: str, description?: str, due_date?: ISO-date, priority?: low/medium/high/urgent}]}`. | `test_reasoning_extract_prompt.py::test_fr_cr_05_193a_returns_summary_and_tasks_json` |
| FR-CR-05-193a-3 | **Step 1 НЕ резолвит entities** — Step 1 функция MUST NOT принимать `known_people` / `known_orgs` / `team_members` / `counterparties`. Raw mentions остаются как в транскрипте. | `test_reasoning_extract_prompt.py::test_fr_cr_05_193a_no_resolution_in_step_one` |
| FR-CR-05-193a-4 | **Step 1 safe fallback на пустой/мусорный input** — `transcript=''/None` → return `{summary_detailed:'', summary_short:'', tasks:[]}`, LLM не зовётся. | `test_reasoning_extract_prompt.py::test_fr_cr_05_193a_handles_empty_transcript` |
| FR-CR-05-193a-5 | **Step 1 safe fallback на malformed LLM JSON** — non-JSON / partial → возврат empty structure + warning log, не raise. | `test_reasoning_extract_prompt.py::test_fr_cr_05_193a_handles_malformed_llm_json` |
| FR-CR-05-193a-6 | **Step 1 required task fields** — task в output MUST иметь `raw_owner_mention` + `title`. Tasks без них dropped из result. | `test_reasoning_extract_prompt.py::test_fr_cr_05_193a_each_task_has_required_fields` |
| FR-CR-05-193a-7 | **Step 1 optional task fields** — `due_date` (ISO-date или None), `priority` (low/medium/high/urgent, default `medium`), `description` (str или None). | `test_reasoning_extract_prompt.py::test_fr_cr_05_193a_due_date_priority_optional` |
| FR-CR-05-193a-8 | **Step 1 summary_short distinct from summary_detailed** — обе non-empty, summary_short < summary_detailed по длине (LLM генерит независимо, не префикс). | `test_reasoning_extract_prompt.py::test_fr_cr_05_193a_summary_short_is_subset_or_distinct` |
| FR-CR-05-193b-1 | **Step 2: single matcher LLM call** (gpt-5.5, `reasoning_effort=medium`). One call per (text + raw_owners). Service `app/services/entity_matcher.py::match_entities(text, raw_owners, known_people, known_orgs, llm_backend, model)`. | `test_entity_matcher_people.py` + `test_entity_matcher_orgs.py` (combined) |
| FR-CR-05-193b-2 | **Step 2 people exact match** — raw mention совпадает с `TeamMember.real_name` exactly (case-insensitive) → `tm_real_name=real_name`. | `test_entity_matcher_people.py::test_fr_cr_05_193b_exact_real_name_match` |
| FR-CR-05-193b-3 | **Step 2 people domain-disambiguation via notes** — для ambiguous raw (e.g., «Дима» = Дима Дроздов OR Дмитрий Седов) matcher LLM получает notes ВСЕХ кандидатов и решает по контексту. | `test_entity_matcher_people.py::test_fr_cr_05_193b_disambiguates_via_notes` + `…_passes_notes_to_llm` |
| FR-CR-05-193b-4 | **Step 2 people no_match → None** — если raw не в `known_people`, `tm_real_name=None` (для аудита). Task всё равно создаётся БЕЗ owner. | `test_entity_matcher_people.py::test_fr_cr_05_193b_no_match_returns_none` |
| FR-CR-05-193b-5 | **Step 2 people phonetic translit** — translit RU→EN (`Yulya` → `Юля`), EN→RU (`Jared` → `Джарад`) через LLM matcher. | `test_entity_matcher_people.py::test_fr_cr_05_193b_phonetic_translit` |
| FR-CR-05-193b-6 | **Step 2 summary mentions resolved independently** — упоминания в summary не привязаны к task'у. `summary_replacements_people` содержит ВСЕ matched mentions из summary, не только task owners. | `test_entity_matcher_people.py::test_fr_cr_05_193b_summary_mentions_resolved_independently` |
| FR-CR-05-193b-7 | **Step 2 orgs aliases match** — `Counterparty.aliases` (через `counterparty_attrs.source='aliases'`) использованы для phonetic match (Шеффлер ↔ Schaeffler). | `test_entity_matcher_orgs.py::test_fr_cr_05_193b_orgs_alias_match` |
| FR-CR-05-193b-8 | **Step 2 orgs abbreviation match** — короткие формы («Bain» → «Bain Capital», «KKR» → известный enum). | `test_entity_matcher_orgs.py::test_fr_cr_05_193b_orgs_abbreviation_match` |
| FR-CR-05-193b-9 | **Step 2 orgs cyrillic↔latin** — двусторонний переход. | `test_entity_matcher_orgs.py::test_fr_cr_05_193b_orgs_cyrillic_latin` |
| FR-CR-05-193b-10 | **Step 2 empty inputs safe** — `raw_owners=[]` / `known_people=[]` / `known_orgs=[]` → возврат пустых структур, LLM не зовётся (no cost). | `test_entity_matcher_people.py::test_fr_cr_05_193b_empty_known_people_safe` + `test_fr_cr_05_193b_empty_raw_owners_returns_empty_task_owners` |
| FR-CR-05-193b-11 | **Step 2 invalid LLM JSON safe** — мусорный output → fallback: `task_owners=[{raw_owner, tm_real_name=None}]` для каждого raw, summary_replacements=[]. | `test_entity_matcher_people.py::test_fr_cr_05_193b_llm_invalid_json_safe` |
| FR-CR-05-193b-12 | **Step 2 people TM filter — humans only** — `get_humans_for_matcher(session)` фильтрует bots / inactive / empty real_name. | `test_entity_matcher_people.py::test_fr_cr_05_193b_filters_humans_only` |
| FR-CR-05-193c-1 | **Step 3: deterministic apply — text replace** — regex word-boundary aware replace в `summary_detailed` + `summary_short` по `summary_replacements_people` + `summary_replacements_orgs`. Idempotent (повторный run не меняет). | `test_entity_apply.py::test_fr_cr_05_193c_text_replace_word_boundary` |
| FR-CR-05-193c-2 | **Step 3: task.owner lookup** — `tm_real_name` → SELECT TeamMember WHERE `real_name=:name` → set `owner_display_name=real_name`, `owner_user_id=slack_user_id` (если есть). | `test_entity_apply.py::test_fr_cr_05_193c_owner_lookup_sets_user_id` |
| FR-CR-05-193c-3 | **Step 3: DELEGATE_TASKS_TO apply** — если owner.notes содержит `DELEGATE_TASKS_TO: <name>` AND target существует in TeamMember → swap owner. Используется FR-CR-05-192r marker. | `test_entity_apply.py::test_fr_cr_05_193c_delegate_marker_applied` |
| FR-CR-05-193c-4 | **Step 3: DO_NOT_CALL skip** — если owner.notes содержит `DO_NOT_CALL` (case-insensitive) → owner_display_name=None + extra.skipped_owner. Task создаётся БЕЗ owner. | `test_entity_apply.py::test_fr_cr_05_193c_do_not_call_skip` |
| FR-CR-05-193c-5 | **Step 3: idempotency** — повторный apply того же `summary_replacements` не меняет текст. Маркеры остаются. | `test_entity_apply.py::test_fr_cr_05_193c_idempotent_apply` |
| FR-CR-05-193c-6 | **Step 3: unmatched mentions preserve** — raw упоминания НЕ в `summary_replacements` остаются в тексте как есть. | `test_entity_apply.py::test_fr_cr_05_193c_unmatched_mentions_preserved` |
| FR-CR-05-193c-7 | **Step 3: error in apply doesn't break pipeline** — exception в Step 3 → log warning, persist Step 1 raw result (saммари без canonical replace, owner=None для tasks). | `test_entity_apply.py::test_fr_cr_05_193c_error_safe_fallback` |
| FR-CR-05-193d-1 | **TeamMember.notes DSL parser** — extract `DELEGATE_TASKS_TO: <name>` + `DO_NOT_CALL` markers из freeform notes. Case-insensitive, whitespace-tolerant, single-hop only. | `test_team_member_notes_dsl.py::test_fr_cr_05_193d_parse_delegate_marker` + `…_parse_do_not_call` |
| FR-CR-05-193d-2 | **Counterparty.aliases storage** — `counterparty_attrs` row с `source='aliases'`, `attributes={"aliases":["Шеффлер","Шафлер"]}`. Unique constraint на (`counterparty_id`, `source`). | `test_counterparty_aliases.py::test_fr_cr_05_193d_aliases_storage_schema` |
| FR-CR-05-193d-3 | **get_humans_for_matcher(session)** — фильтр active=True + real_name non-empty + not bot. Возвращает list[dict] для matcher prompt. | `test_entity_matcher_people.py::test_fr_cr_05_193b_filters_humans_only` |
| FR-CR-05-193d-4 | **get_orgs_with_aliases(session)** — JOIN `counterparties` + `counterparty_attrs(source='aliases')`, возвращает list[{cp_id, name, aliases:list[str]}]. | `test_counterparty_aliases.py::test_fr_cr_05_193d_get_orgs_with_aliases` |
| FR-CR-05-193e-1 | **entity_resolution_cache table** — schema `(cache_key sha256 PRIMARY KEY, payload jsonb, created_at, expires_at, hits_count int)`. TTL 7 days default. | `test_entity_resolution_cache.py::test_fr_cr_05_193e_schema_and_ttl` |
| FR-CR-05-193e-2 | **Cache key composition** — `sha256(text + known_people_hash + known_orgs_hash)`. Гарантия что изменение TeamMember/Counterparty инвалидирует кэш. | `test_entity_resolution_cache.py::test_fr_cr_05_193e_cache_key_includes_known_hashes` |
| FR-CR-05-193e-3 | **Cache hit observability** — log event `entity_matcher_cache_hit` / `_miss` с метриками. | `test_entity_resolution_cache.py::test_fr_cr_05_193e_logs_hit_miss` |
| FR-CR-05-193e-4 | **Cache expiration cleanup** — periodic op `ops/cleanup_entity_cache.py` удаляет rows where `expires_at < now()`. | `test_entity_resolution_cache.py::test_fr_cr_05_193e_cleanup_expired` |
| FR-CR-05-193f-1 | **Backfill counterparty aliases — op script** `ops/backfill_counterparty_aliases.py`. LLM batch (50 cp per call, gpt-5.5 mini). | `test_backfill_counterparty_aliases.py::test_fr_cr_05_193f_batch_50_per_call` |
| FR-CR-05-193f-2 | **Backfill idempotent** — skip counterparty if aliases уже есть в counterparty_attrs(source='aliases'). | `test_backfill_counterparty_aliases.py::test_fr_cr_05_193f_skip_already_aliased` |
| FR-CR-05-193f-3 | **Backfill dry-run** — `--dry-run` flag печатает proposed aliases без записи. | `test_backfill_counterparty_aliases.py::test_fr_cr_05_193f_dry_run_no_writes` |
| FR-CR-05-193f-4 | **Backfill LLM mock-able** — service зависит от passed llm_backend, тест mock'ает. | `test_backfill_counterparty_aliases.py::test_fr_cr_05_193f_uses_injected_llm_backend` |
| FR-CR-05-193f-5 | **Backfill empty counterparties → no-op** — 0 rows → no LLM calls, exit 0. | `test_backfill_counterparty_aliases.py::test_fr_cr_05_193f_empty_db_no_calls` |
| FR-CR-05-193g-1 | **Pipeline integration: new _step_extract_via_reasoning** — заменяет `_step_detailed_summary` + `_step_short_summary` + `_step_extract_tasks`. Идёт после `_step_transcribe`, до `_step_save_doc` / `_step_send_summary`. | `test_entity_pipeline_e2e.py::test_fr_cr_05_193g_zoom_full_pipeline` |
| FR-CR-05-193g-2 | **Zoom integration** — `app/zoom/pipeline.ZoomPipeline._step_extract_via_reasoning` вызывается из `process_one`. Idempotent через `ZoomRecording.extracted_via_reasoning bool` flag (новая колонка). | `test_entity_pipeline_e2e.py::test_fr_cr_05_193g_zoom_full_pipeline` |
| FR-CR-05-193g-3 | **Fireflies integration** — `app/fireflies/pipeline.FirefliesPipeline._step_extract_via_reasoning`. Симметрично Zoom. | `test_entity_pipeline_e2e.py::test_fr_cr_05_193g_fireflies_full_pipeline` |
| FR-CR-05-193g-4 | **Feature flag `ENTITY_RESOLUTION_V2_ENABLED`** — default `true`. False → fallback на старые 3 step'a (FR-CR-05-191/192k path). Для zero-downtime rollback. | `test_entity_pipeline_e2e.py::test_fr_cr_05_193g_feature_flag_disabled_uses_legacy` |
| FR-CR-05-193g-5 | **Pipeline idempotent re-run** — если `extracted_via_reasoning=True AND last_error IS NULL`, skip step (используем cache при необходимости). | `test_entity_pipeline_e2e.py::test_fr_cr_05_193g_idempotent_skip` |
| FR-CR-05-193h-1 | **tasks.owner_display_name strict** — только canonical из `TeamMember.real_name` или NULL. Никаких raw emails, никаких unknown mentions. | `test_task_owner_strict.py::test_fr_cr_05_193h_no_raw_email_in_owner` |
| FR-CR-05-193h-2 | **tasks.owner_user_id derived from TM** — если у TeamMember есть slack_user_id или telegram_user_id, копируется в task. | `test_task_owner_strict.py::test_fr_cr_05_193h_owner_user_id_from_tm` |
| FR-CR-05-193h-3 | **tasks.extra.matcher_meta** — JSON `{raw_owner_mention, tm_id, tm_real_name, reasoning, status: 'matched'/'no_match'/'delegated'/'skipped_do_not_call'}` для аудита. | `test_task_owner_strict.py::test_fr_cr_05_193h_matcher_meta_stored` |
| FR-CR-05-193h-4 | **tasks.status='proposed' default** — Step 3 создаёт task с state=proposed (требует Confirm в TG), кроме случаев когда matcher_meta.status='delegated' (тогда сразу 'todo'). | `test_task_owner_strict.py::test_fr_cr_05_193h_default_state_proposed` |
| FR-CR-05-193h-5 | **tasks без owner всё равно создаются** — `owner_display_name=None` + `matcher_meta.status='no_match'`. Для оператор-аудита неизвестных упоминаний. | `test_task_owner_strict.py::test_fr_cr_05_193h_no_match_still_persists_task` |
| FR-CR-05-193h-6 | **tasks с DO_NOT_CALL skip** — `owner_display_name=None` + `matcher_meta.status='skipped_do_not_call'` + `matcher_meta.original_owner=<name>`. | `test_task_owner_strict.py::test_fr_cr_05_193h_do_not_call_persists_with_status` |
