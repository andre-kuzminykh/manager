...
| FR-CR-05-119 | Operator-pinned single-source-of-truth for action items: tasks live as Task rows; the short-summary «To-Do» block is rebuilt from those rows; the detailed summary (Google Doc) drops the «СЛЕДУЮЩИЕ ШАГИ» section. Plus a stronger named-assignee prompt rule so «Алине поручено …» / «Дима, нужно …» can't fall through to admin / a substitute teammate. **Detailed summary prompt:** `test_fireflies.py::test_detailed_summary_prompt_drops_next_steps_section` — `СЛЕДУЮЩИЕ ШАГИ` removed from the bulleted template; explicit «FR-CR-05-119: do NOT emit a «СЛЕДУЮЩИЕ ШАГИ» / «Action items» / «Tasks» / «To-Do» section» instruction added. **Short summary prompt:** `::test_short_summary_prompt_forbids_llm_emitting_todo` — LLM body must end at «Суть»; pipeline appends the To-Do block. **Deterministic To-Do builder:** `::test_build_todo_section_renders_tasks_verbatim_with_owner` — `_build_todo_section(session, source_kind, source_conversation_id)` lists every non-deleted Task in id-order, `N) {description} ({owner_display_name})`, drops the section entirely when no tasks were extracted; uses `description` (verbatim what the LLM wrote on the Task row) as the line body. **LLM-emitted To-Do strip:** `::test_strip_llm_todo_block_removes_emitted_section` — even with the prompt forbidding it, the LLM occasionally still emits «To-Do:» / «Следующие шаги:» / «Action items:» blocks; `_strip_llm_todo_block` removes them before the deterministic one is appended. **Pipeline reorder:** `_step_extract_tasks` now runs BEFORE `_step_short_summary` in both `app/fireflies/pipeline.py::process_one` and `app/zoom/pipeline.py::process_one`; `_step_short_summary` signature gained `session` so the To-Do builder can query the just-added Task rows. **Named-assignee rule (rule 7):** `::test_task_extraction_prompt_named_assignee_overrides_admin_default` pins the prompt language: «NAMED ASSIGNEE OVERRIDES EVERYTHING. When the transcript explicitly names a person, find that name in `known_employees` and pick that row's slack_user_id. NEVER substitute a different teammate. NEVER fall back to admin when a name was named.» Worked regression examples include «Алине поручено добавить блок reminder» (must NOT land on admin / AI Lead) and «Дима, нужно провести тест» (must NOT land on a different teammate). **Test infra:** `test_pipeline_posts_tg_card_per_extracted_task` filter tightened — task cards filtered by `👤` marker so the new short-summary To-Do (which also contains task descriptions) doesn't double-count as cards. |
