...
class FirefliesPipeline:
    ...
    def _step_detailed_summary(
        ...
        # FR-EC-CRITIC — improve the TRANSCRIPT FIRST (resolve CRM names), so
        # the detailed summary + tasks + short summary are built canonical from
        # the start; re-applied to the summary after canonicalisation so FR wins.
        ...
        if session is not None:
            try:
                ...
                if fr_map:
                    ...
                    log.info("fireflies_fr_transcript_improved",
                             fireflies_id=row.fireflies_id, rewrites=fr_map)
            except Exception as e:  # noqa: BLE001
                log.warning("fireflies_fr_transcript_improve_failed",
                            fireflies_id=row.fireflies_id, error=str(e))
        user_prompt = (
        ...
        if fr_map:
            try:
                ...
                log.warning("fireflies_fr_reapply_failed",
                            fireflies_id=row.fireflies_id, error=str(e))
    ...
    def _step_match_counterparties(
        ...
        if getattr(self._settings, "counterparty_match_v2_mode", "off") == "on":
            return self._recognize_counterparties_catalog(
                row, source_kind="fireflies", source_id=row.fireflies_id)
        directory = (
            session.query(Counterparty)
    ...
    def _recognize_counterparties_catalog(
        self, row: MeetingRecording, *, source_kind: str, source_id: str
    ) -> int:
        """FR-CR-05-241 mode=on — RECOGNIZE-ONLY against the 4000 vector
    ...
    def _step_canonicalize_task_names(
        ...
        # `Counterparty` directory («Amazon.com», «SDF» mangled correct names).
        # Legacy directory only when MEETING_LEGACY_COUNTERPARTY_CANON_ENABLED=true.
        if self._settings.meeting_legacy_counterparty_canon_enabled:
