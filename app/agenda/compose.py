...
- previous_recap: список 2-4 коротких предложений. Это будет отрисовано как сплошной абзац — renderer САМ добавит префикс «На прошлой встрече: » один раз. Каждый item — отдельное предложение, НЕ начинай с «На прошлой встрече» / «На предыдущей встрече» / «В прошлый раз» / любых аналогов (renderer уже это написал). Без маркеров, без нумерации, без эмодзи. ВАЖНО: фокусируйся ТОЛЬКО на темах прошлой встречи, которые касаются текущих attendees (см. поле Attendees выше). Если какая-то тема прошлой встречи не относится ни к одному из текущих участников — пропускай её. Если prior_recordings пусто — верни пустой список.

- open_questions: 0-2 дополнительных пункта повестки (не дубликаты задач) — ТОЛЬКО про текущих attendees, например «Артем — уточнить timeline» или «Ирина — статус по KBW». Без эмодзи, без маркеров. Если в open_tasks уже всё покрыто — верни пустой список.

- tasks_checklist: ВСЕ open_tasks как объекты {task_id, title, description, status, owner, due}. Сохрани task_id, owner, due, status ровно как пришло во input — passthrough. title — обычно совпадает с input, можно слегка переформулировать (без потери смысла) если нужно для читаемости. description — короткое (≤220 символов) объяснение что нужно сделать. Если в input description пусто — выведи пустую строку, ничего не выдумывай. ВАЖНО: open_tasks тебе уже отфильтрован по attendees — НЕ выкидывай задачи, НЕ переписывай owner.
