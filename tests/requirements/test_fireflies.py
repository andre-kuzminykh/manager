...
def test_rerun_wipes_pending_drafts_keeps_confirmed_and_ignored(session):
    """FR-CR-05-128 — `--rerun` (or any second pipeline run for
    the same recording) re-extracts tasks; old PROPOSED /
    EDITED drafts must be deleted first to avoid piling up
    duplicate widgets. CONFIRMED drafts are sacred (operator
    already approved → Task exists → re-run must NOT undo
    that). IGNORED drafts are kept too (audit trail of what
    was rejected).
    """
    from app.fireflies.pipeline import _wipe_pending_meeting_drafts
    from app.models import ActionDraftState

    # Three drafts for the same meeting in different states.
    pending_d = _add_meeting_draft(
        session, source_kind="fireflies", conversation_id="trans-rerun",
        title="Pending — should be wiped",
    )
    confirmed_d = _add_meeting_draft(
        session, source_kind="fireflies", conversation_id="trans-rerun",
        title="Confirmed — must survive",
    )
    confirmed_d.state = ActionDraftState.confirmed
    ignored_d = _add_meeting_draft(
        session, source_kind="fireflies", conversation_id="trans-rerun",
        title="Ignored — must survive",
    )
    ignored_d.state = ActionDraftState.ignored
    # Different meeting — must NOT be touched.
    other_d = _add_meeting_draft(
        session, source_kind="fireflies", conversation_id="trans-other",
        title="Other meeting — leave alone",
    )
    session.flush()

    deleted = _wipe_pending_meeting_drafts(
        session, source_kind="fireflies", conversation_id="trans-rerun",
    )
    assert deleted == 1
    ...
    surviving_titles = {
        (d.payload or {}).get("title")
        for d in session.query(ActionDraft).all()
    }
    assert "Pending — should be wiped" not in surviving_titles
    assert "Confirmed — must survive" in surviving_titles
    assert "Ignored — must survive" in surviving_titles
    assert "Other meeting — leave alone" in surviving_titles


def test_meeting_extract_is_idempotent_on_rerun(
    patched_session_scope, SessionFactory, monkeypatch
):
    """FR-CR-05-128 — running the pipeline twice on the same
    recording (the `--rerun` shape: row.tasks_extracted reset
    to False) creates ONE proposed draft per task, not two.
    Pin: stale pending drafts wiped before re-creation."""
    monkeypatch.setenv("TELEGRAM_ADMIN_USER_IDS", "777")
    from app.config import get_settings
    ...
    try:
        settings = _settings_with_audio_dir()
        client = _FakeFirefliesClient(transcripts=[_fake_transcript("trans-idem")])
        llm = _FakeLLM(tasks=[{"title": "Same task", "owner": "777"}])
        pipeline = FirefliesPipeline(
            settings=settings, client=client, llm_backend=llm,
        ...
        # Reset the «tasks_extracted» flag (rerun shape).
        from app.models import MeetingRecording

        with SessionFactory() as s:
            row = (
                s.query(MeetingRecording)
                .filter(MeetingRecording.fireflies_id == "trans-idem")
                .one()
            )
            row.tasks_extracted = False
            row.tasks_extracted_count = None
        ...
        # Still only ONE proposed draft for «Same task», not two.
        from app.models import ActionDraft, ActionDraftState
        ...
        with SessionFactory() as s:
            ...
            assert len(ff_drafts) == 1
            assert ff_drafts[0].payload["title"] == "Same task"
    finally:
        get_settings.cache_clear()  # type: ignore[attr-defined]
