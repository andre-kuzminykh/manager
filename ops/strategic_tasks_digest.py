...
def _fuzzy_dedupe_tasks(tasks: list[dict], *, llm, model: str) -> tuple[list[dict], int]:
    ...
    system = (
        "Ты дедуплицируешь список задач — убираешь ПОВТОРЫ. Две задачи это "
        "ДУБЛИКАТЫ, если у них ОДИН И ТОТ ЖЕ объект (человек/контрагент/фонд) И "
        "ОДНО И ТО ЖЕ действие — даже если формулировки разные или одна "
        "детальнее другой. Группируй ВСЕ такие повторы.\n"
        "Примеры ДУБЛИКАТОВ (надо в одну группу): оба «follow-up с X»; оба "
        "«отправить блерб Y»; оба «напомнить Z про интро»; оба «письмо с "
        "запросом интро из листа W».\n"
        "НЕ дубликаты (раздельно): один объект, но РАЗНЫЕ действия — напр. "
        "«подготовить материалы к звонку с X» и «отправить инвайт X»; разные "
        "люди/контрагенты. При разных действиях — НЕ группируй."
    )
    user = (
