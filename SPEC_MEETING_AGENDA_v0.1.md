...
> **🟢 LIVE — спека в основном совпадает с кодом (~26/29 FR).** Аудит 2026-06-03 —
> `AUDIT.md` §2. Расхождения:
> - **FR-MA-3.1** — `open_tasks_for_recordings` фильтрует ещё по `extra.direction ∈
>   DIRECTIONS_IMPORTANT` (FR-CR-05-192aa, `service.py:333`), задачи без direction
>   отбрасываются. **Ломает собственный тест спеки.** Решить: задокументировать
>   фильтр в спеке ИЛИ убрать из кода.
> - **§16.3** — `AGENDA_MIN_PRIOR_MEETINGS`: спека 1, код **2** (`config.py:250`).
> - **§12.3** — `limit`: спека 30, код **100**.
> - **FR-MA-3.2** — сорт: `created_at` отсутствует в sort-key.
> - Тестов: 64 (не 21 как заявлено в спеке).
...
| FR-MA-1.19 | Send-time host filter для Zoom + Fireflies post-meeting summary: ZOOM_REQUIRED_EMAIL_STRICT_HOST=true → НЕ присылать в Slack/TG summary где host != operator. Zoom — по `zoom_recordings.host_email`; Fireflies — по `participants[0]` (first participant). NULL host = default deny (mark `short_summary_sent=true`, no retry). (operator-pinned 2026-05-15: «какого хера опять ты присылаешь из zoom и firefiles что-то где не организатор 1@thehumanoid.ai»). | `test_zoom_step_short_summary_skips_when_host_email_not_operator` + `test_zoom_step_short_summary_proceeds_when_host_is_operator` + `test_fireflies_step_send_short_summary_skips_when_first_participant_not_operator` + `test_fireflies_step_send_short_summary_proceeds_when_operator_is_first` |
