...
class ZoomPipeline:
    ...
    def _step_transcribe(
        ...
        # FR-CR-05-153 — operator-pinned: «давай сделай нормально».
        # Zoom's own VTT transcript is FIRST CHOICE — it's
        # generated by Zoom's transcription engine, generally
        # reliable for Russian speech, and immune to Whisper's
        # silent-audio loop hallucinations («Редактор субтитров»,
        # «Университет youtube https://vk.com.ua», etc.).
        # Whisper kept as fallback for the rare case where Zoom
        # didn't produce a VTT (e.g. cloud-recording transcription
        # disabled at the workspace level).
        vtt_url = self._find_vtt_download_url(row)
        if vtt_url:
            vtt_text = self._client.fetch_vtt_transcript(vtt_url)
            if vtt_text and len(vtt_text) > 200:
                log.info(
                    "zoom_vtt_used_primary",
                    zoom_id=row.zoom_id, vtt_chars=len(vtt_text),
                )
                from app.services.trace_log import trace_event as _zte_p
                _zte_p(
                    source="zoom", recording_id=row.zoom_id,
                    event="zoom_vtt_used_primary",
                    vtt_chars=len(vtt_text),
                )
                row.transcript_text = vtt_text
                row.transcribed = True
                row.last_error = None
                return True
            else:
                log.info(
                    "zoom_vtt_primary_unavailable",
                    zoom_id=row.zoom_id,
                    vtt_url_found=bool(vtt_url),
                    vtt_chars=len(vtt_text or ""),
                    fallback="whisper",
                )
        else:
            log.info(
