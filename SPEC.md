...
> 🏛️ **Техническая архитектура вынесена в [`docs/TECHNICAL_ARCHITECTURE.md`](docs/TECHNICAL_ARCHITECTURE.md)**
> (authoritative). Этот документ (SPEC.md) — функциональные требования
> (558× `FR-CR-*`). Связь: FR ссылаются на арх-якоря вида `→ ARCH §N`
...
> **FR-prefix → ARCH-секция индекс** (для FR без inline-якоря; см.
> [BDD_SPEC.md](docs/BDD_SPEC.md) для feature-группировки):
>
> | FR-prefix / тема | ARCH § |
> |---|---|
> | Base passive detection, intent | §6.3 |
> | CR-01 lifecycle / owner / workload | §4, §7 |
> | CR-02 conversational create | §2, §6.3 |
> | CR-5/6 subscriptions / digests | §4 |
> | Google Sheets/Tasks sync | §7 |
> | FR-CR-05-11x..13x Note Taker pipeline | §6.1, §6.2 |
> | FR-CR-05-125/191 counterparty/summary canon | §5 |
> | FR-CR-05-13x..14x participants / owner-fallback | §4, §6 |
> | FR-CR-05-15x doc/transcribe/webhook | §3, §6 |
> | FR-CR-05-16x directions / agenda / briefs | §4, §5 |
> | FR-CR-05-178/184 ephemeral send | §4 |
> | FR-CR-05-192* agenda polish | §4 |
> | FR-CR-05-193* Entity Resolution V2 | §5 (inline-якоря) |
> | FR-CR-05-194/198/199* Slack publish | §4, §2 (inline) |
> | FR-CR-05-195/196/197 pipeline reliability | §6, §8 (inline) |
> | FR-CB-* Counterparty Briefs | §5 (SPEC_COUNTERPARTY_BRIEFS) |
> | FR-CB2-* CEO Brain | §5 (SPEC_CEO_BRAIN_BOT) |
...
| FR-CR-05-151 | Listener auto-retries orphaned / errored Zoom recordings (drop `_zoom_started_at` cutoff). **Operator regression chain:** «как быть уверенeым что ты подтягивашeь свежие звонки и делаешь транскрипты», «как же сделать чтобы автоматом это все происходило», «надо ее подхватить как обработается». **Root cause:** `_maybe_poll_zoom` had a hard cutoff `mt < self._zoom_started_at → skipped_old`. Any recording older than the listener's most-recent restart got dropped — including (a) **orphans** where pipeline started but `process_one`'s outer session rolled back mid-step (e.g. listener rebuild killed the in-flight `migrate_zoom`), (b) **errored** rows with `last_error='no audio_url on Zoom record'` waiting for Zoom to finish processing, (c) **historical** recordings from before listener startup. All three classes silently dropped on every poll, never auto-retrying — operator had to run `migrate_zoom --zoom-id …` manually. **New logic:** listener pre-fetches existing `ZoomRecording` rows for the listed UUIDs in ONE `IN (…)` query, then per-meta: SKIP iff DB row exists AND `tasks_extracted=true` AND `last_error IS NULL` (fully done); otherwise PROCESS (covers new/orphan/errored/in-progress). `process_one` is idempotent — each step short-circuits when its DB flag is already set, so re-running on a half-done row picks up where it left off without re-doing work. Cutoff entirely removed. Per-poll log replaces `skipped_old` with `skipped_already_done` + `retried_orphan` for ops visibility. **Tests** (replaces `test_zoom_listener_poll_skips_recordings_before_startup`, 2 new): `test_zoom_listener_poll_processes_all_unfinished_recordings` (3 metas: done-skipped, orphan-with-error retried, fresh-new processed); `test_zoom_listener_poll_retries_row_with_no_error_but_not_finished` (`audio_downloaded=true, transcribed=true, detailed_summarised=false, last_error=null` → still retried because `tasks_extracted=false`). Suite: 1656 passed. **Operator effect:** listener now self-heals — recordings that get stuck in any partial state retry on every 60s poll. Operator's «новая уже обрабатывается, надо ее подхватить» case: as soon as Zoom marks the file `completed`, the next poll's `_step_download_audio` succeeds, downstream steps cascade, summary lands in TG / Slack. No manual `migrate_zoom` calls needed. The "first deploy with empty DB" backfill concern is naturally solved — `tasks_extracted=true` hits skip the same way. |
| FR-CR-05-192 | **Canonical Counterparty directory seed from operator's curated TSV** (operator-pinned 2026-05-21: «и вот компании список зафиксируй»). The legacy Google Sheet sync (`source='Status outreach'`) populated `counterparties` with 16 rows whose `name` was the **Type** field (Financial/VC, Government, MENA, ...) instead of the actual company name. Each Counterparty hub thus represented a CATEGORY, and the satellite `CounterpartyAttribute.attributes['Company']` carried the real name — so the FR-CR-05-129 `resolve_mentions_to_directory` LLM saw 16 generic category names and returned `None` for every real-world mention like «CDIB Capital» / «Affinity Partners» / «Bosch». Org canonicalization (FR-CR-05-191) was therefore inoperative. The fix: operator pasted the canonical TSV (~200 entries with `Type / Company / Contact Info` columns) to `ops/data/counterparties_canonical.tsv`. New op `ops/populate_counterparties_canonical.py`: (1) parses the TSV via `parse_tsv` — strips `#`-comments, header row, blank Company cells, pure-whitespace rows, tab-less lines; (2) for each `(Type, Company)`, derives `name_normalised = " ".join(name.lower().split())` (same shape FR-CR-05-132 uses for dedupe); (3) INSERT into `counterparties` keyed by `name_normalised` — idempotent on re-run (existing row by normalised name → skip insert, refresh attribute only); (4) UPSERT a `CounterpartyAttribute` row with `source='canonical_seed'` carrying `{Type, Company}` JSON for analytics; (5) NEVER touches `source='Status outreach'` rows from the legacy sync (operator can clean those up later). After running this op the directory in `summary_canonicalize.resolve_organizations_to_counterparties` jumps from 16 categories → ~200 real names, so org-mentions in detailed_summary / short_summary now resolve to canonical forms via FR-CR-05-191. The op accepts `--tsv-path` (default `ops/data/counterparties_canonical.tsv`) and `--dry-run`. | `tests/requirements/test_populate_counterparties_canonical.py::test_parse_tsv_skips_comments_and_header` + `…skips_blank_company` + `…skips_pure_whitespace_rows` + `…strips_whitespace_in_cells` + `…skips_rows_without_tab` + `test_normalise_name_lowercases_and_collapses_whitespace` + `…handles_empty_and_whitespace` + `test_populate_inserts_new_rows` + `test_populate_idempotent_by_normalised_name` + `test_populate_preserves_existing_other_source_attributes` |
| FR-CR-05-200 | **Direction-классификация на ИНГЕСТЕ (gpt-4o-mini), хранится в драфте** (operator-pinned 2026-05-24: «и они уже классифицированы должны быть … чтобы каждая задача с gpt-4-mini в проде классифицировала по тем направлениям что в фильтре»). До этого только Zoom/FF tasks несли `extra.direction` (FR-CR-05-163, классификация в пайплайне). TG/Slack action_drafts классификации не имели — стратегический дайджест вынужден был доклассифицировать на лету каждый запуск. **Fix:** `Orchestrator.create_draft` (единая точка для всех 5 passive call-sites: telegram_ingest ×2, slack_ingest, slack_bot handlers events/shared) после `classification.draft_payload()` для интентов `create_task`/`update_task` зовёт `Orchestrator.classify_draft_direction(payload)` → `task_direction.classify_one_direction(...)` на **gpt-4o-mini** (`settings.openai_model`), пишет результат в `payload["direction"]`. Best-effort: try/except + gate на `settings.openai_api_key` — ошибка/таймаут/отсутствие ключа НЕ блокируют создание драфта (в тестах с пустым ключом — no-op, без сетевых вызовов). Backend кэшируется на инстансе оркестратора (`_cls_backend`). `classify_one_direction` — per-task обёртка над `classify_directions` (один таск за вызов → нет batch-дропов). | `tests/requirements/test_strategic_tasks_digest.py::test_fr_cr_05_200_classify_one_direction` + `…create_draft_classifies_task_intent` + `…create_draft_skips_without_api_key` · → ARCH §4,§5 |
| FR-CR-05-201 | **Стратегический утренний дайджест — фильтр + LLM-рендер по направлениям/контрагентам/функциям** (operator-pinned 2026-05-24: «надо задач с 22 мая отфильтровать по фильтру и с ними уже работать», «сделай другие разбивки по тем что стоят в задачах типа Tether», «задачи должны быть полные понятные», «имена нормальные в скобках … из таблицы с командой мэтчить»). `ops/strategic_tasks_digest.py` (read-mostly): (1) тянет `action_drafts` state=proposed в окне дат; (2) `_assign_directions` использует СОХРАНЁННЫЙ `payload["direction"]` (FR-CR-05-200); бэклог без direction доклассифицирует gpt-4o-mini батчами по 5 + дозапрос пропусков, и **сохраняет** обратно в payload (один раз); фильтр ВКЛЮЧЁН по умолчанию — оставляет только `DIRECTIONS_IMPORTANT` (FR-CR-05-163), `--all` отключает; (3) LLM (gpt-5.5, `fireflies_tasks_model`) на каждую задачу даёт `group` (контрагент: Tether/XTX/…), `function` (Follow-up/Интро/Материалы/Звонки/Аутрич/Pipeline/Ресёрч/Прочее), полный самодостаточный `action` (что+кому+что именно, получатель из данных, без выдумок) и `critical` (bool); JSON-per-id, дозапрос пропусков; (4) `_normalize_owner` резолвит ответственного в РЕАЛЬНОЕ имя из `team_members` — по `telegram_username` и по имени (полное/первое слово) → канон `real_name`; `OWNER_OVERRIDES` для бот-аккаунтов (`CEO_office1 bot → Irina Shipilova`); если в таблице нет — отдаёт как есть, без выдумок; (5) `_render`: DIRECTION → именованный ENTITY-кластер (контрагент с ≥2 задачами) → остальное по FUNCTION; сквозная нумерация, заголовок «Задачи на сегодня» (БЕЗ «критичное»), без гиперссылки. | `tests/requirements/test_strategic_tasks_digest.py::test_fr_cr_05_201_render_groups_by_direction_then_entity_then_function` + `…format_chunk_parses_group_function_action_critical` + `…format_all_rerequests_missing` + `…assign_directions_uses_stored_skips_llm` + `…normalize_owner_matches_team_table` + `…owner_override_ceo_office_bot` · → ARCH §4,§5 |
| FR-CR-05-202 | **Slack-доставка: parent=критичные, тред=остальное; крон 8:00 Europe/London, окно «вчера»** (operator-pinned 2026-05-24: «давай мы это отправим в 8 утра по лондону … берем вчерашние задачи … главным сообщением будут показываться самые критичные а потом в треде остальные»). `_post_to_slack(parent_text, rest_text)` шлёт `chat.postMessage`: критичные задачи (LLM `critical` ИЛИ payload `priority` ∈ {high,urgent}) — в родительское сообщение «Задачи на сегодня»; остальные — ответами в тред (title «Остальные задачи», сквозная нумерация через `start_n`). Канал/токен — `--slack-channel`/`AUTO_SEND_TO_SLACK_CHANNEL` + `--slack-token-key`/`ceo_brain_slack_bot_token` (DM). Отправка ТОЛЬКО при `--send` (default = превью, в Slack ничего). `--yesterday` (`_resolve_window`) считает окно = вчерашний календарный день по Europe/London `[вчера 00:00, сегодня 00:00)` (DST-safe через zoneinfo) — для крона в 8:00. Деплой: host-cron `CRON_TZ=Europe/London; 0 8 * * * docker exec manager-zoom-ff-1 python -m ops.strategic_tasks_digest --yesterday --send` (скрипт должен быть в образе — см. §8 prod-drift). | `tests/requirements/test_strategic_tasks_digest.py::test_fr_cr_05_202_resolve_window_yesterday_london` + `…resolve_window_since_until` + `…critical_split_parent_thread` + `…post_to_slack_parent_then_thread` · → ARCH §3,§4 |
| FR-CR-05-208 | **Участники встречи — матчинг календаря по ВРЕМЕНИ (Fireflies переименовывает встречи)** (operator-pinned 2026-05-28: пост встречи «28/05 - Mitsubishi» показал внутреннюю команду вместо реальных участников; «вот участники в календаре — должны быть только они … чтобы это работало всегда»). Fireflies генерирует своё семантическое название («Mitsubishi: роботы…»), которое НЕ совпадает с названием события Google Calendar («Kodai Yamagishi … Zoom call»), поэтому `_find_matching_event` (fuzzy по названию+времени) не находил событие → `calendar_attendees=None` → пост брал ненадёжный `participants` (внутренняя команда + бот `1@thehumanoid.ai`). **Fix**: в `_find_matching_event` добавлен `allow_time_only` — при отсутствии URL/title-матча берём БЛИЖАЙШЕЕ по времени событие в окне ±15 мин, у которого ЕСТЬ участники (события без участников — «bedtime», «Set your working location», «Outside working hours» — пропускаются, это не встречи). `resolve_calendar_attendees_for_fireflies()` зовёт резолвер с `allow_time_only=True`; пайплайн Fireflies (`_populate_calendar_attendees`) использует его. Имена резолвятся по справочнику (`_build_email_to_name_map` → внутренние в каноничные «Jochen Rudat»; внешние `*@mitsubishicorp.com` → displayName). Zoom-путь не тронут (`allow_time_only=False`, title-overlap по-прежнему обязателен). | `test_fireflies_calendar_time_match.py::test_fr_cr_05_208_time_only_matches_when_title_differs` + `…skips_events_without_attendees` + `…picks_closest` + `…fireflies_resolver_resolves_calendar_names` · → ARCH §6 |
| FR-CR-05-232 | **Slack-ингест: ответственный = адресат (не автор) + чистые имена владельца** (operator-pinned 2026-06-01, по итогам live dry-run на `#legal`: «ответственный → адресат, не автор» + «имена нормальные… из таблицы с командой мэтчить»). Dry-run (`ops/slack_ingest_dryrun.py`) вскрыл два дефекта извлечения: (1) на под-задачах, где LLM не выбрал владельца, цепочка падала на АВТОРА сообщения («kaa») — неверно для делегирующих сообщений «please review/sign @X»; (2) display-name владельца нёс мусор — «[Sterling Law] Ilia Martynov» (префикс конторы), «kaa» (хэндл вместо имени). **Fix в общей `_resolve_owner`** (Telegram не затронут — он не передаёт `mention_uids`): (a) helpers `_clean_display` (срезает скобочный префикс `^[...]`), `_looks_handleish` (single-token lowercase = логин), `_best_employee_display` (предпочитает proper `real_name`, если display — хэндл; всегда чистит префикс) — применяются во всех ветках (LLM-id / name-match / sender); (b) **шаг 2.5 «адресат над автором»**: если LLM не дал владельца, но в сообщении @упомянуты известные коллеги КРОМЕ автора — назначаем первого такого адресата, а не отправителя. Slack-листенер парсит `<@U…>` (`_mention_uids`, dedupe + minus author) и передаёт в цепочку; dry-run — тоже. | `test_owner_addressee_clean_names.py::test_clean_display_strips_bracket_prefix` + `…test_looks_handleish` + `…test_best_employee_display_prefers_clean_human_name` + `…test_resolved_id_uses_clean_display` + `…test_addressee_preferred_over_author_fallback` + `…test_addressee_skips_author_mention` + `…test_no_mentions_keeps_author_fallback` + `…test_addressee_ignored_when_not_in_registry` + `…test_mention_uids_parsing_excludes_author_and_dedups` · → ARCH §4,§5 |
| FR-CR-05-231 | **Чистый дедуплицированный entity-catalog из выгрузки оператора (staging, сборка ллм «понемногу»)** (operator-pinned 2026-05-31: «составь список из вот всего что я скинул, мне надо новую бд сделать и с ллм собрать контекст рядом — мне нужно название, флаг это организация или физлицо, если физлицо то к какой организации относится и в description полное описание, для векторного поиска берём название и описание. Это будет новая таблица без дублей, давай сначала сделаем временную, а потом заменим вместо той» + «понемногу бери контекст, собирай в таблицу без дублей // чтобы контекст не переполнялся»). Оператор скинул TSV-выгрузку ~1.4 МБ (инвесторы/эдвайзеры/компании/люди с био/контактами/LinkedIn) — несколько разнородных таблиц склеены, quoted multi-line ячейки, искажённые формы → column-parser бесполезен, нужен ллм. **Реализация (ADDITIVE, staging-only, прод-директория не трогается)**: миграция `0038_entity_catalog_staging` (revises 0037) — таблица `entity_catalog_staging(id, name, name_normalised, is_org bool, parent_org, description, aliases, mentions_count, created_at, updated_at, UNIQUE(name_normalised,is_org))` + ledger `entity_catalog_ingest(chunk_no PK, row_lo, row_hi, char_len, entities_found, status, note)`. Сервис `app/services/entity_catalog.py`: (1) `iter_source_chunks` — quote-aware TSV-чтение → ДЕТЕРМИНИРОВАННЫЕ size-bounded чанки целых логических строк (multi-line ячейки flatten в ' / '; ~6k chars/чанк = 196 чанков на этой выгрузке) — детерминизм = основа resume; (2) `extract_entities_from_chunk` — один strict-JSON ллм-вызов на чанк (`response_format=json_object`), `_clean_entity` санитизирует (is_org bool, orgs без parent_org, name ≥2 симв); (3) `upsert_entity` — дедуп по `(normalise_name, is_org)`, МЕРЖ: длиннейшая поверхность как canonical (короткая → alias), `merge_description` аккумулирует контекст без дублей (bounded 4000), дозаполняет parent_org, bump mentions_count; (4) `ingest_source` — оркестрация с per-chunk чекпойнтом: уже `done`-чанки пропускаются (idempotent, НОЛЬ повторных ллм-вызовов), `--limit-chunks`/`--start-chunk` для сборки «понемногу», commit после каждого чанка (краш теряет ≤1 чанк). CLI `ops/build_entity_catalog.py` (`--source/--model/--max-chars/--limit-chunks/--start-chunk/--reasoning/--dry-run/--reset`). **Аудит near-дублей** (operator-pinned 2026-05-31: «мне надо проверить чтобы не было дублей» + «о инвест-арм сохраняй»): UNIQUE-ключ гарантирует НОЛЬ точных дублей, но `find_duplicate_candidates` ловит *похожие* через token-set логику — равные множества токенов / подмножество / `SequenceMatcher.ratio ≥ 0.86` для опечаток; union-find в кластеры; орг/физлицо не смешиваются. **Точность** (по итогам первого живого прогона на 559 строках): (1) blocking исключает generic/arm-слова → орги с общим лишь «capital»/«partners» («Vest Capital» vs «IST Capital») вообще не сравниваются; (2) одиночное имя («Andrew») не поглощает полные тёзки (подмножество требует фамилию с обеих сторон); (3) **инвест-арм сохраняются раздельно** — пара, отличающаяся ТОЛЬКО arm-маркером (`ventures`/`capital`/`fund`), НЕ кластеризуется («Anthropic» vs «Anthropic Capital», «Accenture»/«Accenture Ventures», «Salesforce»/«Salesforce Ventures» — это разные сущности), а более полное юр./описательное имя («Phoenix Court» ⇄ «Phoenix Court Group Limited», «Schwarz» ⇄ «Schwarz Corporate Solutions KG») по-прежнему сливается. Read-only CLI `ops/audit_entity_catalog_dupes.py` печатает кластеры для ревью (не авто-мёржит). Сборка на изолированной тест-БД `slack-task-db-pgv-test`, потом swap на прод (отдельный обратимый шаг — follow-up). | `test_entity_catalog_build.py::test_chunker_is_deterministic_and_covers_all_rows` + `…respects_char_budget` + `…flattens_quoted_multiline_cells` + `…test_extract_parses_and_sanitises` + `…tolerates_code_fences_and_bad_json` + `…test_upsert_dedups_and_accumulates_context` + `…keeps_org_and_person_separate` + `…fills_missing_parent_org` + `…test_merge_description_no_dupe_when_contained` + `…test_ingest_is_resumable_and_idempotent` + `…dry_run_makes_no_calls_and_no_writes` · → ARCH §6 |
| FR-CR-05-230 | **Auto-enrollment kill-switch: не плодить карточки контрагентов из несматченных упоминаний** (operator-pinned 2026-05-31: «если мы почистим всю эту базу и я скину реально что там должно быть?» + «отключить создание новых»). Диагностика дублей вскрыла КОРЕНЬ: директорию гадят 3 enrollment-пути (`summary_canonicalize._ensure_counterparties_exist`, `counterparty_enrollment._resolve_or_create_one`, `counterparty_enrollment_batch._resolve_or_create_one`) — каждый при обработке встречи МИНТИЛ новую `Counterparty`, если упоминание не матчилось по точному `name_normalised`. Искажённые Whisper-варианты («Тезером», «миры», «Тезору») никогда не матчатся точно → база накопила alias/дубль-карточки (9 строк «Tether»!). **Fix**: настройка `counterparty_autoenroll_enabled` (default **False**, alias `COUNTERPARTY_AUTOENROLL_ENABLED`); все 3 пути перед созданием проверяют флаг и при OFF возвращают None/0 (не создают). С выключенным флагом директория меняется ТОЛЬКО через Google-Sheet синк (wipe-replace) — оператор готовит чистый канонический список, синк его заливает, энроллмент больше не загрязняет. Existing-match по name_normalised по-прежнему работает (возврат найденного). | `test_counterparty_autoenroll_killswitch.py::test_flag_defaults_off` (default False); `…test_all_three_creation_sites_are_gated` (гейт во всех 3 файлах); `…test_enrollment_returns_none_when_disabled` (OFF → None, 0 новых строк); `…test_enrollment_creates_when_enabled` (ON → создаёт) · → ARCH §6 |
| FR-CR-05-229 | **Гигиена директории на этапе эмбеддинга: не эмбеддить категории-теги и alias-карточки + prune** (operator-pinned 2026-05-31, после end-to-end ревизии: v2 показывал ниже summary-coverage чем v1 — диагностика вскрыла, что причина не в матчере, а в ГРЯЗНОЙ директории `counterparties`. Два класса мусора засоряли vector top-K, т.к. короткие: (1) **категории-теги** утёкшие из sheet column-A label — «Financial/VC», «Partnerships», «MENA», «Intros», «Strategic», «Network» — не компании; (2) **alias/произношение-карточки** напложенные авто-энроллментом из искажённых Whisper-упоминаний — «Mirae/«миры»», «FURTS/«фьюртс»», «PML/«пим»», «Anthropic/«Entropiq»» — дубли реального canonical. Директорию гадят 4 писателя (sheet-sync + 3 enrollment-сервиса), но чистить ИСТОЧНИКИ — отдельный большой заход; здесь — быстрый изолированный фильтр НА ЭТАПЕ ЭМБЕДДИНГА (исходные строки не трогаются, обратимо). **Fix**: `is_embeddable_counterparty(name)` → False если имя содержит ёлочки `«/»` (alias-карта) или в `CATEGORY_TAG_BLACKLIST`; `collect_entity_texts` пропускает такие cp. `refresh_embeddings` теперь ещё и **prune**: удаляет из `entity_embeddings` строки, чья сущность больше не collectable (удалена ИЛИ только что отфильтрована) — scoped по (kind, model), возвращает `pruned` в статистике. Так top-K перестаёт содержать теги/алиасы, v2 выбирает canonical (как v1). | `test_entity_embeddings_service.py::test_alias_pronunciation_cards_not_embeddable` / `…test_category_tags_not_embeddable` / `…test_real_companies_are_embeddable` / `…test_blank_names_not_embeddable`; live-refresh на изолированной тест-БД покажет `pruned=N` + перепрогон трейса · → ARCH §6 |
| FR-CR-05-228 | **End-to-end трейс entity-match: последний Zoom + последний Fireflies, полный путь до saved summary** (operator-pinned 2026-05-31: «мне только на последнем зуме и FF надо провести; в итоге получается детальный саммери с нужными сущностями? мне полный трейс по сущностям проследить»). `ops/review_entity_match.py` расширен: `--source zoom|fireflies|both` (default `both`, берёт LATEST каждого с непустым `transcript_text`), `--zoom-id`/`--fireflies-id` для конкретной записи. На каждом recording в **одном проходе** показывает: (1) длина транскрипта + saved `detailed_summary`; (2) Pass-1 экстракцию с `reasoning=high`; (3) per-mention v1 (whole-directory) и v2 (pgvector top-K + critic gpt-4o); (4) **что уже лежит в `CounterpartyMention` для этого recording** (что прод ранее записал) + per-mention пометку `db?✓/·`; (5) **summary coverage**: какие из v1/v2-resolved имён реально встречаются в saved `detailed_summary` (case-insensitive substring count) и каких в нём нет — отвечает на «попало ли в финальный artefact». Полный JSON-трейс на хост (`--out`). READ-ONLY на БД и summary. Симметрично работает с `MeetingRecording` (FF) и `ZoomRecording` через одинаковый `_walk_one`. | `test_review_entity_match.py::_context_for` (окно ±N вокруг упоминания) + `…::_summary_hit_count` (case-insensitive substring count, safe на None, пропуск пустых имён); live-трейс на изолированной pgvector-тест-БД (operator runbook) · → ARCH §6 |
| FR-CR-05-227 | **Обогащение text_repr контрагентов реальными контекстами встреч из `counterparty_mentions`** (operator-pinned 2026-05-31, развитие FR-CR-05-220 после ревизии: ассимметрия query=mention+context vs стораджа text_repr=только имя приводила к тому, что (1) категории-теги «Partnerships», «MENA», «Intros» доминировали в top-K из-за коротких имён и большего сигнала, (2) alias-карточки «Mirae/«миры»» неотличимы от canonical «Mirae» — у обоих text_repr ровно «слово», (3) 'Objective First' проваливался из top-10 потому что в векторе ничего кроме имени). **Fix**: `build_text_repr_counterparty` теперь принимает `mention_contexts: Iterable[str]` (default `[]`) — до 3 последних контекстов упоминаний (cap 300 chars каждый) из `counterparty_mentions.context`. `collect_entity_texts` одним запросом подтягивает контексты сгруппированные по `counterparty_id` (ORDER BY created_at DESC, id DESC, top-3 на cp). Эффект на корпусе 752/966 mentions: ~1.28 фразы на контрагента → реальная meeting-prose в векторе. Старое поведение (без mention_contexts) полностью сохранено — детерминизм builder'а (sorted keys атрибутов) не нарушен. Hash text_repr меняется → refresh-cron при следующем прогоне переэмбеддит затронутые сущности (контрагенты с историей mentions). Дедуп директории (двойные карточки canonical/alias) — отдельный шаг FR-CR-05-228 как работа с данными. | `test_entity_embeddings_service.py::test_counterparty_text_repr_with_mention_contexts` (контексты попадают в text_repr); `…::test_counterparty_text_repr_caps_mention_contexts` (cap по числу контекстов + по длине каждого) + регрессия — старые builder-тесты не сломаны · → ARCH §6 |
| FR-CR-05-226 | **Извлечение упоминаний (Pass-1) на максимальном reasoning** (operator-pinned 2026-05-31: «вот тут давай максимальный ризонинг» — про `counterparty_extract` шаг). Pass-1 (выцепить ВСЕ искажённые Whisper-формы — «Тезер», «фьюртс», «Шаффлера») — precision-critical и общий для v1+v2, поэтому гоняется с `reasoning_effort=high`. В `review_entity_match.py` флаг `--extract-reasoning` (default `high`) → `extract_counterparty_mentions(reasoning_effort=…)`. Бэкенд прокидывает `reasoning_effort` в OpenAI с graceful-fallback (FR-CR-05-120: если модель отвергает — параметр снимается и повтор). Критик остаётся gpt-4o. | покрытие — существующий `extract_counterparty_mentions` + backend reasoning_effort-plumbing; live-ревизия · → ARCH §6 |
| FR-CR-05-225 | **Критик entity-match на gpt-4o (per-mention) + harness: разведены extract/critic модели, `--limit`/`--skip-v1`** (operator-pinned 2026-05-31: «давай каждого отдельно с gpt-4o» — после того как ревизия на gpt-5.5 заняла ~13с/упоминание ×46). Архитектура остаётся **per-mention** (не батч — лучше трассируется, точечнее), но критик переключён с дефолтного `openai_model` (gpt-5.5) на **gpt-4o**: выбор из ~10 кандидатов не требует gpt-5.5-reasoning, а per-mention fan-out делает дешёвую/быструю модель критичной. Настройка `entity_match_critic_model` (default `gpt-4o`, alias `ENTITY_MATCH_CRITIC_MODEL`). В `review_entity_match.py` разведены `--extract-model` (прод-модель — честный v1-baseline + общий Pass-1) и `--critic-model` (gpt-4o для v2); добавлены `--limit N` (ревизия первых N упоминаний для быстрой итерации) и `--skip-v1` (пропустить дорогой baseline). | покрытие — FR-CR-05-222/224 (матчер/harness не изменились структурно, только модель + CLI-флаги); live-ревизия на изолированной тест-БД с gpt-4o · → ARCH §6 |
| FR-CR-05-224 | **Review harness `ops/review_entity_match.py` (read-only): «какие извлеклись → на какие заменились»** (эпик entity-matching, шаг ревизии; operator-pinned 2026-05-31: «попробуем на старом зуме найти кого извлёк и что подставил и почему», «показать конкретно какие сущности извлеклись и на какие заменились»). Берёт один Zoom (`--zoom-id` или `--latest`), извлекает упоминания контрагентов из транскрипта (общий Pass-1 `extract_counterparty_mentions`), затем резолвит КАЖДОЕ двумя путями: **v1** = текущий прод (весь directory → LLM, `resolve_mentions_to_directory`), **v2** = новый pgvector top-K → LangGraph-критик (`match_entity`). Печатает side-by-side таблицу `mention | v1→подставил | v2→подставил | conf` с пометкой `◀ DIFF` на расхождениях + итог agree/diff, и пишет полный JSON-отчёт (`--out`, дефолт `/tmp/entity_match_review.json`) с top-5 кандидатами и reasoning на каждое упоминание. **READ-ONLY** — в БД не пишет; безопасно на проде и на изолированной тест-БД. Контекст критику — окно ±350 символов вокруг упоминания (`_context_for`). | `test_review_entity_match.py::_context_for` — окно центрируется на упоминании, case-insensitive, fallback на голову при отсутствии; live-ревизия (operator runbook) на последнем прод-zoom на изолированной pgvector-тест-БД · → ARCH §6 |
| FR-CR-05-222 | **Agentic entity-matcher: LangGraph-подграф критика (retrieve → critique → disambiguate)** (эпик entity-matching, шаг 4 — сердце). `app/services/entity_match_v2.py`. Граф: `START → retrieve → critique → [confident? → END : disambiguate → retrieve]`. **retrieve**: `retrieve_fn(query, k)` (pgvector top-K), query = mention + до 300 chars контекста; на 2-й попытке K расширяется 10→25. **critique**: LLM-тул `record_entity_match{matched_entity_id|null, confidence, reasoning}` — критик видит mention (часто искажённый Whisper «Адног»→ADNOC), context и ТОЛЬКО кандидатов из ретрива; guard: `matched_entity_id` обязан быть из списка кандидатов или null (галлюцинированный id обнуляется). **routing**: `confidence ≥ 0.6` или `attempt ≥ 2` → final; иначе disambiguate (расширить K, переоценить). no-candidates → мгновенный null без вызова LLM. Результат всегда **аудируемый**: `{kind, mention, matched_entity_id, confidence, reasoning, candidates, attempts}` — для ревизии «извлекли X → подставили Y, почему» (FR-CR-05-224). `retrieve_fn` и `backend` инжектятся → юниты без БД/OpenAI. | `test_entity_match_v2.py` — confident single-pass (attempts=1, K=10), low-conf → disambiguate (K 10→25, 2 critique-вызова, резолв на 2-й), give-up после MAX_ATTEMPTS, invalid-id-from-critic → null, no-candidates short-circuit (LLM не зовётся), результат аудируем (candidates+reasoning), критик-промпт перечисляет кандидатов · → ARCH §6 |
| FR-CR-05-221 | **Cron-скрипт наполнения эмбеддингов `ops/refresh_entity_embeddings.py`** (эпик entity-matching, шаг 3). CLI: `--kinds counterparty,team_member,employee`, `--model` (дефолт из `settings.embedding_model` = `text-embedding-3-large`, alias `EMBEDDING_MODEL`), `--batch-size 256`, `--dry-run` (считает сущности без вызовов OpenAI и без записи). Строит `session_scope()` + `OpenAI(api_key)` → `make_openai_embed_fn` → `refresh_embeddings` (инкрементально по хэшу). Валидирует kinds, без ключа OpenAI выходит с кодом 2. Планируется как `*/10 * * * *` после directory-синков. Добавлена настройка `embedding_model` в config. | покрытие — через FR-CR-05-220 (`refresh_embeddings`/`collect_entity_texts` юнит+pg-gated); live-прогон скрипта на изолированной pgvector-тест-БД (operator runbook): dry-run счёт → реальный embed → повторный прогон embedded=0 · → ARCH §6 |
| FR-CR-05-220 | **Сервис эмбеддингов: text_repr-builders + OpenAI embed + hash-инкрементальный upsert + косинусный поиск** (эпик entity-matching, шаг 2). `app/services/entity_embeddings.py`. **text_repr** (что эмбеддим) per kind: counterparty = `name` + flatten ключевых атрибутов из всех satellite-блобов (sorted keys, скаляры/короткие списки, 1 уровень вложенности, cap 600 chars); team_member = `real_name` + role + @username + notes[:200]; employee = real_name/display_name + title. Всё детерминировано (sorted) → стабильный `text_repr_hash` (sha256). **Инкрементальность**: `refresh_embeddings` грузит существующие хэши по `model`, эмбеддит ТОЛЬКО строки с изменившимся/отсутствующим хэшем (батчи по 256), upsert через `INSERT … ON CONFLICT(kind,entity_id,model) DO UPDATE` с `(:emb)::vector`. Повторный прогон без изменений = 0 вызовов OpenAI. **Поиск**: `search_entities(kind, query_text, k=10)` эмбеддит запрос и возвращает top-K одного kind по `1 - (embedding <=> q)` (косинус-similarity, точный scan). Всё, что ходит в OpenAI, инжектится (`EmbedFn` / `make_openai_embed_fn(client)`) → юнит-тесты без сети. | `test_entity_embeddings_service.py` — builders (name-only, с атрибутами, детерминизм sorted-keys, team_member, employee real_name-приоритет), hash (стабильность + whitespace-различие + длина 64), `_vec_literal` формат, `make_openai_embed_fn` (порядок батча, пустой вход без вызова) + pg-gated `test_refresh_and_search_roundtrip` (реальный pgvector через `TEST_PG_VECTOR_URL`: embed → idempotent re-refresh=0 → cosine search score∈[0,1]) · → ARCH §6 |
| FR-CR-05-219 | **pgvector + таблица `entity_embeddings` (фундамент эпика entity-matching)** (operator-pinned 2026-05-31: «люди, задачи и контрагенты должны лежать в векторных БД отдельно, брать точечно векторно, а не весь скоуп… через pgvector»). Первый шаг эпика: инфраструктура эмбеддингов для directory-сущностей (counterparty/team_member/employee), чтобы Zoom/FF-матчер (FR-CR-05-222) делал точечный косинусный поиск top-K вместо «весь directory в промпт». **Решения** (operator): embedding = OpenAI `text-embedding-3-large` (3072 dim); хранение = ОТДЕЛЬНАЯ таблица (не колонка в исходных) — можно A/B-модели и откат без ALTER; scope = только Zoom/FF; разработка на ИЗОЛИРОВАННОЙ тест-БД `slack-task-db-pgv-test` (образ `pgvector/pgvector:pg16`, отдельный volume, restore прод-дампа) — прод `slack-task-db` (postgres:16-alpine, БЕЗ pgvector) НЕ трогается до выката. **Реализация**: миграция `0037_entity_embeddings` (revises 0036) — `CREATE EXTENSION IF NOT EXISTS vector` + таблица `entity_embeddings(id, kind, entity_id, model, dim, embedding vector(3072), text_repr, text_repr_hash, created_at, updated_at, UNIQUE(kind,entity_id,model))` + btree-индексы на `kind` и `(kind,text_repr_hash)`. **БЕЗ ANN-индекса**: pgvector hnsw/ivfflat капятся на 2000 dim, а 3072 > 2000 → точный косинусный scan (~12.6k строк = sub-50ms); HNSW-через-halfvec оставлен на будущее. ORM-модель `EntityEmbedding` (pgvector.sqlalchemy.Vector). Downgrade дропает только таблицу, extension НЕ трогает (destructive). | `test_entity_embeddings_migration.py` — цепочка ревизий 0037→0036, идемпотентность (IF NOT EXISTS ×2), `vector(3072)`, unique-key, downgrade без DROP EXTENSION, отсутствие ANN-индекса >2000 dim, инварианты модели (dim=3072, kind-константы). Live `alembic upgrade head` — на изолированной pgvector-тест-БД (operator runbook) · → ARCH §6 |
| FR-CR-05-218 | **Дедлайн ставится в ЛЮБОМ случае + date-стадия видит весь контекст (multi-task + chat history)** (operator-pinned 2026-05-31: «дедлайн должен быть до пн вт встретиться, то есть до вторника» + «в любом случае конец этого дня». Регресс — на снапшоте 4965 multi-task split дал 2 задачи, но chunk Андрея «А до этого пусть Андрей поговорит с Олегом 20 мин» не содержит явной даты, поэтому `due_date=None`, а оператор хочет «до встречи Иры», т.е. ≤ 2026-06-01). Корни: (1) `_extract_one_task` передавал в date-стадию ТОЛЬКО `chunk_text` без полного source и без context_messages — LLM не видел дату из chunk 1 и не мог разрешить «до этого»; (2) после всех попыток (LLM/anchor-фильтр/python-resolver) `due_date` мог остаться `None`. **Fix**: (1) `build_date_user_prompt` расширен опциональными блоками `wider_source` (полное сообщение, если chunk ≠ ему) и `prior_context` (последние сообщения чата, обрезаны по 300 chars каждое, последние 10 шт). (2) `node_date` (single-task) теперь передаёт `prior_context=state["context_messages"]`. (3) `_extract_one_task` принимает `full_source` и передаёт его и `context_messages` в date-стадию; anchor-фильтр и `_date_is_acceptable` смотрят и в chunk, и в wider source. (4) `node_date` и `_extract_one_task` в финале — если `picked is None`, ставим `today`. (5) `DATE_SYSTEM_PROMPT` правило 12 «CONTEXT-AWARE DEADLINES»: подзадача-подготовка с маркерами «а до этого / до этого / перед / before that» получает дедлайн ≤ дедлайна зависимой задачи + прайор-контекст для «как договорились»/«as agreed». Worked example — наш регресс (Ира пн/вт, Андрей до этого). | `test_intent_graph.py::test_fr_cr_05_218_today_fallback_when_no_anchor_anywhere` (LLM null + нет якоря → due_date=today); `…::test_fr_cr_05_218_build_date_user_prompt_passes_wider_source` (билдер несёт wider_source + prior_context, не дублирует когда chunk==source); `…::test_fr_cr_05_218_date_prompt_teaches_preparation_dependency` (правило в промпте + worked example); `…::test_date_node_does_not_fall_back_when_llm_intentionally_null` обновлён: today вместо None · → ARCH §4 |
| FR-CR-05-217 | **DETECT-промпт: разные адресаты + разные действия = SPLIT (даже если 2-я часть — «а до этого…»)** (operator-pinned 2026-05-31, развитие FR-CR-05-214/215/216 на том же сообщении 4965: «а на андрея встреча назначается поговорить? то есть тут у одного сообщения две задачи возникает»). Сообщение «Ир поставь встречу в пон или вт … А до этого пусть Андрей поговорит с Олегом Синявским 20 мин» содержит ДВЕ задачи разным адресатам (Ире — встреча, Андрею — разговор), но detect-стадия возвращала `task_count=1`, и задача Андрея растворялась в `description`. Существующая MULTI-TASK SPLITTING секция учила сплитить по «и/and» в одном сказуемом, но не учила сплитить когда «2-я часть оформлена как подготовка к 1-й» («а до этого пусть X сделает Y»). **Fix**: в `app/intent/detect_prompt.py::DETECT_SYSTEM_PROMPT` добавлена секция «ALSO SPLIT WHEN DIFFERENT PEOPLE GET DIFFERENT ACTIONS» с worked example именно нашего регресса (обе цитаты + правильное разбиение на chunks + явный BAD-pattern «Андрей только в description»). Действует на все будущие телеграм/слак-сообщения; черновик 5598 не пересоздаём (уже корректен для Иры). | `test_intent_pipeline.py::test_detect_prompt_splits_different_addressees` (правило FR-CR-05-217 + DIFFERENT PEOPLE + обе цитаты + BAD-pattern в промпте); regression-safety — старые worked-examples («сделать отчёт и презентацию», «к завтра сделать презу») сохранены · → ARCH §4 |
| FR-CR-05-216 | **Дата ОСТАЁТСЯ в title + полное описание (разворот FR-CR-04-5 date-stripping)** (operator-pinned 2026-05-31: «там должно быть полный тайтл, а не просто "поставить встречу", а в описании полные подробности»; выбор стиля — «дата в title + суть», «не резать даты в desc»). Прежняя политика (FR-CR-04-5/88/89) резала любую дату из title через `strip_date_phrase` и тем же резала description → из «поставить встречу на пн/вт для обсуждения» получалось куцее «поставить встречу», а из описания вылетали «на пн/вт», «20 мин». Оператор хочет читать «когда» и «зачем» прямо в заголовке и видеть полные детали в описании. **Fix**: (1) `pipeline.py` — оба title call-site (`node_describe` + `_extract_one_task`) больше НЕ зовут `strip_date_phrase` ни для title, ни для description — берут LLM-вывод как есть; импорт убран. (2) `classifier.py` prefilter-override fallback тоже не режет дату (`title = source_text[:200]`). (3) `TITLE_SYSTEM_PROMPT` — инструкция «Strip any date» заменена на «KEEP THE TIMEFRAME IN THE TITLE» (срок «на пн/вт», «к пятнице» остаётся в заголовке) + обязательная PURPOSE/тема («для обсуждения»), лимит 80→120 символов, worked-пример «Поставить встречу на пн/вт для обсуждения». Дата ПО-ПРЕЖНЕМУ дублируется в отдельное `due_date` (date-node не тронут). `strip_date_phrase` сохранён в `date_resolver` (+ FR-CR-05-215 фикс) — функция остаётся, просто не вызывается из пайплайна. | `test_intent_pipeline.py::test_pipeline_keeps_date_and_full_description` (бывший `…strips_date…`, переписан под новую политику); `test_prefilter_override.py::test_prefilter_overrides_no_action_for_task_keywords` (startswith-ассерт толерантен) · → ARCH §4,§5 |
| FR-CR-05-215 | **`strip_date_phrase` срезает «или Y» в date-alternative конструкциях + поддержка «в/во» перед weekday** (operator-pinned 2026-05-31, развитие FR-CR-05-214: при повторной экстракции той же задачи title LLM выдавал отличный `"поставить встречу на понедельник или вторник"`, но `strip_date_phrase` срезал только первую дату-фразу — `"на понедельник"` — и оставлял орфан `"или вторник"`, получая `"поставить встречу или вторник"`; то же ломало description). Корень в `app/intent/date_resolver.py::strip_date_phrase`: основной цикл `re.sub(..., count=1)` выходит после первого совпадения; «или вторник» / «or Tuesday» — отдельная дата-фраза, но без preposition не подпадала. Второй смежный баг: `_RU_PREPS = к(?:о)?|до|на|ко` НЕ включал `в/во`, поэтому «в пятницу» / «во вторник» не срезалось целиком. **Fix**: (1) после основного цикла добавлен sweep по orphan-паттернам `\b(или|и|or|and)\s+<weekday|N month|today/tomorrow|N.N>`, охватывающий RU/EN weekday/month/относительные/числовые формы; (2) для weekday введён отдельный prefix-набор `_RU_PREPS_WD = к(?:о)?|до|на|ко|в(?:о)?` — `в` добавлен ТОЛЬКО для weekday, чтобы не съесть «в этом окружении» / «в комнате». | `test_date_resolver.py::test_strip_date_phrase` — новые кейсы: «понедельник или вторник», «в пятницу или субботу», «Monday or Tuesday», «сегодня или завтра», «today or tomorrow» + anti-regression на «в этом окружении» / «в комнате 5» · → ARCH §4 |
| FR-CR-05-214 | **Title/description на gpt-4o (intent остаётся gpt-4o-mini) + запрет выдуманных имён** (operator-pinned 2026-05-29: задача из телеги «Ир поставь встречу в пон или вт» (автор = Артём Соколов) превратилась в «Игорь попросил Ирину назначить встречу» — никакого Игоря ни в сообщении, ни в 39 сообщениях истории не было; «сделаем интент 4o-mini, title/desc понятные с gpt-4o»). Корень: title/description-стадия (`title_prompt`) крутилась на том же `openai_model` = **gpt-4o-mini** (в prod через env), который галлюцинировал имя. **Fix**: (1) новый параметр `openai_title_model` (default **gpt-4o**, alias `OPENAI_TITLE_MODEL`); `classify_with_backend` прокидывает его в `run_pipeline(title_model=…)` (инфраструктура `PipelineState.title_model` + `_call_tool(model=…)` уже была, передавался `None`); оба title call-site (dict-стадия и dataclass-чанки) теперь зовут `_call_tool(..., model=state title_model)`. Intent-detection остаётся на дефолте бэкенда (gpt-4o-mini), date — `openai_date_model` (gpt-4o). (2) В `TITLE_SYSTEM_PROMPT` добавлено жёсткое правило «NEVER INVENT PERSON NAMES» с этим регресс-примером: использовать только имена из source/context, проситель = автор сообщения, «Ир …» — адресат (не разворачивать в чужое имя). | `test_llm_cost_models.py::test_fr_cr_05_214_title_model_default_gpt4o` + `test_intent_pipeline.py::test_title_prompt_forbids_invented_names` + `…test_fr_cr_05_214_intent_pipeline_uses_title_model_for_title_stage` · → ARCH §4,§5 |
| FR-CR-05-213 | **Экспорт — флаг `--all` (лить ВСЕ задачи, не только стратегические)** (operator-pinned 2026-05-28: задача «приоритезировать и подготовить отчёт по AI» (`other`) не попадала в лист, т.к. лист был strategic-only; оператор: «ВСЕ задачи»). **Fix**: флаг `--all` в `ops/sheet_sync_export_tasks.py` снимает фильтр `direction in DIRECTIONS_IMPORTANT` в обоих циклах (drafts + tasks) — льются и `other`, и `None` (категория → `Other`). Без флага поведение прежнее (только стратегические). Крон переведён на `--append --all`. | (флаг-гейт фильтра; покрытие — существующие тесты экспорта) · → ARCH §3 |
| FR-CR-05-212 | **Стратегический экспорт — режим `--append` (дописывать новое, не перезаписывать лист)** (operator-pinned 2026-05-28: «хватит обновлять целую таблицу, надо делать append»). `--replace` стирал ручные правки/статусы в листе при каждом прогоне — для крона не годится. **Fix**: флаг `--append` в `ops/sheet_sync_export_tasks.py` — читает ключи `(title, Added-at)` существующих строк (сырое чтение колонок A+N через Sheets API, не зависит от `read_rows()`), `_filter_new_rows` оставляет только отсутствующие, `append_rows` дописывает их в конец БЕЗ `clear`; существующие строки и правки оператора не трогаются. `--replace` остаётся для разового сброса. Крон гоняет `--append`. | `test_sheet_export_source_link.py::test_fr_cr_05_212_append_filters_already_present_rows` · → ARCH §3 |
| FR-CR-05-211 | **Заголовок задачи — запрет голого местоимения-объекта** (operator-pinned 2026-05-28: «Позвонить им — кому им? почему в тайтле не пишется»). Заголовки вида «Позвонить им» / «Ответить им» / «Call them» проходили: промпт `title_prompt.py` запрещал голый глагол (FR-CR-05-89) и предлог в конце (FR-CR-05-88), но НЕ голое местоимение-объект. Контекст (напр. «Charles Busson и партнёры») есть в описании, но в заголовок не раскрывался. **Fix**: в `TITLE_SYSTEM_PROMPT` добавлено правило — местоимения «им/их/ему/ей/его/them/him/her/it» как объект заголовка запрещены, надо раскрывать в имя из source+context («Позвонить Charles Busson и партнёрам»), а если контекст не называет — добавлять «(уточнить кому)». Действует на slack/telegram-ингест (образ `slack-task-bot`) — применится после пересборки. | `test_intent_pipeline.py::test_title_prompt_forbids_bare_pronoun_object` · → ARCH §4 |
| FR-CR-05-210 | **Дефолт дедлайн-ВРЕМЕНИ = 23:59 (конец дня), + вывод «Deadline time» в листе** (operator-pinned 2026-05-28: «Deadline time должно быть автоматом 23:59 этого дня, если не указано обратное» + «и Deadline time должно выводиться в таблице ceo_brain_tasks»). Раньше дефолт времени дедлайна был 18:00 (FR-CR-05-63). **Fix**: дефолт `time(18,0)` → `time(23,59)` во всех точках создания задач — `task_due.parse_due_time_from_llm(fallback=...)`, `create_task_from_draft` (slack/telegram), `zoom`/`fireflies` пайплайны (LLM-fallback + meta-рендер). Явно указанное время по-прежнему сохраняется. QA-детекторы «дефолтного дедлайна» (`quality_checklist`, `trace_record_full`) обновлены с `==18:00` на `==23:59`. **Лист**: `ops/sheet_sync_export_tasks.py` теперь заполняет колонку «Deadline time» через хелпер `_deadline_time(due, due_time)` — явное время, иначе `23:59` если есть дата дедлайна, иначе пусто (для drafts из `payload["due_time"]`, для tasks из `task.due_time`). Тесты дефолта 18:00→23:59 обновлены (`test_task_due`, `test_fr_11_12_persistence`, `test_zoom`). | `test_sheet_export_source_link.py::test_fr_cr_05_210_deadline_time_defaults_to_end_of_day` + `test_task_due.py::test_due_time_default_is_23_59` · → ARCH §4 |
| FR-CR-05-209 | **Cross-source дедуп ловит ПЕРЕИМЕНОВАННУЮ встречу по времени** (operator-pinned 2026-05-28: «зум запись должна не падать, а понимать что эта встреча уже была и не запускать пайплайн»). FR-CR-05-203 матчил по нормализованному названию — но Zoom («Kodai … Zoom call») и Fireflies («Mitsubishi: роботы…») для ОДНОЙ встречи несут разные названия, поэтому Zoom-запись не распознавалась как дубль и падала (`permanent_failure`). **Fix**: в `find_cross_source_duplicate` добавлен второй путь — `time_window_minutes=10`: человек в один момент в ОДНОЙ встрече, поэтому ближайшая уже запостенная запись в ДРУГОМ источнике в пределах ±10 мин — это та же встреча, даже при разных названиях. Только cross-source (`kind != self_kind`), чтобы не схлопнуть легитимную пере-запись того же источника. `process_one` (оба пайплайна) ловит это ПЕРЕД download → второй захват не качается/не падает. | `test_cross_source_dedup.py::test_fr_cr_05_209_time_only_matches_renamed_meeting` + `…time_only_outside_window_proceeds` + `…time_only_is_cross_source_only` · → ARCH §6 |
| FR-CR-05-207 | **Утренний стратегический дайджест — НЕ слать пустое сообщение без задач** (operator-pinned 2026-05-28: «и пока не шли пустое todo без задач»). Крон `strategic_tasks_digest --yesterday --send` шлёт «Задачи на сегодня» в Slack. `main()` уже выходит до отправки, если стратегических задач нет (`if not tasks: return 0` после фильтра направлений), но защита была только на стороне вызывающего. **Fix**: гард на самой границе отправки — `_post_to_slack(parent_text, rest_text, …)` в начале проверяет `if not (parent_text.strip() or rest_text.strip()): return {"ok": False, "skipped": "empty"}` и НЕ делает ни одного `chat.postMessage`. Так пустой дайджест не уйдёт ни при каком пути (вкл. будущие рефакторинги/вызовы). Когда задачи есть — `parent_text` всегда содержит заголовок + контент, поэтому гард реальные дайджесты не трогает. | `test_strategic_tasks_digest.py::test_fr_cr_05_207_post_to_slack_skips_empty` · → ARCH §3,§4 |
| FR-CR-05-206 | **Стратегический экспорт — добавить ОСТАЛЬНЫЕ задачи из встреч/чатов (`tasks`: zoom/ff/slack)** (operator-pinned 2026-05-27: «давай добавляем в эту таблицу остальные» — после FR-CR-05-205 в листе `1h1wCHmr` были только telegram-драфты, ~901 стратегическая задача из встреч отсутствовала). Стратегические задачи из Zoom/Fireflies/Slack живут в таблице `tasks` (direction в `task.extra["direction"]`), а НЕ в `action_drafts` (там `task_id` всегда NULL). **Fix**: `ops/sheet_sync_export_tasks.py` после цикла по `action_drafts` добавляет второй проход по `tasks`: `select(Task).where(deleted_at IS NULL).where(source_kind IN (zoom, fireflies, slack)).where(created_at >= since)`; фильтр `extra["direction"] in DIRECTIONS_IMPORTANT` + непустой title. Мапит в те же 16 колонок: **Status** = реальный статус задачи (`STATUS_DISPLAY_BY_KEY`, фолбэк на `--status`), **Источник** = `Zoom/Fireflies/Slack` (из `source_kind`), **Ссылка** = `google_doc_url` отчёта (join `zoom_recordings`/`meeting_recordings` по `source_conversation_id`; фолбэк `source_permalink`) через новый helper `_source_and_link_for_task(session, task)` (защитный, ошибка БД → фолбэк). Telegram-драфты и meeting-tasks не пересекаются (в `tasks` нет telegram-стратегических) → слияние дизъюнктно, дедуп не нужен. **Slack-gap → классифицируем ТОЛЬКО НОВЫЕ задачи (operator 2026-05-27 «хочу чтобы новые слак задачи классифицировались и писались в таблицу, только новые»):** живые slack-задачи попадали в `tasks` БЕЗ `direction` (None) → не проходили стратегический фильтр (в листе слака не было). Две причины: (1) `create_task_from_draft` строила `extra` только с `owner_assumed`, теряя `payload["direction"]`; (2) часть slack-задач создаётся через модалку (`views.py` `_ensure_draft_for_submission` → `ActionDraft` напрямую), минуя ингест-классификацию FR-CR-05-200. **Forward-only fix (без массового бэкфилла старых — он перебирал сотни задач по одному LLM-вызову и вешал прогон):** (a) `create_task_from_draft` копирует `payload["direction"]` → `extra["direction"]` (единый chokepoint, meeting-tasks уже несут direction из FR-CR-05-163); (b) `FinalizeService._ensure_direction` в `finalize_draft` (универсальная точка материализации ВСЕХ slack-путей: модалка/@mention/confirm) при ПУСТОМ direction делает ОДИН gpt-4o-mini вызов через новый `task_direction.ensure_direction(payload, …)` и пишет в `draft.payload` ДО `create_task_from_draft`; best-effort, gated на `openai_api_key`, никогда не блокирует создание задачи. Экспорт остаётся **строго read-only** (только читает `direction`, `session.rollback()`), старые неклассифицированные slack-задачи в лист НЕ тянет — только новые. | `test_task_direction_carryover.py::test_fr_cr_05_206_direction_carries_into_task_extra` + `…no_direction_leaves_extra_unset` + `…ensure_direction_sets_when_missing` + `…ensure_direction_noop_when_present` + `…ensure_direction_noop_without_title` + `test_sheet_export_source_link.py::test_fr_cr_05_206_zoom_task_source_and_doc_report` + `…slack_task_uses_permalink` + `…fireflies_task_prefers_doc_report` + `…task_no_recording_falls_back_to_permalink` · → ARCH §3,§4 |
| FR-CR-05-205 | **Стратегический экспорт задач в лист — +2 колонки «Источник» и «Ссылка» (на отчёт)** (operator-pinned 2026-05-27: «стратегические в лист + два столбца — источник и ссылка откуда задача пришла с контекстом»). `ops/sheet_sync_export_tasks.py` льёт `action_drafts` (DIRECTIONS_IMPORTANT) в лист `1h1wCHmr`, но без источника/ссылки. **Fix**: в `app/sheet_sync/config.py` `TASK_COLUMNS` добавлены две колонки в конце — `(«Источник», "source")` + `(«Ссылка», "source_link")` (read-only / export-only; `sheets_client` всё считает из `TASK_COLUMNS` динамически, `feeder` мапит поля явно — round-trip не задет). Экспорт джойнит `ActionDraft.task_id → Task` и заполняет: **Источник** = `Zoom/Fireflies/Slack/Telegram` (из `task.source_kind`); **Ссылка** = для встреч (zoom/ff) — `google_doc_url` отчёта (join `zoom_recordings`/`meeting_recordings` по `source_conversation_id`; фолбэк `source_permalink`), для slack/telegram — `task.source_permalink`. Нет `task_id` → пусто. `assert len(TASK_HEADERS)` обновлён 14→16. | `test_sheet_export_source_link.py::test_fr_cr_05_205_headers_have_source_and_link` + `…zoom_task_links_to_doc_report` + `…slack_task_uses_permalink` + `…no_task_id_empty` · → ARCH §3 |
| FR-CR-05-204 | **LLM-cost: дешёвые модели для не-reasoning задач** (operator-pinned 2026-05-27: «оптимизировать затраты на LLM, где-то не надо 5.5»). Три дефолта моделей понижены с `gpt-5.5`: `OPENAI_DATE_MODEL` → `gpt-4o` (парсинг даты — структурная задача, не reasoning), `OPENAI_DEDUP_MODEL` → `gpt-4o` (семантический дедуп; **НЕ** `gpt-4o-mini` — он пропускал near-dup кейсы, FR-CR-05-102), `FIREFLIES_SHORT_SUMMARY_MODEL` остаётся `gpt-5.5` (качество RU-фразы), но БЕЗ reasoning — call-site `complete_text` не передаёт `reasoning_effort`, поэтому reasoning-токены не тратятся. DATE/DEDUP call-site тоже не передают `reasoning_effort`, поэтому смена на gpt-4o механически безопасна (gpt-4o/mini не поддерживают `reasoning_effort`). Меняются дефолты в `app/config.py` (durable, переживает пересоздание), env-override остаётся. Reasoning-вызовы (`reasoning_extract` high, `entity_matcher` medium, `zoom_participants`) пока остаются на gpt-5.5 — отдельные FR. | `test_llm_cost_models.py::test_fr_cr_05_204_date_model_default_gpt4o` + `…dedup_default_gpt4o_not_mini` + `…short_summary_stays_gpt55` · → ARCH §5,§6 |
| FR-CR-05-203 | **Cross-source Zoom↔Fireflies meeting dedup — второй захват не обрабатываем** (operator-pinned 2026-05-27: встречи, записанные ОДНОВРЕМЕННО Zoom Cloud И ботом Fireflies, давали две независимые строки → обе транскрибировались и постились → дубли в Slack: «27/05 Weekly Top Management» zoom `R22oe36` (90 tasks) + ff `01KSMEJQ` (72); «27/05 POC Goals» zoom `pPT/UPQg` (65) + ff `01KSMQV0` (60)). Per-row идемпотентность (`slack_post_ts`, FR-CR-05-194) это НЕ ловит — строки разные. **Fix**: `app/services/meeting_dedup.py` — `normalize_meeting_title()` (срезает префикс «DD/MM[/YYYY] - », NFKD, lowercase, collapse whitespace; «27/05 - Weekly Top Management meeting» ≡ «Weekly Top Management meeting») + `find_cross_source_duplicate(session, *, title, meeting_date, self_kind, self_id, window_minutes=30)` — ищет в ОБЕИХ таблицах (`zoom_recordings` + `meeting_recordings`) запись той же встречи (нормализованный title совпал И `meeting_date` в пределах ±30 мин) c `slack_post_ts IS NOT NULL` (уже запостена), исключая саму себя. В `process_one()` ОБОИХ пайплайнов, ПОСЛЕ retry-cap (FR-CR-05-196) и ПЕРЕД download/transcribe: `if find_cross_source_duplicate(...): report.skipped_reason='duplicate_other_source'; log; return report` — второй захват НЕ скачивается / не транскрибируется / не постится. First-to-post wins. Helper защитный — любая ошибка БД → `None` (дедуп никогда не блокирует обработку). | `test_cross_source_dedup.py::test_fr_cr_05_203_normalize_strips_date_prefix` + `…helper_matches_other_source` + `…no_duplicate_proceeds` + `…zoom_skips_when_other_source_posted` + `…fireflies_symmetric` · → ARCH §6 |
| FR-CR-05-199 | **V2 publish — БД: ВСЕ tasks, Slack: priority direction только** (operator-pinned 2026-05-22: «я хочу в бд все, а в слак в тред только приоритетные, ну также как мы делали формат, а шорт саммери это пэрент с тайтлом и гиперссылкой, как щас»). Контракт V2 publish для Slack-вывода = **тот же что у legacy auto-publish** (FR-CR-05-119 / FR-CR-05-163): parent message — short_summary с title+hyperlink + «TODO:» trailer; thread reply — только приоритетные направления (DIRECTIONS_IMPORTANT = beta/budget/design/investors/deliverables). В БД при этом сохраняются ВСЕ tasks (включая direction='other') через `_replace_tasks_in_db` — для аудита, истории и будущих agenda lookbacks. **Reusable infra**: `_build_todo_section` принимает kwarg `filter_by_direction: bool = True` (default = legacy filter). `publish_zoom_recording_to_slack` принимает `all_tasks_in_thread: bool = False` (default = legacy). V2 publish зовёт обе с дефолтами — capability «ВСЕ в thread» остаётся в коде для будущих use cases (например debug/audit channel), но V2 production publish использует legacy filter. | `test_slack_publish_all_tasks.py::test_fr_cr_05_199_thread_contains_all_tasks` (capability test: filter_by_direction=False работает) + `…parent_only_important` (filter_by_direction=True фильтрует) + `…backwards_compat_default_filter_only` (default = filter) · → ARCH §4,§2 |
| FR-CR-05-199c | **LLM canonicalizer резолвит email из calendar attendees** (operator-pinned 2026-05-22 после первого V2 publish post: «это один и тот же участник» — про `Артем Соколов` и `1@thehumanoid.ai` в строке Участники). Google Calendar attendees выдают user.resolved_name (имя) ИЛИ email если имя не разрешено. До этого FR `canonicalize_participants_via_llm` передавал в LLM prompt только `real_name` из known_people — email mapping отсутствовал, поэтому email-attendee оставался raw и попадал в «Участники:» как дубликат настоящего имени. **Fix:** `get_humans_for_matcher` дополнительно тянет `tm.email` из TeamMember row и кладёт в dict; LLM canonicalizer prompt теперь рендерит каждого known_person как `real_name=«X» email=«user@domain»` (если email есть). SYSTEM_PROMPT добавлен пункт «EMAIL → real_name через email match» с примером `1@thehumanoid.ai → Артем Соколов`. После этого dedupe сохраняет порядок и собирает уникальный список — `1@thehumanoid.ai` мэппится в `Артем Соколов`, который уже в списке, итог в meeting_participants — один элемент `Артем Соколов`. | `test_team_member_canonical.py::test_fr_cr_05_199c_email_resolves_via_known_people` + `…email_passed_to_llm_prompt` · → ARCH §4,§2 |
| FR-CR-05-199b | **V2 publish parent header — DD/MM-Title hyperlink + Участники** (operator-pinned 2026-05-22: «нет заголовка с гиперссылкой и участников»). Legacy `_step_short_summary` composит row.short_summary с (1) первой строкой `<a href=google_doc_url>DD/MM - <row.title></a>` через `_force_meeting_title_first_line` + `_wrap_short_summary_with_doc_link`, (2) строкой `Участники: имя, имя, ...` (LLM генерирует из meta_block). V2 `reasoning_extract` выдаёт чистое тело без header'а, поэтому `ops/v2_publish_meeting.py` должен **посткомпозировать** parent перед передачей в `publish_zoom_recording_to_slack` как `override_short_summary`. Алгоритм: `composed = TITLE_PLACEHOLDER + Участники-line + V2 body`; `_force_meeting_title_first_line(composed, row.title, row.meeting_date)` заменяет placeholder на «DD/MM - Title»; `_wrap_short_summary_with_doc_link(composed, row.google_doc_url)` оборачивает первую строку в HTML hyperlink. Источник имён участников — `result["meeting_participants"]` (canonical TM real_name'ы из LLM canonicalize, FR-CR-05-193b-7), которые уже отфильтрованы от calendar resources (FR-CR-05-192ac). | `test_slack_publish_all_tasks.py::test_fr_cr_05_199b_v2_publish_includes_title_hyperlink` (anti-regression: использует _wrap_short_summary_with_doc_link + _force_meeting_title_first_line) + `…includes_participants_line` (Участники: + meeting_participants ссылка) · → ARCH §4,§2 |
| FR-CR-05-198 | **V2 publish CLI — overwrite legacy DB + Slack-send в новом стиле** (operator-pinned 2026-05-22: «мне нужно отправить это в слак только отправить раз в бд есть»). Single-shot операторский скрипт `ops/v2_publish_meeting.py` для уже-обработанных meeting'ов в БД: читает row → прогоняет V2 (reasoning extract → canonicalize_participants → matcher → rewrite → apply_task_owner с DELEGATE swap) → soft-delete существующих tasks → insert V2 tasks → перезапись row.detailed_summary/short_summary → classify_directions (FR-CR-05-163) → publish в Slack channel. **Опции**: `--no-publish` (только DB), `--no-db` (только Slack). Поддерживает `--fireflies-id` и `--zoom-id`. Использует существующий `publish_zoom_recording_to_slack` (FR-CR-05-194), сделанный универсальным через автодетект `source_kind`/`source_conversation_id` от полей row (zoom_id → zoom, иначе fireflies_id → fireflies). | manual smoke verification на 5 свежих FF за 22/05 (см. FR-CR-05-199 для test'ов автоматизации); `publish_zoom_recording_to_slack` source detection — auto-tested через manual smoke · → ARCH §4 |
| FR-CR-05-195 | **Fireflies API `duration` field is MINUTES** (operator-pinned 2026-05-22: «новые длинные FF не появляются», investigation revealed 5 fresh 22/05 transcripts with `duration_seconds=19..32` were all real ≥20-minute meetings — Schwarz Corp Solutions, Shorooq, Freuds, etc.). Per Fireflies GraphQL docs, the `duration` field on `Transcript` returns a float in minutes, not seconds. Until now `FirefliesClient.list_transcripts` stored the raw value into `MeetingRecording.duration_seconds`, causing the `min_meeting_seconds=300` (5-minute) gate in `process_one` to wrongly skip every meeting — a 50-minute interview was stored as `duration_seconds=50` which is < 300 → `fireflies_pipeline_skipped_duration_too_short`. **Fix**: multiply by 60 at ingestion: `int(r["duration"] * 60)` when `duration` is a number, preserve `None` and `0` as-is (audio_*.ogg ingestions with no measurable duration). **Backfill migration** `0035_fireflies_duration_to_seconds.py`: `UPDATE meeting_recordings SET duration_seconds = duration_seconds * 60 WHERE duration_seconds > 0 AND duration_seconds < 300`. The threshold `< 300` filters records that were stored as minutes (any value < 5 stored-as-seconds is implausibly short and likely was minutes). After deploy, pipeline correctly identifies real meetings vs aborted ones. | `test_fireflies_duration_units.py::test_fr_cr_05_195_duration_multiplied_by_60_at_ingestion` + `…duration_zero_stays_zero` + `…duration_float_converted_to_int_seconds` + `…duration_none_passthrough` · → ARCH §6 |
| FR-CR-05-196 | **Runaway retries cap** (operator inspection 2026-05-22 revealed `Летучка СЕО Office c Ириной` row `n9knUot2QDeCTtu6G2` had accumulated **138 attempts** with persistent `last_error="transcribe failed: Whisper empty + no VTT fallback"`). Every 60-second poll re-tried the same broken row, wasting Whisper budget and log space; Zia/Artem (73 attempts), Weekly Top Mgmt (71), Fundraising 21/05 (47) similar. **Fix**: module-level constant `MAX_ATTEMPTS_BEFORE_GIVE_UP = 20` exported from both `app.zoom.pipeline` and `app.fireflies.pipeline`. In each `process_one()`, BEFORE the existing skip gates (`duration_too_short`, etc.), add: `if row.attempts >= MAX_ATTEMPTS_BEFORE_GIVE_UP: row.last_error = 'permanent_failure_attempts_exceeded'; report.skipped_reason = 'permanent_failure_attempts_exceeded'; log.info('zoom_pipeline_skipped_permanent_failure' / 'fireflies_pipeline_skipped_permanent_failure', attempts=row.attempts, cap=MAX_ATTEMPTS_BEFORE_GIVE_UP); return report`. Threshold 20 ≈ 20 minutes of retries with default 60s poll interval, enough to absorb transient Whisper / API blips but bounded. Operator can manually reset `attempts=0` to retry if the underlying issue was fixed. | `test_retry_cap_and_sentinels.py::test_fr_cr_05_196_zoom_attempts_20_caps_to_permanent_failure` + `…fireflies_attempts_cap_symmetric` + `…zoom_attempts_19_proceeds` · → ARCH §6,§8 |
| FR-CR-05-197 | **Zoom 24h sentinel skip** (operator inspection 2026-05-22: 4 recordings had `duration_seconds=86400` — exact 24h, e.g. `n9knUot2QDeCTtu6G2` Летучка x2, `2E0VhScRTZe/aKsoew` Артем-Дима, `cZ1Q9gl6R+uhBsUXu5` Family Office). This is a Zoom artifact — typically continuous-recording sessions (Zoom Phone, scheduled but unattended meeting rooms) where Zoom reports the room's full booking window as duration instead of actual meeting length. These rows have no playable audio (or huge unsegmentable audio that Whisper can't handle) but pipeline kept retrying. **Fix**: module-level constant `ZOOM_PHONE_24H_SENTINEL_SECONDS = 86400` in `app.zoom.pipeline`. In `process_one()`, BEFORE attempting download/transcribe: `if row.duration_seconds == ZOOM_PHONE_24H_SENTINEL_SECONDS: row.last_error = 'zoom_phone_24h_sentinel'; report.skipped_reason = 'zoom_phone_24h_sentinel'; log.info('zoom_pipeline_skipped_24h_sentinel', zoom_id=row.zoom_id); return report`. Exact match (`== 86400`), not range — real long meetings (4-6h e.g. Family Office full-day) are kept as-is. Fireflies side не requires symmetric fix (the 24h sentinel is Zoom-specific). | `test_retry_cap_and_sentinels.py::test_fr_cr_05_197_zoom_24h_sentinel_skipped` + `…zoom_normal_duration_proceeds` · → ARCH §6 |
| FR-CR-05-193 | **Universal Entity Resolution V2 — high-level feature** (operator-pinned 2026-05-22: «давай одним размышляющим промтом будем вычленять из транскрипта саммери и список всех задач, а потом над этим мы делаем мэтчинг с ллм промтом людей и контрагентов»). 3-этапный pipeline: Step 1 reasoning LLM → Step 2 matcher LLM → Step 3 deterministic apply. Заменяет старые `_step_detailed_summary` + `_step_short_summary` + `_step_extract_tasks` в Zoom/Fireflies pipelines. Детальные требования — sub-FR 193a* (Step 1), 193b* (Step 2), 193c* (Step 3), 193d* (data layer), 193e* (cache), 193f* (backfill), 193g* (pipeline integration), 193h* (task storage), NFR-193* (non-functional). | покрытие через ID-locked test files перечисленные в каждом sub-FR ниже · → ARCH §5 |
| FR-CR-05-193a-1 | **Step 1: single reasoning LLM call** (gpt-5.5, `reasoning_effort=high`). One call per transcript, no retry inside, no multi-step internal logic. Service `app/services/reasoning_extract.py::extract_summary_and_tasks(transcript, meeting_date, duration_seconds, llm_backend, model)`. | `test_reasoning_extract_prompt.py::test_fr_cr_05_193a_single_llm_call_per_invocation` · → ARCH §5 |
| FR-CR-05-193a-2 | **Step 1 output JSON schema** `{summary_detailed: str, summary_short: str, tasks: [{raw_owner_mention: str, title: str, description?: str, due_date?: ISO-date, priority?: low/medium/high/urgent}]}`. | `test_reasoning_extract_prompt.py::test_fr_cr_05_193a_returns_summary_and_tasks_json` · → ARCH §5 |
| FR-CR-05-193a-3 | **Step 1 НЕ резолвит entities** — Step 1 функция MUST NOT принимать `known_people` / `known_orgs` / `team_members` / `counterparties`. Raw mentions остаются как в транскрипте. | `test_reasoning_extract_prompt.py::test_fr_cr_05_193a_no_resolution_in_step_one` · → ARCH §5 |
| FR-CR-05-193a-4 | **Step 1 safe fallback на пустой/мусорный input** — `transcript=''/None` → return `{summary_detailed:'', summary_short:'', tasks:[]}`, LLM не зовётся. | `test_reasoning_extract_prompt.py::test_fr_cr_05_193a_handles_empty_transcript` · → ARCH §5 |
| FR-CR-05-193a-5 | **Step 1 safe fallback на malformed LLM JSON** — non-JSON / partial → возврат empty structure + warning log, не raise. | `test_reasoning_extract_prompt.py::test_fr_cr_05_193a_handles_malformed_llm_json` · → ARCH §5 |
| FR-CR-05-193a-6 | **Step 1 required task fields** — task в output MUST иметь `raw_owner_mention` + `title`. Tasks без них dropped из result. | `test_reasoning_extract_prompt.py::test_fr_cr_05_193a_each_task_has_required_fields` · → ARCH §5 |
| FR-CR-05-193a-7 | **Step 1 optional task fields** — `due_date` (ISO-date или None), `priority` (low/medium/high/urgent, default `medium`), `description` (str или None). | `test_reasoning_extract_prompt.py::test_fr_cr_05_193a_due_date_priority_optional` · → ARCH §5 |
| FR-CR-05-193a-8 | **Step 1 summary_short distinct from summary_detailed** — обе non-empty, summary_short < summary_detailed по длине (LLM генерит независимо, не префикс). | `test_reasoning_extract_prompt.py::test_fr_cr_05_193a_summary_short_is_subset_or_distinct` · → ARCH §5 |
| FR-CR-05-193b-1 | **Step 2: single matcher LLM call** (gpt-5.5, `reasoning_effort=medium`). One call per (text + raw_owners). Service `app/services/entity_matcher.py::match_entities(text, raw_owners, known_people, known_orgs, llm_backend, model)`. | `test_entity_matcher_people.py` + `test_entity_matcher_orgs.py` (combined) · → ARCH §5 |
| FR-CR-05-193b-2 | **Step 2 people exact match** — raw mention совпадает с `TeamMember.real_name` exactly (case-insensitive) → `tm_real_name=real_name`. | `test_entity_matcher_people.py::test_fr_cr_05_193b_exact_real_name_match` · → ARCH §5 |
| FR-CR-05-193b-3 | **Step 2 people domain-disambiguation via notes** — для ambiguous raw (e.g., «Дима» = Дима Дроздов OR Дмитрий Седов) matcher LLM получает notes ВСЕХ кандидатов и решает по контексту. | `test_entity_matcher_people.py::test_fr_cr_05_193b_disambiguates_via_notes` + `…_passes_notes_to_llm` · → ARCH §5 |
| FR-CR-05-193b-4 | **Step 2 people no_match → None** — если raw не в `known_people`, `tm_real_name=None` (для аудита). Task всё равно создаётся БЕЗ owner. | `test_entity_matcher_people.py::test_fr_cr_05_193b_no_match_returns_none` · → ARCH §5 |
| FR-CR-05-193b-5 | **Step 2 people phonetic translit** — translit RU→EN (`Yulya` → `Юля`), EN→RU (`Jared` → `Джарад`) через LLM matcher. | `test_entity_matcher_people.py::test_fr_cr_05_193b_phonetic_translit` · → ARCH §5 |
| FR-CR-05-193b-6 | **Step 2 summary mentions resolved independently** — упоминания в summary не привязаны к task'у. `summary_replacements_people` содержит ВСЕ matched mentions из summary, не только task owners. | `test_entity_matcher_people.py::test_fr_cr_05_193b_summary_mentions_resolved_independently` · → ARCH §5 |
| FR-CR-05-193b-7 | **Step 2 orgs aliases match** — `Counterparty.aliases` (через `counterparty_attrs.source='aliases'`) использованы для phonetic match (Шеффлер ↔ Schaeffler). | `test_entity_matcher_orgs.py::test_fr_cr_05_193b_orgs_alias_match` · → ARCH §5 |
| FR-CR-05-193b-8 | **Step 2 orgs abbreviation match** — короткие формы («Bain» → «Bain Capital», «KKR» → известный enum). | `test_entity_matcher_orgs.py::test_fr_cr_05_193b_orgs_abbreviation_match` · → ARCH §5 |
| FR-CR-05-193b-9 | **Step 2 orgs cyrillic↔latin** — двусторонний переход. | `test_entity_matcher_orgs.py::test_fr_cr_05_193b_orgs_cyrillic_latin` · → ARCH §5 |
| FR-CR-05-193b-10 | **Step 2 empty inputs safe** — `raw_owners=[]` / `known_people=[]` / `known_orgs=[]` → возврат пустых структур, LLM не зовётся (no cost). | `test_entity_matcher_people.py::test_fr_cr_05_193b_empty_known_people_safe` + `test_fr_cr_05_193b_empty_raw_owners_returns_empty_task_owners` · → ARCH §5 |
| FR-CR-05-193b-11 | **Step 2 invalid LLM JSON safe** — мусорный output → fallback: `task_owners=[{raw_owner, tm_real_name=None}]` для каждого raw, summary_replacements=[]. | `test_entity_matcher_people.py::test_fr_cr_05_193b_llm_invalid_json_safe` · → ARCH §5 |
| FR-CR-05-193b-12 | **Step 2 people TM filter — humans only** — `get_humans_for_matcher(session)` фильтрует bots / inactive / empty real_name. | `test_entity_matcher_people.py::test_fr_cr_05_193b_filters_humans_only` · → ARCH §5 |
| FR-CR-05-193c-1 | **Step 3: deterministic apply — text replace** — regex word-boundary aware replace в `summary_detailed` + `summary_short` по `summary_replacements_people` + `summary_replacements_orgs`. Idempotent (повторный run не меняет). | `test_entity_apply.py::test_fr_cr_05_193c_text_replace_word_boundary` · → ARCH §5 |
| FR-CR-05-193c-2 | **Step 3: task.owner lookup** — `tm_real_name` → SELECT TeamMember WHERE `real_name=:name` → set `owner_display_name=real_name`, `owner_user_id=slack_user_id` (если есть). | `test_entity_apply.py::test_fr_cr_05_193c_owner_lookup_sets_user_id` · → ARCH §5 |
| FR-CR-05-193c-3 | **Step 3: DELEGATE_TASKS_TO apply** — если owner.notes содержит `DELEGATE_TASKS_TO: <name>` AND target существует in TeamMember → swap owner. Используется FR-CR-05-192r marker. | `test_entity_apply.py::test_fr_cr_05_193c_delegate_marker_applied` · → ARCH §5 |
| FR-CR-05-193c-4 | **Step 3: DO_NOT_CALL skip** — если owner.notes содержит `DO_NOT_CALL` (case-insensitive) → owner_display_name=None + extra.skipped_owner. Task создаётся БЕЗ owner. | `test_entity_apply.py::test_fr_cr_05_193c_do_not_call_skip` · → ARCH §5 |
| FR-CR-05-193c-5 | **Step 3: idempotency** — повторный apply того же `summary_replacements` не меняет текст. Маркеры остаются. | `test_entity_apply.py::test_fr_cr_05_193c_idempotent_apply` · → ARCH §5 |
| FR-CR-05-193c-6 | **Step 3: unmatched mentions preserve** — raw упоминания НЕ в `summary_replacements` остаются в тексте как есть. | `test_entity_apply.py::test_fr_cr_05_193c_unmatched_mentions_preserved` · → ARCH §5 |
| FR-CR-05-193c-7 | **Step 3: error in apply doesn't break pipeline** — exception в Step 3 → log warning, persist Step 1 raw result (saммари без canonical replace, owner=None для tasks). | `test_entity_apply.py::test_fr_cr_05_193c_error_safe_fallback` · → ARCH §5 |
| FR-CR-05-193d-1 | **TeamMember.notes DSL parser** — extract `DELEGATE_TASKS_TO: <name>` + `DO_NOT_CALL` markers из freeform notes. Case-insensitive, whitespace-tolerant, single-hop only. | `test_team_member_notes_dsl.py::test_fr_cr_05_193d_parse_delegate_marker` + `…_parse_do_not_call` · → ARCH §5,§7 |
| FR-CR-05-193d-2 | **Counterparty.aliases storage** — `counterparty_attrs` row с `source='aliases'`, `attributes={"aliases":["Шеффлер","Шафлер"]}`. Unique constraint на (`counterparty_id`, `source`). | `test_counterparty_aliases.py::test_fr_cr_05_193d_aliases_storage_schema` · → ARCH §5,§7 |
| FR-CR-05-193d-3 | **get_humans_for_matcher(session)** — фильтр active=True + real_name non-empty + not bot. Возвращает list[dict] для matcher prompt. | `test_entity_matcher_people.py::test_fr_cr_05_193b_filters_humans_only` · → ARCH §5,§7 |
| FR-CR-05-193d-4 | **get_orgs_with_aliases(session)** — JOIN `counterparties` + `counterparty_attrs(source='aliases')`, возвращает list[{cp_id, name, aliases:list[str]}]. | `test_counterparty_aliases.py::test_fr_cr_05_193d_get_orgs_with_aliases` · → ARCH §5,§7 |
| FR-CR-05-193e-1 | **entity_resolution_cache table** — schema `(cache_key sha256 PRIMARY KEY, payload jsonb, created_at, expires_at, hits_count int)`. TTL 7 days default. | `test_entity_resolution_cache.py::test_fr_cr_05_193e_schema_and_ttl` · → ARCH §5,§7 |
| FR-CR-05-193e-2 | **Cache key composition** — `sha256(text + known_people_hash + known_orgs_hash)`. Гарантия что изменение TeamMember/Counterparty инвалидирует кэш. | `test_entity_resolution_cache.py::test_fr_cr_05_193e_cache_key_includes_known_hashes` · → ARCH §5,§7 |
| FR-CR-05-193e-3 | **Cache hit observability** — log event `entity_matcher_cache_hit` / `_miss` с метриками. | `test_entity_resolution_cache.py::test_fr_cr_05_193e_logs_hit_miss` · → ARCH §5,§7 |
| FR-CR-05-193e-4 | **Cache expiration cleanup** — periodic op `ops/cleanup_entity_cache.py` удаляет rows where `expires_at < now()`. | `test_entity_resolution_cache.py::test_fr_cr_05_193e_cleanup_expired` · → ARCH §5,§7 |
| FR-CR-05-193f-1 | **Backfill counterparty aliases — op script** `ops/backfill_counterparty_aliases.py`. LLM batch (50 cp per call, gpt-5.5 mini). | `test_backfill_counterparty_aliases.py::test_fr_cr_05_193f_batch_50_per_call` · → ARCH §5 |
| FR-CR-05-193f-2 | **Backfill idempotent** — skip counterparty if aliases уже есть в counterparty_attrs(source='aliases'). | `test_backfill_counterparty_aliases.py::test_fr_cr_05_193f_skip_already_aliased` · → ARCH §5 |
| FR-CR-05-193f-3 | **Backfill dry-run** — `--dry-run` flag печатает proposed aliases без записи. | `test_backfill_counterparty_aliases.py::test_fr_cr_05_193f_dry_run_no_writes` · → ARCH §5 |
| FR-CR-05-193f-4 | **Backfill LLM mock-able** — service зависит от passed llm_backend, тест mock'ает. | `test_backfill_counterparty_aliases.py::test_fr_cr_05_193f_uses_injected_llm_backend` · → ARCH §5 |
| FR-CR-05-193f-5 | **Backfill empty counterparties → no-op** — 0 rows → no LLM calls, exit 0. | `test_backfill_counterparty_aliases.py::test_fr_cr_05_193f_empty_db_no_calls` · → ARCH §5 |
| FR-CR-05-193g-1 | **Pipeline integration: new _step_extract_via_reasoning** — заменяет `_step_detailed_summary` + `_step_short_summary` + `_step_extract_tasks`. Идёт после `_step_transcribe`, до `_step_save_doc` / `_step_send_summary`. | `test_entity_pipeline_e2e.py::test_fr_cr_05_193g_zoom_full_pipeline` · → ARCH §6 |
| FR-CR-05-193g-2 | **Zoom integration** — `app/zoom/pipeline.ZoomPipeline._step_extract_via_reasoning` вызывается из `process_one`. Idempotent через `ZoomRecording.extracted_via_reasoning bool` flag (новая колонка). | `test_entity_pipeline_e2e.py::test_fr_cr_05_193g_zoom_full_pipeline` · → ARCH §6 |
| FR-CR-05-193g-3 | **Fireflies integration** — `app/fireflies/pipeline.FirefliesPipeline._step_extract_via_reasoning`. Симметрично Zoom. | `test_entity_pipeline_e2e.py::test_fr_cr_05_193g_fireflies_full_pipeline` · → ARCH §6 |
| FR-CR-05-193g-4 | **Feature flag `ENTITY_RESOLUTION_V2_ENABLED`** — default `true`. False → fallback на старые 3 step'a (FR-CR-05-191/192k path). Для zero-downtime rollback. | `test_entity_pipeline_e2e.py::test_fr_cr_05_193g_feature_flag_disabled_uses_legacy` · → ARCH §6 |
| FR-CR-05-193g-5 | **Pipeline idempotent re-run** — если `extracted_via_reasoning=True AND last_error IS NULL`, skip step (используем cache при необходимости). | `test_entity_pipeline_e2e.py::test_fr_cr_05_193g_idempotent_skip` · → ARCH §6 |
| FR-CR-05-193h-1 | **tasks.owner_display_name strict** — только canonical из `TeamMember.real_name` или NULL. Никаких raw emails, никаких unknown mentions. | `test_task_owner_strict.py::test_fr_cr_05_193h_no_raw_email_in_owner` · → ARCH §7 |
| FR-CR-05-193h-2 | **tasks.owner_user_id derived from TM** — если у TeamMember есть slack_user_id или telegram_user_id, копируется в task. | `test_task_owner_strict.py::test_fr_cr_05_193h_owner_user_id_from_tm` · → ARCH §7 |
| FR-CR-05-193h-3 | **tasks.extra.matcher_meta** — JSON `{raw_owner_mention, tm_id, tm_real_name, reasoning, status: 'matched'/'no_match'/'delegated'/'skipped_do_not_call'}` для аудита. | `test_task_owner_strict.py::test_fr_cr_05_193h_matcher_meta_stored` · → ARCH §7 |
| FR-CR-05-193h-4 | **tasks.status='proposed' default** — Step 3 создаёт task с state=proposed (требует Confirm в TG), кроме случаев когда matcher_meta.status='delegated' (тогда сразу 'todo'). | `test_task_owner_strict.py::test_fr_cr_05_193h_default_state_proposed` · → ARCH §7 |
| FR-CR-05-193h-5 | **tasks без owner всё равно создаются** — `owner_display_name=None` + `matcher_meta.status='no_match'`. Для оператор-аудита неизвестных упоминаний. | `test_task_owner_strict.py::test_fr_cr_05_193h_no_match_still_persists_task` · → ARCH §7 |
| FR-CR-05-193h-6 | **tasks с DO_NOT_CALL skip** — `owner_display_name=None` + `matcher_meta.status='skipped_do_not_call'` + `matcher_meta.original_owner=<name>`. | `test_task_owner_strict.py::test_fr_cr_05_193h_do_not_call_persists_with_status` · → ARCH §7 |
| NFR-CR-05-193-1 | **Step 1 wall-time < 60s** для transcript ≤ 60 минут (~30k tokens input). | (perf measured in e2e, not unit-testable directly) |
| NFR-CR-05-193-2 | **Step 2 wall-time < 10s** для batch matcher (50 known_people + 550 known_orgs). | (perf measured in e2e) |
| NFR-CR-05-193-3 | **Step 3 wall-time < 100ms** (pure Python, no LLM). | `test_entity_apply.py::test_fr_cr_05_193c_step3_wall_time_under_100ms` |
| NFR-CR-05-193-4 | **Cost ≤ $0.30 / meeting** — Step 1 (~$0.20 reasoning) + Step 2 (~$0.05 matcher) + Step 3 ($0). | (measured in e2e) |
| NFR-CR-05-193-5 | **Graceful degradation** — LLM unavailable (Step 1 OR Step 2) → log error, pipeline `last_error` set, retry на следующем тике через FR-CR-05-151. | `test_entity_pipeline_e2e.py::test_fr_cr_05_193_nfr5_graceful_degradation_llm_down` |
| NFR-CR-05-193-6 | **Observability** — per-step trace events: `reasoning_extract_started`, `reasoning_extract_done`, `entity_matcher_started`, `entity_matcher_done`, `entity_apply_started`, `entity_apply_done`. | `test_entity_pipeline_e2e.py::test_fr_cr_05_193_nfr6_observability_events_emitted` |
| FR-CR-05-192ac | **Agenda attendees: drop Google Calendar resource attendees**
| FR-CR-05-192ac | **Agenda attendees: drop Google Calendar resource attendees** (operator-pinned 2026-05-22: «надо не писать "- London, UK-5-Artem's Office (1)" и подобный офис»). Google Calendar встречи в Humanoid содержат resource (meeting room) attendees типа `Humanoid Office - London`, `UK-5-Artem's Office (1)` — это переговорные комнаты, не люди. После FR-CR-05-181 attendees resolution через TeamMember+Calendar.invitees они проходили в `candidate.attendees` и попадали в `Участники:` строку Slack-агенды как ложные «участники». Patch: helper `_is_resource_attendee(item)` в `app/agenda/service.py` распознаёт ресурсы по трём признакам — (a) `item['resource'] is True` (официальный Calendar API field), (b) email оканчивается на `.resource.calendar.google.com`, (c) displayName / label содержит маркеры `humanoid office`, `artem's office`, ` office - `, `meeting room`, `переговорная`. `_resolve_attendees(...)` пропускает их через `if _is_resource_attendee(it): continue` ДО resolve label step. Гарантия: только реальные люди (резолвленные через TeamMember/Employee email map или fallback на displayName) попадают в attendees list. Cross-references: FR-CR-05-181 (attendees resolution), FR-CR-05-169 (calendar_factory). Применяется в обоих placeholder — `build_candidates` (Agenda thread «Участники:» line) и downstream компонентах которые читают `candidate.attendees` (compose prompt, doc body). | `tests/requirements/test_agenda_attendees_filter.py::test_fr_cr_05_192ac_drops_resource_true_field` + `…test_fr_cr_05_192ac_drops_resource_email_domain` + `…test_fr_cr_05_192ac_drops_humanoid_office_label` + `…test_fr_cr_05_192ac_drops_artem_office_label` + `…test_fr_cr_05_192ac_drops_meeting_room_label` + `…test_fr_cr_05_192ac_keeps_real_person_attendee` + `…test_fr_cr_05_192ac_handles_string_attendee` + `…test_fr_cr_05_192ac_handles_invalid_input` |
| FR-CR-05-192y-polish | **Runner cutoff = `now - OPERATOR_INGEST_LOOKBACK_HOURS` (default 24h)** — fix for FR-CR-05-192y regression (operator-pinned 2026-05-22: «мне надо новые встречи автоматом подтягивать»). Изначально `started_at = datetime.now(UTC)` снапшот при start runner'а — но каждый рестарт двигал cutoff ВПЕРЁД, и встречи которые случились между двумя restart'ами (например, Ирина 22/05 09:00 UTC при restart в 10:18 UTC после patch'ей) терялись как `skipped_old`. Polish: `started_at = datetime.now(UTC) - timedelta(hours=OPERATOR_INGEST_LOOKBACK_HOURS)` где env default 24h. Restart двигает cutoff forward ТОЛЬКО если последний restart был >24h назад. В day-to-day operation (несколько restart'ов в день) — cutoff остаётся стабильным «за последние 24h». Все ТЕКУЩИЕ встречи дня автоматически захватываются после любого restart. Безопасность: за 24h pipeline не дублирует уже обработанные через FR-CR-05-151 idempotent skip (`tasks_extracted=True AND last_error IS NULL` → skip). Override: `OPERATOR_INGEST_LOOKBACK_HOURS=0` → strict «только после restart» behavior (old FR-CR-05-192y), `OPERATOR_INGEST_LOOKBACK_HOURS=168` → неделя для catch-up. | `tests/requirements/test_zoom_ff_runner_wiring.py::test_fr_cr_05_192y_polish_cutoff_uses_lookback_hours` + `…test_fr_cr_05_192y_polish_default_lookback_24h` + `…test_fr_cr_05_192y_polish_zero_lookback_equals_now` |
| FR-CR-05-192ab | **Agenda thread: open_tasks ТОЛЬКО из последней (= самой свежей) prior-recording** (operator-pinned 2026-05-22: «откуда 100? и из этих надо по фильтру»). До патча `build_candidates` собирал zoom_ids из ВСЕХ prior recordings за `AGENDA_LOOKBACK_DAYS=90` (для Fundraising daily — 29 встреч = 469 открытых задач aggregated). Затем `open_tasks_for_recordings(zoom_ids=[все 29])` дёргал tasks привязанные к ЛЮБОЙ из них и top-100 по priority. Это создавало путаницу: оператор ожидает «task'и которые остались с вчерашней встречи» (33), а получал backlog со всех 29 встреч (LIMIT 100). Patch в `app/agenda/service.py::build_candidates`: после `find_prior_recordings(...)` (возвращает прайорные recordings отсортированные по `meeting_date desc`) — если env `AGENDA_TASKS_FROM_LAST_PRIOR_ONLY` НЕ установлен в `false/0/no/off`, передаём в `open_tasks_for_recordings` только `[prior[0].zoom_id]` (single — самая свежая встреча). Иначе — старое поведение (все prior). Default = last-only (operator-pinned). **Важно**: эта правка касается ТОЛЬКО `open_tasks` (тред агенды). `prior_recordings` (для LLM-compose recap) остаётся полным — там `prior_recordings[0].short_summary` уже сам берёт last (FR-CR-05-192u), а full list нужен для отслеживания серии. Cross-references: FR-CR-05-192aa (direction filter применяется к этим же tasks после изменения), FR-CR-05-167 (attendee filter работает на этом же сокращённом наборе). | `tests/requirements/test_agenda_last_prior_only.py::test_fr_cr_05_192ab_default_returns_first_prior_only` + `…test_fr_cr_05_192ab_env_disabled_returns_all_priors` + `…test_fr_cr_05_192ab_empty_priors_returns_empty` + `…test_fr_cr_05_192ab_single_prior_still_works` + `…test_fr_cr_05_192ab_prior_recordings_full_list_preserved` |
| FR-CR-05-192aa | **Agenda thread выводит только важные направления — фильтр `extra.direction ∈ DIRECTIONS_IMPORTANT`** (operator-pinned 2026-05-22: «все задачи должны попадать в поток, но в слак мы выводим задачи в треде только которые прошли этот фильтр»). До патча `app/agenda/service.py::open_tasks_for_recordings` брал ВСЕ Zoom-задачи привязанные к prior-recordings (для Fundraising daily это 469 строк после миграции legacy + retroactive TG-confirm). Тред агенды получался разлапистым — операционная рутина забивала стратегические follow-up'ы. Решение: ре-используем существующую FR-CR-05-163 классификацию (`app.services.task_direction`: 5 важных категорий — `beta`, `budget`, `design`, `investors`, `deliverables`; всё остальное → `other`). Patch: после `session.execute(stmt).scalars().all()` добавлен Python-side filter `[t for t in rows if isinstance(t.extra, dict) and t.extra.get('direction') in DIRECTIONS_IMPORTANT]`. **Контракт**: (A) ВСЕ задачи продолжают писаться в `tasks` (storage = unfiltered) — `_step_extract_tasks` в обеих pipelines не меняется, `direction='other'` тоже сохраняется; (B) В тред агенды попадают ТОЛЬКО важные — это означает что задачи без direction (`extra` is None / no `direction` key) тоже отсеиваются (LLM не классифицировал → safer to skip); (C) `LIMIT 100` остаётся (FR-CR-05-192y precedence), но применяется после direction-фильтра, так что top-100 теперь только из важных. Operator escape-hatch: env `AGENDA_TASK_FILTER_DIRECTIONS_DISABLED` (`true`/`1`/`yes`/`on`) полностью отключает фильтр — поведение возвращается к pre-patch (все задачи в треде). Backfill для исторических задач — one-shot script вызывает `classify_directions(...)` per recording для всех Zoom-tasks без `extra.direction`, идемпотентно (skip if already classified). После backfill для Fundraising daily 170 unclassified → 154 investors / 9 budget / 7 other; итоговое распределение 425 important vs 44 other из 469 total. Filter применяется только к agenda thread display path — `_attendee_match_keys` / FR-CR-05-167 attendee filter сохраняется как separate downstream operation. Cross-refs: FR-CR-05-163 (classify_directions), FR-CR-05-192y (LIMIT=100 + runner), FR-CR-05-192q check #14 (direction-filter в quality checklist post-factum). | `tests/requirements/test_agenda_direction_filter.py::test_fr_cr_05_192aa_filter_keeps_important_directions` + `…test_fr_cr_05_192aa_filter_drops_other_direction` + `…test_fr_cr_05_192aa_filter_drops_no_direction_tasks` + `…test_fr_cr_05_192aa_env_disabled_returns_all_tasks` + `…test_fr_cr_05_192aa_priority_sort_preserved_after_filter` + `…test_fr_cr_05_192aa_limit_applied_after_filter` + `…test_fr_cr_05_192aa_storage_unaffected_by_filter` |
| FR-CR-05-192y | **Standalone Zoom + Fireflies runner с operator-email фильтром и cutoff "только новые"** (operator-pinned 2026-05-22: «мне надо чтобы легаси только телеграм и слак задачи слушал, а остальное менеджер» + «обрабатывать и выводить только там где есть Артем 1@thehumanoid.ai» + «начинаем только новые подтягивать»). Архитектурное разделение: legacy `slack-task-tg-listener` / `slack-task-slack-ingest` слушают только TG/Slack-сообщения и пишут задачи в `manager-db.tasks` (`source_kind in (slack, telegram)`). НОВЫЙ контейнер `manager-zoom-ff-1` запускает тонкий entrypoint `ops/zoom_fireflies_runner.py` который инстанцирует `FirefliesPipeline` + `ZoomPipeline` БЕЗ TG-loop (раздельные daemon-threads, signal handlers SIGTERM/SIGINT, мейн поток просто ждёт `stop` event). Pre-list-фильтры в каждом тике: (A) **cutoff "только новые"** — `started_at = datetime.now(UTC)` снапшот при старте процесса (через `_meeting_date_utc(md)` helper приводит naive datetimes к UTC); в poll loop'ах FF и Zoom отбрасываются записи с `meeting_date < started_at`, лог считает их как `skipped_old`; гарантия: рестарт runner двигает cutoff ВПЕРЁД (история не подтягивается на ребуте). (B) **operator email filter** — `_operator_email(s)` сначала читает env `OPERATOR_REQUIRED_EMAIL`, иначе fallback на `settings.zoom_required_email`, пустые/пробелы → `None`; для Fireflies строгий post-list фильтр `_ff_has_operator(t, op_email)`: `op_email.lower()` обязан быть в `FirefliesTranscript.participants` (case-insensitive, whitespace-tolerant), иначе skip с трассой `skipped_no_op`; для Zoom тот же email уезжает в `ZoomClient.list_recordings(required_email=..., strict_host=settings.zoom_required_email_strict_host)` — легаси-path уже это умеет. Sender в pipeline = `_NoopSender` со `__getattr__` lambda — гарантия что любой `sender.send_xxx(...)` вызов из pipeline-кода (`send_message` / `answer_callback_query` / etc) не падает, но и ничего не шлёт (никаких побочных уведомлений в Slack/TG). Опциональный `calendar_factory` строится через `build_calendar_credentials_factory_with_sa_fallback(s)` — если не удалось, runner стартует с `calendar_factory=None` и pipeline silently skip'ает enrichment атендов (existing FR-CR-05-169 fallback). Контейнер запускается так: тот же `manager-bot:latest` image, `--volumes-from manager-bot-1` (наследует `/app/secrets`), `--env-file` снимок env manager-bot-1 + 3 overrides (`OPERATOR_REQUIRED_EMAIL=1@thehumanoid.ai` / `ZOOM_REQUIRED_EMAIL=1@thehumanoid.ai` / `ZOOM_REQUIRED_EMAIL_STRICT_HOST=false`), `--network manager_default` (доступ к manager-db). Если ни FF, ни Zoom не enabled (`*_REALTIME_ENABLED=false` или соотв. credentials пусты) — runner логирует `no_pipelines_enabled` и exit-code 2 (контейнер падает фастфейлом, не висит зомби). Cross-references: длительность <5 мин уже отсекается через FR-CR-05-192t (`MIN_MEETING_SECONDS=300`); attendees enrichment через FR-CR-05-169 calendar_factory; orphan-retry для половинно-обработанных записей через FR-CR-05-151 (skip только когда `tasks_extracted=True AND last_error IS NULL`). | `tests/requirements/test_zoom_ff_runner_wiring.py::test_fr_cr_05_192y_operator_email_resolution_priority` + `…test_fr_cr_05_192y_operator_email_empty_returns_none` + `…test_fr_cr_05_192y_ff_has_operator_email_filter_positive` + `…test_fr_cr_05_192y_ff_has_operator_email_filter_negative_email_absent` + `…test_fr_cr_05_192y_ff_has_operator_email_filter_empty_participants` + `…test_fr_cr_05_192y_ff_has_operator_email_filter_case_insensitive` + `…test_fr_cr_05_192y_meeting_date_utc_handles_naive_and_aware` + `…test_fr_cr_05_192y_cutoff_started_at_initialized_to_utc_now` + `…test_fr_cr_05_192y_cutoff_filter_drops_records_before_startup` + `…test_fr_cr_05_192y_noop_sender_safe_for_any_attr_call` |
| FR-CR-05-192w | **CEO Brain bot also runs task classifier** (operator-pinned 2026-05-22: «CEO Brain — это, другого не надо»). Two Slack apps share the manager-bot-1 process: the main slack-bot uses `SLACK_BOT_TOKEN` (handle_message → classify_and_persist), and the CEO Brain bot uses `CEO_BRAIN_SLACK_BOT_TOKEN` (agent responder). The operator DM's only with CEO Brain → the main bot never sees the message → no task drafts. Wired into `app/ceo_brain/slack_handler._attach_handlers`: built the same `Services` bundle the main bot uses (`IntentClassifier`, `ContextRetriever`, `Orchestrator`, `EmployeeDirectory`) + `RateAwareSlackSender` and stored at module scope. After `dispatcher.handle_event` archives the payload, `_maybe_classify_and_persist_task(db, payload)` mirrors `handle_message`'s guards (skip bot-own / subtype `message_changed`/`deleted`/`bot_message`/`channel_join` / text-less w/o audio) and calls `classify_and_persist(..., invocation_type=InvocationType.passive)`. If an `ActionDraft` is returned, `orchestrator.decide_passive` decides post-vs-silent, then `bk.draft_card(...)` is rendered and posted as a thread reply via the sender. Card carries Confirm / Edit / Cancel buttons — Confirm flips draft → Task row with `source_kind=slack`. Best-effort throughout: services-build failure logs `ceo_brain_classifier_services_build_failed` and the agent path still works; classify failure logs `ceo_brain_classify_and_persist_failed`; card-post failure logs `ceo_brain_draft_card_post_failed` but the draft persists. **Hybrid override** — `CEO_BRAIN_TASK_CLASSIFIER_ENABLED` env (default `true` for back-compat); when set to `false`/`0`/`no`/`off` the whole classifier block is skipped and `ceo_brain_task_classifier_disabled_by_env` is logged. Used when a separate stack (e.g. the legacy `slack-task-slack-ingest` container) is the canonical source of Slack-derived tasks — operator pinned this hybrid mode same day («легаси только телеграм и слак задачи слушал, а остальное менеджер»). | `tests/requirements/test_ceo_brain_classifier_wiring.py::test_fr_cr_05_192w_env_flag_default_true` + `…test_fr_cr_05_192w_env_flag_false_strings_recognised` + `…test_fr_cr_05_192w_env_flag_unrecognised_value_defaults_true` + `…test_fr_cr_05_192w_session_rollback_after_archive_unique_violation` (cross-checked from existing `tests/requirements/test_ceo_brain_archive_best_effort.py`) |
| FR-CR-05-192v | **CEO Brain dispatcher: archive write is best-effort + session rollback** (operator-pinned 2026-05-22: «не работает вызов агента когда пишу в бота ни в дм ни в тредах»). Production regression: a `PermissionError` on `/app/traces/slack-archive/<channel>/<date>.jsonl` (host-mounted volume owned by `admin_`, container runs as `app`) crashed `handle_event` → outer `_handle` caught it as `ceo_brain_dispatch_failed` → the responder NEVER fired → the bot looked dead in DMs and thread replies. Same crash later observed on `psycopg.errors.UniqueViolation` against `uq_slack_message_archive_channel_ts` when the live event listener and the history poller raced to insert the same `(channel_id, ts)`. Fix is a two-layer try/except chain: **(A) `app/ceo_brain/dispatcher.handle_event`** wraps the `write_archive(...)` call in try/except; on any exception sets `DispatchResult.archive_failed=True`, calls `session.rollback()` (inner try/except so rollback failure can't break archiving), logs `ceo_brain_archive_write_failed`, and CONTINUES to the responder dispatch — audit trail is best-effort, live agent is paramount. **(B) `app/ceo_brain/archive/service.py`** — the `pg_sink.write` call already catches `SQLAlchemyError` internally and falls back to `jsonl_sink.write_pending`; the polish added `pg_session.rollback()` inside that branch BEFORE the warning log so callers receiving the same session (`classify_and_persist`, responder DB queries) get a clean transaction instead of `PendingRollbackError`. `DispatchResult.archive_failed: bool` added to the dataclass for caller branching. | `tests/requirements/test_ceo_brain_archive_best_effort.py::test_fr_cr_05_192v_archive_failure_still_runs_responder` + `…test_fr_cr_05_192v_archive_success_unchanged` + `…test_fr_cr_05_192v_archive_failure_logs_warning` + `…test_fr_cr_05_192v_dispatch_result_has_archive_failed_field` + `…test_fr_cr_05_192v_archive_failure_rolls_back_session` |
| FR-CR-05-192u | **Agenda compose: no LLM — verbatim passthrough of prior short_summary + open tasks** (operator-pinned 2026-05-22: «"На прошлой встрече:" давай напишем а остальное оставим как есть, то есть никакого ллм»). The original `app.agenda.compose.compose_agenda` called gpt-5.5 to synthesize `previous_recap`, suggest `open_questions`, and tweak task `description`/`title`. Operator's contract is simpler: the agenda IS the previous meeting's short_summary body + the current open-task list, with «На прошлой встрече: » prepended by the slack renderer. New behavior: `compose_agenda` always routes to `_compose_lite`, which (a) calls `_strip_prior_short_summary_body` on `candidate.prior_recordings[0].short_summary` — removes the `<a href="…">DD/MM - Title</a>` first-line wrapper, the «Участники: …» line, and any trailing «TODO:»/«To-Do:» block, decoding HTML entities; (b) emits `previous_recap=[stripped_body]` so the Slack renderer adds the «На прошлой встрече: » prefix exactly once; (c) emits `open_questions=[]` (operator: «остальное оставим как есть»); (d) emits `tasks_checklist` as a deterministic passthrough of `candidate.open_tasks` — `task_id`/`title`/`description`/`owner`/`due`/`status` reproduced verbatim with no rewrites; (e) builds `doc_body_md` with the same template as the LLM mode (just deterministic). The old LLM path is preserved as `_compose_agenda_llm_legacy` for tests/future toggle but is NOT called from production. The public `compose_agenda(candidate, llm_backend=…, model=…)` keeps the signature for source-compat with the runner but ignores both params. Wall time drops from 30-180 sec (gpt-5.5 reasoning_effort=high) to <50 ms. Cost drops to zero. | `tests/requirements/test_agenda_lite_compose.py::test_fr_cr_05_192u_strip_removes_title_wrapper` + `…_strip_removes_participants_line` + `…_strip_removes_trailing_todo_block` + `…_strip_empty_or_missing` + `…_strip_decodes_html_entities` + `…_compose_lite_no_llm_call` + `…_compose_lite_open_questions_always_empty` + `…_compose_lite_tasks_passthrough` + `…_compose_lite_empty_priors` + `…_compose_lite_doc_body_has_recap_and_tasks_table` + `…_public_compose_agenda_ignores_llm_backend` |
| FR-CR-05-192t | **Min-meeting-duration gate — skip recordings shorter than 5 min** (operator-pinned 2026-05-22: «не обрабатывать встречи меньше 5 мин»). Procedural / aborted-call recordings waste LLM budget and produce no actionable output. Locked: `Settings.min_meeting_seconds` (default 300, alias `MIN_MEETING_SECONDS`). Both `app/fireflies/pipeline.FirefliesPipeline.process_one` and `app/zoom/pipeline.ZoomPipeline.process_one` short-circuit BEFORE any LLM step (download / transcribe / detailed / short / tasks) when `row.duration_seconds < threshold`. Skip happens AFTER `_upsert_recording` (DB still has the row for audit / retry) and sets `report.skipped_reason="duration_too_short"`. Threshold=0 disables the gate (operator escape hatch). Boundary semantics: «<» (strict less than) — exactly 300 sec passes through. | `tests/requirements/test_min_meeting_duration.py::test_fr_cr_05_192t_default_threshold_is_300_seconds` + `…test_fr_cr_05_192t_threshold_overridable_via_env` + `…test_fr_cr_05_192t_pipeline_skips_short_meetings_zoom` + `…test_fr_cr_05_192t_pipeline_runs_when_at_or_above_threshold` + `…test_fr_cr_05_192t_threshold_zero_disables_gate` |
| FR-CR-05-192r-live | **Delegate marker also applied in the LIVE pipelines** (`app/fireflies/pipeline._step_extract_tasks` + `app/zoom/pipeline._step_extract_tasks`), not only the ephemeral `ops/send_summaries_19_21` path. Shared helper `app/services/team_members.apply_delegate_marker(owner_user_id, known_employees) → (new_uid, delegate_name)` is the single source of truth. After the LLM picks an `owner_user_id` and the fallback chain resolves it, both pipelines call the helper; when the chosen owner's `notes` carry `DELEGATE_TASKS_TO: <real_name>` AND the delegate exists in `known_employees` with a non-empty `slack_user_id`, the pipeline swaps to the delegate's uid and appends `_delegated_to_<name>` to the `owner_resolution` trace tag. Single-hop only (no recursive lookup). Sanity guard from FR-CR-05-192k preserved: delegate target without a slack_user_id is a silent no-op (caller's fallback chain handles). Operator-pinned 2026-05-22 also fired the one-shot retro op `ops/fix_delegate_artem_to_irina.py` to UPDATE existing DB Task rows (`owner_display_name='Артем Соколов' → 'Ирина Шипилова'`) so historical data matches the new contract. | `tests/requirements/test_apply_delegate_marker.py::test_fr_cr_05_192r_live_delegate_swaps_when_marker_present` + `…test_fr_cr_05_192r_live_delegate_marker_case_insensitive` + `…test_fr_cr_05_192r_live_no_swap_when_marker_absent` + `…test_fr_cr_05_192r_live_no_swap_when_delegate_target_missing` + `…test_fr_cr_05_192r_live_no_swap_when_delegate_has_no_slack_uid` + `…test_fr_cr_05_192r_live_capture_stops_at_period` + `…test_fr_cr_05_192r_live_empty_inputs_safe` + `…test_fr_cr_05_192r_live_source_employee_not_in_known` |
| FR-CR-05-192r | **CEO task delegation via TM notes — never assign tasks to operator-pinned «do not own» teammates** (operator-pinned 2026-05-22: «на артема вообще задачи не ставить, на ирину — это надо в ноутс добавить чтобы знать»). The CEO (Артем Соколов) doesn't own actionable items — they get delegated to assistant Ирина Шипилова. Mechanism: TM row carries a `DELEGATE_TASKS_TO: <real_name>` marker in `notes`. The ephemeral resolver in `ops/send_summaries_19_21._extract_important_tasks_ephemeral` parses the marker via regex `DELEGATE_TASKS_TO:\s*(.+?)(?:\n|$)`, builds a `tm_delegate: dict[real_name → delegate_real_name]` at startup, and after every LLM owner resolution walks the marker chain — if the resolved owner carries a delegate marker AND the delegate exists as a `TeamMember.real_name`, the resolver substitutes the delegate. The check «delegate must itself be a known TM real_name» guards against typos / decommissioned delegates. One-shot retro op `ops/fix_delegate_artem_to_irina.py` (a) INSERTs Ирина Шипилова when missing, (b) appends the marker to Артем's notes idempotently (no-op when «DELEGATE_TASKS_TO» substring already present), and (c) UPDATEs all alive Task rows with `owner_display_name='Артем Соколов'` → `'Ирина Шипилова'` so the `--use-db-tasks` path also picks up the swap before next send. The delegate chain itself is single-hop (no recursive lookup) so a delegate can't accidentally re-delegate to a third party — keeps the rule auditable. | `tests/requirements/test_ephemeral_task_extraction.py::test_fr_cr_05_192r_resolver_swaps_delegate_when_notes_marker_present` + `…test_fr_cr_05_192r_resolver_skips_delegate_when_target_not_in_tm` + `…test_fr_cr_05_192r_resolver_no_delegate_when_marker_absent` + `…test_fr_cr_05_192r_delegate_marker_case_insensitive` |
| FR-CR-05-192q | **20-point quality checklist per meeting summary, locked as the operator's canonical pre-send acceptance gate** (operator-pinned 2026-05-22). Implemented in `ops/quality_checklist.py::_make_check_table`. The checklist runs READ-ONLY against any Zoom/Fireflies record and emits 20 rows with status (`✓` / `✗` / `~` / `—`) + trace/comment. Order of checks is contractual — operator references them by number in pre-send reviews. Block 1 (record metadata): (1) Источник (zoom/fireflies stamped from src), (2) Title (`r.title` non-empty), (3) Документ (`r.google_doc_url` non-empty), (4) Длинное саммари — все обязательные поля («📅 МЕТА» or «Дата и продолжительность» header + «Участники» section + body >1000 chars), (5) Длинное саммари — подробное (≥5000 chars = ✓, 1500-5000 = ~, <1500 = ✗). Block 2 (transcript + canonicalization): (6) Transcript bilingual (≥3% Cyrillic AND ≥3% Latin script, percentages emitted in trace), (7) Контрагенты / люди в длинном — проверены и исправлены (count of TeamMember.real_name + Counterparty.name substrings in detailed_summary + email-leak detection via `\b[a-z0-9_.-]+@thehumanoid\.ai\b` regex; trace lists every leak). Block 3 (slack title): (8) Title link (first line starts with `<a href=…>`, URL emitted). Block 4 (participants): (9) Участники vs Google Calendar (`calendar_attendees` count + resolved_to_TM count), (10) Участники vs Zoom/Fireflies raw (`row.participants` count + first 6 names), (11) Полный список участников (overlap between short_summary «Участники:» line and calendar_attendees ≥ min(6, calendar_resolved), «и другие» marker required when calendar set larger than short line). Block 5 (short summary): (12) Короткое саммари (`row.short_summary` non-empty, chars emitted). Block 6 (To-Do): (13) To-Do — есть задачи если были (DB Task count + actionable-signal regex against transcript: `нужно|надо|подготов|отправ|должен|will|must|need to`), (14) To-Do — отфильтрованы (`extra.direction in DIRECTIONS_IMPORTANT` count vs total, drop-count emitted), (15) To-Do — ответственные из контекста (every task `owner_display_name` ∈ TeamMember; counts in_TM / email-form / unknown), (16) To-Do — задачи из разговора (anchored-to-detailed: each task title+description shares ≥1 multi-char word with detailed_summary), (17) To-Do — сроки если были (count of tasks with `due_date` ≠ default meeting_date 18:00), (18) To-Do — отсутствие сроков явно отмечено (if relative-date markers `завтра|к понедельник|by friday|next week` present in transcript but ALL tasks use default deadline → ~, else ✓). Block 7 (final): (19) Финальная — нет галлюцинаций (combines email-leak + «participants in short line that aren't in either TM or calendar_attendees»), (20) Финальная — uncertain помечены (search for `возможно|вероятно|требует уточнения|uncertain|needs review|tbd` in short_summary; defaults to `~` because short_summary style doesn't carry these markers — operator's manual review responsibility). Status semantics: `✓` automatic verification passed, `✗` automatic verification failed, `~` partial / requires manual review (LLM-stage outputs that can't be machine-verified), `—` check not applicable (e.g., «To-Do owner in TM» when there are 0 tasks). | `tests/requirements/test_quality_checklist.py::test_fr_cr_05_192q_check_01_source_stamped` + `…check_02_title_non_empty` + `…check_03_google_doc_url_required` + `…check_04_detailed_summary_has_mandatory_fields` + `…check_05_detailed_summary_depth_threshold` + `…check_06_transcript_bilingual_3percent_each` + `…check_07_email_leaks_flagged_in_detailed` + `…check_08_title_first_line_has_hyperlink` + `…check_09_participants_vs_calendar` + `…check_10_participants_vs_raw_zoom_fireflies` + `…check_11_full_list_from_all_sources` + `…check_12_short_summary_present` + `…check_13_tasks_present_when_actionable_in_transcript` + `…check_14_tasks_filtered_by_direction_important` + `…check_15_task_owners_in_team_member` + `…check_16_tasks_anchored_to_detailed_summary` + `…check_17_task_deadlines_real_or_default` + `…check_18_missing_deadlines_flagged_when_relative_markers_present` + `…check_19_no_hallucinations_email_leak_or_unverified_participant` + `…check_20_uncertain_markers_default_to_manual_review` |
| FR-CR-05-192k | **Ephemeral task extraction MUST resolve owners via the TeamMember directory, not via raw email mentions in the transcript** (operator-pinned 2026-05-22: «ты должен ходить в таблицу и имена подсовывать в зависимости от Notes и должности» + «надо с задачами туда но не в бд»). Production regression on #1 Object First / Dutchess (19/05): the ephemeral path (`ops/send_summaries_19_21._extract_important_tasks_ephemeral`) used by `send_one_fireflies` + `send_one_zoom` skipped the `known_employees` table the live pipeline's `_step_extract_tasks` passes to the LLM. With no directory in the prompt, the LLM picked the closest matching email mention in the transcript (`sots@thehumanoid.ai`, `jochen@thehumanoid.ai`, `jarc@thehumanoid.ai`) as `task.owner` — 33 out of 33 production tasks landed in Slack with email strings instead of canonical real names. Operator deleted the post. Fix is a two-part contract: **(A) prompt-side** — the user_prompt MUST inline `_render_known_employees_table(as_known_employees(session))` (FR-CR-05-12 columns: slack_user_id, display_name, real_name, role, notes) right before the meeting context, AND the JSON-shape line MUST explicitly state «owner: <ONE of slack_user_id from the table OR exact real_name from the table — NEVER a raw email>». **(B) resolver-side** — the post-LLM walk MUST resolve `task.owner` through a 3-stage lookup: (1) `tm_by_slack[owner]` if `owner` starts with «U», (2) `tm_by_email[owner.lower()]` if `owner` contains `@`, (3) exact `tm_real_names` membership for passthrough. All three maps are prefetched in a single `session_scope` to avoid N+1. Also enforced: `due_date` + `due_time` from the LLM output are forwarded to `_render_tasks_block` instead of the previous hardcoded `today_18:00` fallback (FR-CR-05-185 alignment); description (if present) preferred over title for the task body, capped at 350 chars matching `_build_todo_section`. FR-CR-05-178 mock-mode invariant preserved — no Task rows ever written to DB through this path. | `tests/requirements/test_ephemeral_task_extraction.py::test_fr_cr_05_192k_prompt_includes_known_employees_table` + `…::test_fr_cr_05_192k_prompt_forbids_raw_email_as_owner` + `…::test_fr_cr_05_192k_resolver_maps_email_to_real_name` + `…::test_fr_cr_05_192k_resolver_maps_slack_user_id_to_real_name` + `…::test_fr_cr_05_192k_resolver_passes_through_canonical_real_name` + `…::test_fr_cr_05_192k_resolver_passes_through_unknown_owner` + `…::test_fr_cr_05_192k_render_uses_llm_due_date_when_present` + `…::test_fr_cr_05_192k_render_falls_back_to_today_18_when_no_due_date` + `…::test_fr_cr_05_192k_render_uses_description_then_title` + `…::test_fr_cr_05_192k_render_caps_long_description_at_350` |
| FR-CR-05-189b | **«TODO:» trailer rule** — parent Slack message MUST omit the trailing «TODO:» line whenever no thread reply with tasks will follow. Operator-pinned 2026-05-21: «если их нет не надо писать TODO». Production regression on #4 Bosch: parent had orphan «TODO:» because ephemeral task extraction returned 0 (Bosch is an info-only pitch with no actionables for our team). Fix locked in `ops/_send_helpers.build_parent_raw(body, tasks_text)`: returns `body` as-is when `tasks_text` is empty/None, else `body + "\n\nTODO:"`. Both `send_one_fireflies` and `send_one_zoom` compute `tasks_text` FIRST (existing TODO block from `_split_short_summary` OR ephemeral LLM extraction OR empty when `--no-tasks`), THEN call `build_parent_raw` to compose the parent. The trailer is a contract — it signals the reader «look at the thread below for tasks»; appending it without a thread reply is a lie and gets aggressively tested below. | `tests/requirements/test_send_todo_trailer.py::test_fr_cr_05_189b_empty_tasks_omits_todo_trailer` + `…::test_fr_cr_05_189b_none_tasks_omits_todo_trailer` + `…::test_fr_cr_05_189b_with_tasks_adds_todo_trailer` + `…::test_fr_cr_05_189b_trailer_is_exact_format` + `…::test_fr_cr_05_189b_body_passthrough_with_multiline` |
| FR-CR-05-191b | **Auto-seed Counterparty directory from extracted org mentions** (operator-pinned 2026-05-21: «и из остальных ты всегда будешь так делать»). Production Counterparty table was found broken: only 16 rows, each named after a Type (Financial/VC, Government, etc.) rather than actual companies — `Affinity Partners`, `CDIB`, `Foxconn`, `Bosch`, etc. were all missing, so FR-CR-05-191 org-rewrites resolved 0/200+ extracted mentions. Fix runs in two complementary forms: **(1) retro ops script** `ops/seed_counterparties_from_summaries.py` walks a date range, runs `extract_name_entities` over each `detailed_summary` in parallel (`--workers` ThreadPoolExecutor), filters generic terms (`humanoid`, `company`, `fund`, `bank`, `government`, `capital`, `partners`, …), then find-or-creates a `Counterparty` row per unique `name_normalised`. Supports `--min-occurrences N` to dedupe one-off mentions, `--dry-run` to preview. **(2) pipeline auto-seed**: `canonicalize_summary_text` now defaults `auto_seed_counterparties=True` — every call to canonicalize a detailed/short summary first runs `_seed_counterparties_from_orgs` which is the same find-or-create logic applied to the LLM's freshly-extracted orgs. Result: every future Zoom + Fireflies meeting auto-grows the Counterparty directory as a side effect of generating its summary, so the next meeting's canonicalize step has a richer directory to resolve against. Idempotent via `Counterparty.name_normalised` UNIQUE constraint. Generic-filter list and ≥3-char minimum keep noise («the», «company», bare nouns) out of the directory. | `tests/requirements/test_summary_canonicalize.py::test_canonicalize_summary_text_auto_seeds_new_counterparties` + `…::test_canonicalize_summary_text_seed_skips_generic_terms` + `…::test_canonicalize_summary_text_seed_skips_already_in_directory` + `…::test_canonicalize_summary_text_seed_can_be_disabled` |
| FR-CR-05-191 | **Canonical-name rewriting for detailed_summary + short_summary using TeamMember + Counterparty directories** (operator-pinned 2026-05-21: «и в детальных саммери и в коротких потом все переименовать с учетом таблиц people и контрагентов»). After the LLM produces `detailed_summary` (and again after `short_summary`), both Zoom + Fireflies pipelines MUST call `app.services.summary_canonicalize.canonicalize_summary_text(text, session=…, llm_backend=…, model=…)`. The service runs three deterministic stages: (1) **LLM entity extract** — `extract_name_entities` returns `{people: [...], organizations: [...]}` JSON, including phonetic mis-transcriptions (e.g., «Jared Kinnan» where canonical is «Jarad Cannon»); (2) **Resolve people** — `resolve_people_to_team_members` does case-insensitive exact match then ≥3-char token-overlap fuzzy against active `TeamMember.real_name`, skipping mentions already in canonical form; (3) **Resolve organizations** — `resolve_organizations_to_counterparties` delegates to FR-CR-05-129 `resolve_mentions_to_directory` (LLM-based, battle-tested for phonetic / transliterated forms) against the full `Counterparty` directory. The combined `{mention → canonical}` map then runs through FR-CR-05-129 `canonicalize_text` (regex, longest-mention-first, word-boundary-aware) to produce the rewritten summary. Best-effort: ANY exception in the canonicalization step (LLM timeout, malformed JSON, empty directory) is caught + logged and the pipeline keeps the original summary — never breaks the recording. Trace events: `summary_canonicalize_resolved` (per call: extracted/rewritten counts), `zoom_short_summary_canonicalized` / `fireflies_detailed_summary_canonicalized` (per pipeline integration). Retro fixup: `ops/canonicalize_existing_summaries.py` iterates a date range with `--workers N` ThreadPoolExecutor, `--exclude-title-contains` + `--only-artem` filters mirror `send_batch_chronological`. | `tests/requirements/test_summary_canonicalize.py::test_extract_name_entities_parses_clean_json` + `…strips_code_fences` + `…returns_empty_on_malformed_json` + `…returns_empty_on_blank_text` + `…returns_empty_on_blank_llm` + `test_resolve_people_skips_exact_canonical` + `test_resolve_people_matches_by_last_name_token` + `test_resolve_people_skips_inactive` + `test_resolve_people_empty_mentions` + `test_resolve_people_no_team_members` + `test_resolve_people_picks_best_token_overlap` + `test_resolve_orgs_delegates_to_llm_resolver` + `test_resolve_orgs_skips_null_mapping` + `test_resolve_orgs_skips_canonical_equal` + `test_resolve_orgs_empty` + `test_resolve_orgs_no_directory` + `test_canonicalize_summary_text_end_to_end` + `test_canonicalize_summary_text_no_changes_when_already_canonical` + `test_canonicalize_summary_text_blank_input` + `test_canonicalize_summary_text_llm_extract_failure_is_graceful` |
| FR-CR-05-181 | **Attendees-resolution invariant for Zoom & Fireflies meeting summaries** — locked end-to-end contract welding FR-CR-05-169 (Calendar→People resolve) + FR-CR-05-172 (Zoom-participants reconcile) + FR-CR-05-174 (meta-block prefers calendar_attendees) + FR-CR-05-176 (Fireflies port). **Operator-pinned 2026-05-21**: «зафиксируй этот подход в спецификации, тестами полностью обклей весь код чтобы это было так и всегда так для всех зум встреч саммери». **Invariant**: for ANY ZoomRecording / MeetingRecording with `meeting_date` AND a `calendar_factory` in the pipeline, `_populate_calendar_attendees(row, session)` MUST: (1) fetch Calendar events ±120 min around `meeting_date`; (2) match the recording via Zoom join-URL substring in `event.description` (URL match — strongest) OR title+time fuzzy (≤ 15 min, normalised title containment); (3) resolve each invitee email against `TeamMember` + `Employee` + `Counterparty.attributes['personal_information']['emails']` → `resolved_name` carries the People-table form (Cyrillic if that's how the row was entered), NEVER the raw Calendar `displayName`; (4) for **Zoom only** — pull `ZoomClient.fetch_meeting_participants(uuid)` and reconcile via THREE tiers in order: (a) direct email equality (no LLM), (b) fuzzy token-overlap on resolved/display name vs Zoom user_name (no LLM), (c) gpt-4o-mini call ONLY on stubborn leftovers — REQUIRED for cyrillic↔latin pairs like «Дмитрий Седов» ↔ «Dmitry Sedov». The pipeline MUST construct `openai.OpenAI(api_key=settings.openai_api_key)` and pass it through to `reconcile_with_zoom_participants(openai_client=...)`; without that, cyrillic-latin pairs fall through as `zoom_only` with raw Zoom names — operator regression 2026-05-21; (5) persist the final list on `row.calendar_attendees`. **Downstream invariant**: `build_meta_block_for_summary(row)` MUST render those resolved names in the «Участники: …» line of the LLM prompt for `_step_detailed_summary` — falls back to `row.participants` (raw LLM-from-transcript) only when `calendar_attendees` is empty. **Fireflies sibling** (FR-CR-05-176): the same end-to-end contract minus tier (4) — Fireflies has no `/past_meetings/{uuid}/participants` analogue, so reconcile is skipped. Calendar invitees are trusted as-is. Operator's manual ops-tool `ops/show_attendees_live.py` walks the same 5 steps so any future debug session can verify the invariant for a specific zoom_id without re-running the heavy pipeline. | `…::test_zoom_pipeline_attendees_end_to_end_resolves_through_all_tiers` + `…::test_zoom_pipeline_constructs_openai_client_for_reconcile` + `…::test_fireflies_pipeline_populates_calendar_attendees_end_to_end` + `…::test_fireflies_pipeline_skips_calendar_attendees_when_no_factory` |
| FR-CR-05-180 | Dead-thread blacklist in CEO Brain history poller (operator-pinned 2026-05-21: «флуд thread_not_found в логах»). The poller dragged 2 orphan thread_ts'es through every 1-second tick — each returned `thread_not_found` (parent messages deleted by operator). Result: ~70 `ceo_brain_thread_poll_failed` events per minute drowning every other log line. New in-memory `_dead_threads: set[str]` keyed by `f"{channel}:{thread_ts}"`. On the FIRST `thread_not_found` for a thread, the poller records it as dead and short-circuits subsequent polls for the same key — no further API hits, no further log spam. Other errors (network, 5xx, rate-limit) stay transient and the poller keeps retrying. Set lives in-memory only (resets on bot restart). The archive lookup naturally re-discovers ALIVE threads each tick. | `…::test_history_poller_blacklists_dead_thread_on_thread_not_found` + `…::test_history_poller_does_not_blacklist_on_transient_errors` |
| FR-CR-05-179 | Runtime kill-switches for background pipelines (operator-pinned 2026-05-21: «давай пока остановим все агенды и саммери»). Four env-controlled booleans gate periodic listeners + runners — when False the corresponding background work is a no-op: `ZOOM_REALTIME_ENABLED` (Zoom recordings poller), `FIREFLIES_REALTIME_ENABLED` (Fireflies transcripts poller), `AGENDA_ENABLED` (daily agenda runner), `COUNTERPARTY_BRIEFS_ENABLED` (counterparty briefs runner). All four default to **False** so a fresh deploy with empty `.env` doesn't auto-start any background pipeline; operator must explicitly opt-in each. | `…::test_fireflies_listener_poll_off_when_realtime_disabled` + `…::test_settings_listener_kill_switches_default_to_off` + existing `test_zoom_listener_poll_off_by_default` + `test_runner_disabled_no_op` (agenda) + `test_brief_runner_disabled_no_op` (briefs) |
| FR-CR-05-178 | Mock-mode tasks for manual 19-21 May batch (operator-pinned 2026-05-21: «никакие задачи не создавать в бд по этому всему что щас прогоним ... полностью в моке»). New `--skip-tasks` flag on `ops/reprocess_zoom_summary.py` + default-passed by `ops/batch_reprocess_19_21.py`: sets `row.tasks_extracted=True` BEFORE `process_one`, then monkey-patches ZoomPipeline's `_step_verify_tasks`, `_step_canonicalize_task_names`, `_step_consolidate_tasks`, `_step_classify_directions`, `_step_post_task_cards`, and `_dedupe_meeting_tasks` to no-op lambdas. Result: pipeline runs transcribe → bilingual → calendar_attendees → detailed_summary → counterparty match → Doc export → short_summary as normal, but NEVER inserts a Task row. `ops/send_summaries_19_21.py` extracts tasks EPHEMERALLY at post time (`TASK_EXTRACTION_SYSTEM` + `classify_directions` LLM calls) on `row.transcript_text + detailed_summary`, filters by `DIRECTIONS_IMPORTANT` (FR-CR-05-163), and renders the result directly as the Slack thread reply — no DB persistence. Defensive hard-DELETE of any leaked rows after a successful Slack post. |
| FR-CR-05-177 | OpenAI diarization-model adapter for `app/services/transcription.transcribe_bytes` (operator-pinned 2026-05-21: OpenAI promoted `gpt-4o-transcribe-diarize` out of preview, breaking the existing `whisper-1` flow). Three differences from legacy: (1) `prompt` kwarg rejected with `invalid_request_error: Prompt is not supported for diarization models` — drop it when the model name contains `diarize`; (2) `chunking_strategy="auto"` is REQUIRED for diarization models — inject it conditionally; (3) max audio duration 1400 s/chunk regardless of byte size — apply to ALL non-`whisper-1` models. Plumbed via new `max_duration_seconds` kwarg on `_split_audio_into_chunks`. Plus 300 s OpenAI client timeout so silent hangs surface as failed chunks instead of burning operator wall-clock waiting for the SDK's 10-min default. Diarize calls serialized to `max_workers=1` (parallel diarize on long chunks deadlocked silently in operator's prod). |
| FR-CR-05-176 | Port FR-CR-05-169 calendar_attendees integration to Fireflies pipeline (operator-pinned 2026-05-21: «для финального синтеза мы мэтчим с сущностями из таблицы People и другую для контрагентов»). Same `app.services.calendar_attendees.resolve_calendar_attendees_for_zoom` is reused via duck-typing on the row (`getattr(row, "zoom_meeting_id", None)` → None for `MeetingRecording`, so URL match is skipped and we fall to fuzzy title+time). `FirefliesPipeline.__init__` accepts optional `calendar_factory` kwarg; new `_populate_calendar_attendees` step called from `_step_detailed_summary` right before the meta block is built. New module-level helper `app.fireflies.pipeline.build_meta_block_for_summary` mirrors the Zoom helper: prefers `row.calendar_attendees` over raw Fireflies `participants` for the «Участники: …» line. Unlike Zoom there is NO reconcile step (Fireflies has no join-time participants endpoint — Calendar invitees are trusted as-is). Production wiring: `ops/telegram_listener.py` + `ops/migrate_fireflies.py` thread the same Calendar factory the Zoom path uses. New `ops/reprocess_fireflies_summary.py` canary-parity script for verification. |
| FR-CR-05-173 | Operator-presence gate for short summary post (operator-pinned 2026-05-20: «скидывать только те встречи, где есть 1@humanoid»). The existing FR-CR-05-167 host filter allows posts when `host_email == operator`, but the operator's Zoom room records meetings teammates run without them (recurring rooms, AFK joins). New `_operator_actually_present(row)` proves presence via, in priority: (a) `calendar_attendees` contains an email == `ZOOM_REQUIRED_EMAIL`; (b) `row.participants` (LLM-extracted from transcript) contains «Artem» / «Артем» / «Артём» token; (c) `row.transcript_text` head (first 8000 chars, case-insensitive) carries an «Artem Sokolov» / «Артем Соколов» speaker tag. When none of the signals confirm presence AND the host_email matched (we're inside the operator's room), `_step_short_summary` short-circuits — marks `short_summary_sent=True` and logs `zoom_step_short_summary_skipped_operator_absent` for diagnostics. Doc + tasks_extraction still run (the recording IS the operator's room, the Doc is useful for archival), only the Slack/TG post is suppressed. | `…::test_operator_present_via_calendar_attendees` + `…::test_operator_present_via_participants` + `…::test_operator_present_via_transcript_speaker_tag` + `…::test_operator_absent_skips_short_summary_post` |
| FR-CR-05-172 | Cross-reference Calendar invitees with actual Zoom participants (operator-pinned 2026-05-20: «оставить только тех кто реально подключился + к зуму ещё может кто-то подключиться кого нет в встрече приглашенных»). After `resolve_calendar_attendees_for_zoom` lands the Calendar-resolved list on `row.calendar_attendees`, the zoom pipeline now pulls `/past_meetings/{uuid}/participants` via new `ZoomClient.fetch_meeting_participants(uuid)` and merges via `app.services.calendar_attendees.reconcile_with_zoom_participants(...)`. Three-tier matching: (1) direct `email` equality — cheapest, no LLM; (2) fuzzy word-token intersection on resolved/display name vs Zoom user_name (catches «Ирина Шипилова» @skl.vc → «Ирина Шипилова» @gmail.com); (3) gpt-4o-mini single call ONLY for stubborn leftover pairs on BOTH sides (catches transliteration drift / nickname matches; cap 20 candidates / side). Calendar invitees who didn't join — DROPPED from the final list, surfaced under `unmatched_calendar` for diagnostics. Zoom participants who joined but weren't on the Calendar invite — KEPT in the final list with `zoom_join_method="zoom_only"`; their `user_email` runs through the same `team_members + employees + counterparties` resolver, so a known counterparty who joined as a surprise still gets their canonical name in «Участники». When the Zoom API returns no participants (auth fail, missing scope, fresh recording), the Calendar list is kept verbatim — never empties the rendering surface. New per-attendee fields on `calendar_attendees` JSON: `zoom_join_method` ∈ {email, fuzzy, llm, zoom_only}, `zoom_user_name`, `zoom_user_email`. | `…::test_reconcile_zoom_matches_calendar_invitees_by_email` + `…::test_reconcile_zoom_matches_via_fuzzy_name_without_llm` + `…::test_reconcile_zoom_calls_llm_only_for_leftover_pairs` + `…::test_reconcile_zoom_includes_zoom_only_uninvited_attendees` + `…::test_reconcile_zoom_only_uses_email_resolver_when_available` + `…::test_reconcile_zoom_empty_zoom_keeps_calendar_list` |
| FR-CR-05-170 | Bilingual transcript restoration applied during Zoom ingestion (operator-pinned 2026-05-20). Scope is the summary pipeline — NOT the CEO Brain bot. Triggered in `app/zoom/pipeline.ZoomPipeline._step_transcribe` right after Whisper, BEFORE detailed_summary / short_summary / tasks_extraction. Three steps in `app/services/bilingual_restorer.py`: (1) `should_re_stt_english` — gpt-4o-mini decides if the transcript looks like a bilingual call with garbled English; (2) `re_stt_english_via_whisper` — second Whisper pass at `language="en"` on the SAME audio file (resolves via `row.audio_path`, falls back to on-demand Zoom cloud download); (3) `merge_transcripts_chunked` — gpt-4o reconciles primary + secondary in batches of `ZOOM_BILINGUAL_RECONCILE_BATCH_INPUT_CHARS` (default 30k) so the FULL transcript is always covered regardless of length. Restored text replaces `row.transcript_text`; every downstream consumer (detailed_summary, short_summary, tasks, Doc export, CEO Brain MCP queries via n8n) sees the clean version. Fail-soft on every step — the primary Whisper text stays in place if any sub-step errors. Gated by `ZOOM_BILINGUAL_RESTORATION_ENABLED` (default OFF — operator first verifies via `ops/bilingual_smoke`, then flips on). Env knobs: `ZOOM_BILINGUAL_DETECTOR_MODEL` (gpt-4o-mini), `ZOOM_BILINGUAL_RECONCILER_MODEL` (gpt-4o), `ZOOM_BILINGUAL_WHISPER_MODEL` (whisper-1), `ZOOM_BILINGUAL_RECONCILE_BATCH_INPUT_CHARS` (30000), `ZOOM_BILINGUAL_RECONCILE_MAX_TOKENS_PER_BATCH` (16384). | `…::test_bilingual_restorer_disabled_by_default` + `…::test_bilingual_detector_decides_yes_for_mixed` + `…::test_bilingual_reconcile_merges_via_llm` + `…::test_bilingual_reconcile_chunked_covers_full_transcript` + `…::test_bilingual_restorer_falls_back_when_audio_missing` |
| FR-CR-05-169 | Calendar-driven participants for meeting summaries — replace LLM-extracted-from-transcript «Участники» with attendees resolved from the matching Google Calendar event. **Operator-pinned 2026-05-20:** «zoom митинги должны относиться к календарю, и я из календаря вычленяю участников, мэплю их с таблицей людей и это выдаю в итоге — реальных людей что были на встрече, что к ней привязаны». **Scope:** только саммари-пайплайны Zoom/Fireflies (где сейчас рендерится «Участники:» в Header/Doc/Slack/TG). Никакого отношения к CEO Brain боту. **Currently:** `app.services.zoom_participants.extract_zoom_participants_via_llm` дёргает LLM на transcript+team_members → возвращает массив имён, которые сохраняются в `ZoomRecording.participants` JSON. LLM мангалит имена, может пропустить тихих участников, может ложно включить тех кого упомянули в речи но кто не присутствовал. **New flow** (in `_ensure_team_participants` and `fireflies/pipeline.py` participant-rendering blocks): (1) find matching Calendar event via existing `app.services.calendar_match` patterns (key: Zoom join URL substring match against event.description first, fall back to ±15 min start-time window + fuzzy title match — same logic FR-CR-05-136 already uses for Fireflies title-canonicalisation); (2) pull `event.attendees[]` (Calendar API gives `{email, displayName, responseStatus}`); (3) resolve each email through existing `agenda/service.py::_build_email_to_name_map` extended to include `counterparties.email` (currently it queries `Employee` + `TeamMember` only — add Counterparty as third source so external participants resolve to canonical names from operator's sheet); (4) authoritative list = resolved attendees in event order. **Persistence:** new JSON column `calendar_attendees` on `zoom_recordings` + `meeting_recordings` (alongside existing `participants`): `[{email, display_name, resolved_name, source: "team_member"|"employee"|"counterparty"|"unknown", response_status: "accepted"|"tentative"|"declined"|"needsAction"}]`. Authoritative when present + non-empty → used by Header / Doc / Slack / TG renderers in place of `participants`. When empty (event not found, or no attendees) → fall back to existing LLM-extracted `participants` (graceful degradation). **Unknown attendees** (no email match): rendered as `display_name (email)` so operator can add to sheet later — never silently dropped. **Declined attendees** (`responseStatus="declined"`) excluded from rendered list — they explicitly said they weren't coming. **Trace** event `zoom_calendar_attendees_resolved` (or `fireflies_…`) with `event_id`, `match_method` (url/fuzzy/none), `attendees_count`, `resolved_count`, `unknown_count`, `dropped_declined` for diagnostics. Idempotent — re-runs on the same recording overwrite `calendar_attendees` rather than appending. **Migration:** add nullable JSON column, backfill optional (next pipeline pass naturally fills it for processed-again recordings). **Pipeline integration order:** the new lookup runs BEFORE the LLM-based `_ensure_team_participants` so when Calendar gives us real attendees, we save an LLM call entirely. When Calendar lookup fails, we still call LLM (current behaviour preserved). | `…::test_calendar_attendees_resolved_via_zoom_url_match` + `…::test_calendar_attendees_resolved_via_fuzzy_time_title_fallback` + `…::test_calendar_attendees_resolves_counterparty_email` + `…::test_calendar_attendees_includes_unknown_emails` + `…::test_calendar_attendees_excludes_declined` + `…::test_calendar_attendees_falls_back_to_llm_when_event_missing` + `…::test_summary_header_renders_calendar_attendees_when_present` |
| FR-CR-05-154 | Push canonical title back to Fireflies UI via `updateMeetingTitle` GraphQL mutation. **Operator regression on James Morgon 30/04 rerun:** «встреча все равно называется: '30/04 - James Morgon'» — the calendar match correctly rewrote our DB / Doc / TG / Slack header to «30/04 - James Morgon <> Artem Sokolov | Zoom call» (FR-CR-05-136 + FR-CR-05-144), but the **Fireflies UI** still showed the original because we never pushed the change to Fireflies' side. Operator wants Fireflies UI to also reflect the canonical title. **Service:** new `app/fireflies/client.py::FirefliesClient.update_transcript_title(transcript_id, title)` calls Fireflies' `updateMeetingTitle(input: {title, transcript_id})` mutation. Returns True on `success=true`, False on (a) disabled client / empty inputs (no API call), (b) request exception, (c) GraphQL `errors` array, (d) `success=false` in result. NEVER raises — caller wraps in try/except. Logs `fireflies_title_updated_remote` (success) / `fireflies_update_title_*` (failure variants). **Pipeline integration:** `_step_match_calendar_title` (Fireflies) — after `row.title = new_title; session.flush();` and `fireflies_calendar_match_title_updated` log emit, calls `self._client.update_transcript_title(row.fireflies_id, new_title)` inside try/except. Logs `fireflies_title_pushed_to_remote success=<bool>` + trace event. Failures NEVER cascade — DB title is already updated; remote push is best-effort. **Tests** (5 new in `test_fireflies.py`): mutation request shape (URL, Bearer header, query string contains `updateMeetingTitle`, variables carry title + transcript_id); GraphQL `errors` array → False; `success=false` return → False; request exception → False, no raise; empty token / empty id / empty title → False without HTTP attempt. Suite: 1655 passed. **Operator effect:** next Fireflies-recording rerun pushes the calendar-matched title not just to our DB / Doc / TG / Slack but also to Fireflies' web UI — every surface shows the same canonical title. |
| FR-CR-05-153 | Zoom VTT FIRST, Whisper fallback (was: Whisper first → VTT only on detected hallucination). **Operator regression on Fundraising daily 05/05** (R3Y, 26 min of normal speech): Whisper produced 1795 chars of «Девушкиной науке Университет candy samurai Университет voice … Университет youtube Университет https://vk.com.ua» — different loop vocabulary from the May 4 «Редактор субтитров» case (FR-CR-05-148 markers didn't catch it), AND unique-word ratio was ~10% (above the 5% threshold), so detector returned False, no fallback fired. Operator: «там 26 минут нормальной речи… давай сделай нормально». **Root cause:** Whisper sometimes hallucinates even on perfectly clean Russian audio — encoding quirk, bitrate, or just a Whisper failure mode. Zoom's own VTT engine handles the same audio fine (it's what the operator sees as the «summary» tab in the Zoom UI). **Pipeline flip:** `_step_transcribe` (Zoom) now tries Zoom VTT FIRST via the existing `_find_vtt_download_url` → `fetch_vtt_transcript` path. If VTT is ≥200 chars, use it directly (`row.transcript_text=vtt_text`, log `zoom_vtt_used_primary` + trace event). Whisper code path retained as fallback for the rare case Zoom didn't generate a VTT (workspace-level transcription disabled, or recording too short). The post-Whisper hallucination check (FR-CR-05-148) still runs, and falls back to VTT one more time as a last-ditch (defense-in-depth). **Detector strengthened:** `looks_like_whisper_hallucination` gains: (a) new URL/social markers (`youtube`, `https://vk.`, `vk.com`, `instagram.com`, `facebook.com`, `.com.ua`); (b) unique-word ratio threshold lowered 5% → 8%; (c) NEW bigram-loop signal — any 2-word phrase repeating ≥20× → flagged. Catches «Университет youtube» loop where the head has varied vocabulary but the body is pure repetition. **Tests** (3 new in `test_audio_transcription.py`): `test_looks_like_whisper_hallucination_flags_youtube_vk_loop` (operator's exact production output flagged); `test_looks_like_whisper_hallucination_flags_pure_bigram_loop` («слово фраза» × 100 caught by bigram-counter alone); `test_looks_like_whisper_hallucination_real_transcript_with_some_repetition` (realistic Russian fundraising meeting NOT flagged). Suite: 1650 passed. **Operator effect:** rerunning R3Y / any Zoom recording where Zoom-side VTT is available → VTT used directly, no Whisper, no hallucinations. Whisper bias-prompt logic preserved for fallback path; English/proper-noun fidelity still good when needed. |
| FR-CR-05-152 | Multi-calendar support — `GOOGLE_CALENDAR_ID` accepts a comma-separated list of calendar ids; events are merged across all queried calendars. **Operator regression:** «надо все кроме личного». Calendar inventory diagnostic showed 4 work-relevant calendars: `primary` (1@thehumanoid.ai = «Артем Соколов внешний» — only external meetings), `c_98ccf…` («Артем Соколов личный» — personal stuff like «Чайная церемония», irrelevant), `c_15b48…` («Fundraising calendar» — investor intro calls), `c_e02f2…` («Humanoid Events Calendar» — demo events). «Fundraising daily» lives on primary but «Sculptor intro» on Fundraising calendar — single-calendar config too narrow. **Service:** `app/services/calendar_match.py::fetch_calendar_events_via_api` now parses `calendar_id` as comma-separated list (blanks/whitespace dropped, fallback `["primary"]` when empty). Each calendar queried independently inside one `service` instance; results merged in iteration order. Duplicate events (same `id` across calendars when both host + attendee see them) are deduped by `seen_event_ids` set, keeping FIRST occurrence. Each event dict gets a `_calendar_id` key for trace/debug. Per-calendar HTTP errors (e.g. `403 forbidden` on a revoked shared calendar) are logged and skipped — partial results > complete failure. **Tests** (3 new in `test_calendar_match.py`): `test_fetch_events_via_api_merges_multiple_calendars` (3 calendars, dedup verified, `_calendar_id` carried through, query order pinned); `test_fetch_events_via_api_skips_blank_calendar_ids` (`primary,,  ,c_xxx` → 2 queries, blanks ignored); `test_fetch_events_via_api_one_calendar_failing_doesnt_block_others` (mid-list HTTP error doesn't kill others). Suite: 1647 passed. **Operator config:** `GOOGLE_CALENDAR_ID=primary,c_15b484dbb98135465be706542626a2a323679089464f3c962e5f3a335854a877@group.calendar.google.com,c_e02f2d20fc1cc703b8c2a53e3d5b37f6ece70fcda7f4946896e8bb7b689c02b5@group.calendar.google.com` covers all work-relevant calendars while skipping the personal one. |
| FR-CR-05-150 | Slack-side threading — first chunk to channel as parent, remaining as thread replies. **Operator-pinned:** «как сделать в слаке так, чтобы он слал сообщение, а остальным были как в треде этого сообщения». Combined with FR-CR-05-149 chunk-limit=3500, long meetings now produce multiple `chat.postMessage` calls; this commit threads them so the Slack DM channel shows ONE visible parent post + a clickable thread («N replies») for the rest. **Service:** `app/services/slack_mirror.py::post_meeting_summary_to_slack` captures the first successfully-posted chunk's `ts` into `parent_ts`, then passes `thread_ts=parent_ts` on every subsequent chunk's `chat_postMessage` kwargs. Logs `slack_mirror_chunk_posted` now also include `thread_ts=parent_ts` for chunks 2..N (None on chunk 1). When chunk 1 fails (no `ts` returned), `parent_ts` stays None and chunks 2..N go as un-threaded channel posts (degraded but visible). **Tests** (2 new): `test_post_meeting_summary_threads_remaining_chunks_under_first` (long body splits into ≥3 chunks; chunk 1 has NO `thread_ts`, chunks 2..N all carry `thread_ts=<chunk-1 ts>`); `test_post_meeting_summary_single_chunk_no_thread` (single-chunk body never gets a `thread_ts`). New `_ThreadCapturingFakeWebClient` test helper assigns unique ts per call so threading can be verified. Suite: 1644 passed. **Operator effect:** Slack DM looks like ONE post with a thread underneath — clean, copy-pastable, doesn't pollute the channel timeline. |
| FR-CR-05-149 | Slack `chat.postMessage` server-side auto-split — pre-chunk under 4 000 chars to control split boundaries. **Operator regression:** «он нарзал. также как в тг». Conversations.history diagnostic confirmed Slack creates MULTIPLE messages with adjacent sub-second ts when a single `chat.postMessage` text exceeds ~4 000 chars: Fundraising sync's 10 730-char body landed as 3 separate Slack messages (3995 + 3974 + 2759 chars, ts `1777911477.483559/.511359/.532589`), the second starting mid-task on «23) Update follow-up». Slack splits where IT likes (mid-word, mid-task). FR-CR-05-141 chunk limit was 35 000 — way above Slack's auto-split threshold — so we shipped one big call and Slack diced it. **Fix:** lower `SLACK_TEXT_CHUNK_CHARS` from 35 000 → **3 500** so each chunk stays under Slack's auto-split threshold. The existing `_split_for_slack` (paragraph → line → hard-cut) now produces multiple chunks for long meetings, each becomes one explicit `chat.postMessage` call → one ts → no server-side mid-word fragmentation. Operator gets a Telegram-like sequential post stream in Slack, but each post is a clean unit (whole task, whole paragraph). FR-CR-05-147 numbered-list compactor still applies BEFORE chunking — so if the entire To-Do still fits under 3 500 chars after compaction, it lands as one tight post. **Tests:** existing `test_post_meeting_summary_chunks_long_body_sequentially` updated to use new limit (test body shrunk from 50K → 10K, asserts ≥3 chunks); new `test_slack_chunk_limit_is_under_slack_auto_split_threshold` pins upper bound so a future bump back to 35K breaks loudly with a clear hint. Suite: 1642 passed. **Operator effect:** next mirror lands as N tidy posts (each ≤3500 chars on paragraph boundaries), N = ceil(body_chars / 3500). For a 10 K body that's ~3 posts, but each is a complete logical unit. |
| FR-CR-05-148 | Zoom VTT transcript fallback when Whisper hallucinates. **Operator regression on «Ирина - статус по задачам» (2026-05-04 15:02 UTC, 32 min recording, 18.7 MB M4A audio):** detailed-summary said «не содержит содержательного обсуждения задач», but the actual recording had real content. DB inspection: `transcript_text='Редактор субтитров А.Семкин Корректор А.Егорова Редактор субтитров Н.Александрова Корректор А.Кулакова ...'` repeated for 2962 chars — Whisper looped on Russian dubbed-subtitle credits from its training data instead of transcribing speech. Diagnosed cause: low audio bitrate (78 kbps), or long silent intro, or Whisper-known silent-audio failure mode. **Zoom side has its own VTT transcript** (`recording_type=audio_transcript, file_extension=VTT, 38KB`) — this commit makes the pipeline fall back to it when Whisper output looks like garbage. **Detector:** new `app/services/transcription.py::looks_like_whisper_hallucination(text, expected_min_chars=500)` flags when (a) ≥3 hits of `WHISPER_HALLUCINATION_MARKERS` («Редактор субтитров» / «Корректор» / «Subtitles by» / «Спасибо за просмотр» / «Подписывайтесь» etc.) in the first 1000 chars, OR (b) unique-word ratio <5% over 100+ words. Short / empty transcripts return False — those are «empty meeting» cases, not hallucinations. **VTT fetcher:** new `ZoomClient::fetch_vtt_transcript(url)` HTTPs the VTT download URL with bearer auth, hands body to new `_parse_vtt_to_plain_text(vtt_body)` which strips WEBVTT header / numeric cue-ids / `HH:MM:SS.mmm --> HH:MM:SS.mmm` timing lines / NOTE / STYLE / REGION blocks and returns concatenated cue text joined by `\n`. **Pipeline integration:** `_step_transcribe` (Zoom) — after `transcribe_chunks_parallel` returns, run hallucination detector. If Whisper failed (empty) OR hallucinated, call new `_find_vtt_download_url(row)` (re-list recordings, match by uuid, find file with `recording_type=audio_transcript` + `file_extension=VTT`) and `fetch_vtt_transcript(url)`. If VTT > 100 chars → use it instead. Logs `zoom_whisper_fallback_to_vtt` (with `whisper_failed`/`hallucinated`/`whisper_chars`/`whisper_preview`) and `zoom_vtt_transcript_used` (with `vtt_chars`); both also as trace events. **Tests** (10 new): `test_looks_like_whisper_hallucination_flags_subtitle_credit_loop` (operator's exact 2962-char production output flagged); `test_looks_like_whisper_hallucination_passes_real_transcript` (diverse vocab not flagged); `test_looks_like_whisper_hallucination_short_text_returns_false` (empty / «Spasibo.» / single subtitle line ≠ hallucination); `test_looks_like_whisper_hallucination_flags_low_unique_word_ratio` (200x repeated phrase flagged via ratio); `test_parse_vtt_to_plain_text_strips_timing_and_cue_ids`; `test_parse_vtt_to_plain_text_handles_no_cue_ids`; `test_parse_vtt_to_plain_text_empty_input`; `test_zoom_client_fetch_vtt_transcript_returns_plain_text` (end-to-end: VTT body → bearer-auth HTTP GET → parsed plain text). Suite: 1641 passed. **Operator effect:** rerunning the failed Ирина recording with `--rerun` now triggers VTT fallback automatically — readable transcript replaces the «Редактор субтитров» loop, downstream summary + tasks work. Future Whisper-loop incidents auto-recover without manual intervention. |
| FR-CR-05-147 | Slack-side numbered-list compactor — collapse `\n\n` between consecutive `1) …`, `2) …` items into a single `\n` so Slack renders the To-Do block as ONE compact paragraph instead of N floating bubbles. **Operator regression on the post-FR-CR-05-146 Fundraising-sync rerun:** «опять в слаке отдельные сообщения - сделай под слак отдельную функцию которая соединяет все в одно». Slack mirror logged `chunk_index=1 chunk_total=1` (one `chat.postMessage` call), but Slack's UI renders each `\n\n`-separated paragraph as its own visual bubble for long messages — so a 60-task Telegram-spaced To-Do reads as 60 separate cards. **Service:** new `app/services/slack_mirror.py::_compact_for_slack(text)` runs after `_to_slack_mrkdwn` and before `_split_for_slack`. Logic: (1) collapse 3+ consecutive newlines to exactly 2; (2) iteratively replace `\n\n` BETWEEN consecutive numbered items (`(?<=[^:])\n\n(?=\d+\)\s)` so the gap AFTER the `To-Do:` heading is preserved); (3) leave `\n\n` between Header / Участники / Суть / To-Do / Подробный отчёт untouched. Two-digit numbered items (`9)`, `10)`, `100)`) recognised. Pure text in / out. **Tests** (6 new in `test_slack_mirror.py`): fuses 1)→2)→3) tasks while keeping `To-Do:\n\n1)` and `\n\nПодробный отчёт`; handles two-digit items; collapses excess blank runs; no-op without numbered list; empty input → empty; end-to-end `post_meeting_summary_to_slack` integration verifies the actual Slack `text` param has the compacted list. Suite: 1633 passed. **Operator effect:** next rerun's Slack DM renders the entire To-Do block as one tight paragraph (vs the previous 60-bubble cascade). |
| FR-CR-05-146 | Pipeline parallelism — Whisper chunks, counterparty resolve, participants kickoff, TG cards. **Operator regression:** «почему так долго? ты все асинк запараллелил?» — last 41-min Fundraising-sync run was almost entirely sequential LLM calls. Operator approved 4 parallelisms (rejected 2: don't merge canonicalize+consolidate, don't merge counterparty extract+resolve into one pass). **(a) Whisper chunks parallel** — `app/services/transcription.py::transcribe_chunks_parallel(paths, *, openai_api_key, model, prompt, mimetype_for, max_workers=3)` runs `transcribe_bytes` per chunk in a `ThreadPoolExecutor`; `pool.map` preserves chunk order so the joined transcript stays chronological. Wired into both `app/zoom/pipeline.py::_step_transcribe` and `app/fireflies/pipeline.py::_step_transcribe`. Saves ~2.5 min on a 3-chunk recording (3× speedup). Per-chunk failure returns `None` for that slot — caller (pipeline) detects and bails. **(b) Counterparty resolve batched parallel** — refactored `resolve_mentions_to_directory` to extract per-batch logic into `_resolve_batch`. New kwargs `batch_size: int = 0` (0 disables — legacy single-call) and `max_workers: int = 5`. When `len(mentions) > batch_size`, mentions split into batches of `batch_size`, each processed in parallel via `ThreadPoolExecutor`; results merged. Each batch sees the FULL directory so per-batch disambiguation is unchanged. New settings `COUNTERPARTY_RESOLVE_BATCH_SIZE=20` (default) + `COUNTERPARTY_RESOLVE_MAX_WORKERS=5`. Saves ~5-6 min on a 70-mention meeting (was 7 min single call → ~1-2 min parallel). Per-batch trace events (`counterparty_resolve_call_started` carries `batch_index`/`batch_total`) + summary `counterparty_resolve_batched`. **(c) detailed_summary || participants kickoff** — `_step_detailed_summary` now accepts `session=` kwarg; when provided, calls new `_kickoff_team_participants_async(row, session)` BEFORE the summary LLM call. Helper pre-fetches `known_employees` on the calling thread (DB single-threaded), then submits `_run_participants_llm` to a 1-worker `ThreadPoolExecutor`, stores future on `row.__dict__["_zm_participants_future"]`. `_ensure_team_participants` (called later from `_step_extract_tasks`) joins the future via `future.result(timeout=600)` instead of starting a fresh LLM call; falls back to inline run when no future exists (legacy callers). Post-filter (FR-CR-05-145) still applies. Saves ~2 min (the participants LLM ran during the summary LLM). Logs `zoom_participants_kickoff_async`. **(d) post_task_cards parallel (max 10)** — both `_step_post_task_cards` (Zoom + Fireflies) refactored to `ThreadPoolExecutor(max_workers=min(10, len(task_ids)), thread_name_prefix=…)`. Each worker opens its own `session_scope()` (SQLA sessions aren't thread-safe), fetches its task by id, calls `post_initial_card` with that session. Per-task DB writes happen in independent transactions so no contention. Telegram rate limit (30 msg/sec across chats, 1/sec per chat) honored — operator-pinned «но аккуратно по 10 максимум». Saves a few seconds even on the failing `chat not found` path; bigger win when DMs actually deliver. **Tests** (5 new): `test_transcribe_chunks_parallel_runs_concurrently_and_preserves_order` (3-thread concurrency observed via lock-counter, output ordering pinned); `test_transcribe_chunks_parallel_returns_none_on_no_api_key`; `test_transcribe_chunks_parallel_propagates_per_chunk_failure`; `test_resolve_mentions_to_directory_runs_batches_in_parallel` (30 mentions / batch_size=10 → 3 concurrent calls, all 30 resolved in merged map); `test_resolve_mentions_to_directory_default_no_batching_one_call` (legacy single-call path preserved when batch_size=0); `test_zoom_participants_kickoff_runs_in_parallel_with_detailed_summary` (kickoff seeded by `_step_detailed_summary` populates `_zm_participants_future`; `_ensure_team_participants` joins the future and clears it without making a fresh LLM call). Suite: 1627 passed. **Operator effect:** total per-meeting time drops from ~37 min to ~25 min (estimate). Bigger drop possible later via FR-CR-05-146e (canonicalize+consolidate merge) — operator-pinned «не трогай», so left for now. |
| FR-CR-05-145 | Python-side defense for the notes-forbids-topic rule. **Operator regression on the second Fundraising-sync rerun:** «опять дима дроздов во фандрайзинге участвует - он не должен, а в слак также пришло разными сообщениями сука! проверь раз и навсегда, уже много денег потратил». FR-CR-05-142b strengthened the participants-extraction prompt with «не участвует в X» / «не вести X-задачи» exclusion rule, but the LLM (gpt-5.5 reasoning=high) still emitted Дроздов into `participants=['Дима Дроздов', 'Дмитрий Седов', ...]` for the May 1 meeting because (a) the meeting_title was Zoom's auto-generated «Artem Sokolov's Zoom Meeting» (no «Fundrising» keyword) and (b) the LLM gave more weight to the heading «ВСЕ, ЧТО СВЯЗАНО С ФОНДАМИ» in the notes than to the trailing «// не участвует в Fundrising sync» override clause. Result: 9 of 41 extracted tasks landed on Дроздов, including some that were genuine fundraising follow-ups. **Python post-filter:** new `app/services/team_members.py::filter_participants_by_notes_forbid(participants_real_names, *, known_employees, topic_keywords) -> (kept, dropped)` re-applies `_employee_forbids_topic` (FR-CR-05-142b's helper, now also exposed as public `employee_forbids_topic`) to the LLM's output participants list. Even when the LLM ignores the rule, Python catches it. Returns `(kept, dropped)` so callers can log the diff. **Wider topic source:** the post-filter combines (a) `meeting_title`, (b) first 3000 chars of `detailed_summary`, (c) first 1500 chars of `transcript_text` so the «fundraising» keyword surfaces from the transcript content even when the Zoom auto-title doesn't carry it. **Pipeline integration:** Zoom side wires the post-filter into `_ensure_team_participants` — applied AFTER the LLM call, BEFORE caching on `row.__dict__["_zm_team_participants"]`. Logs `zoom_participants_post_filter_applied` + traces `zoom_participants_post_filter_applied` event with `topic_keywords`/`dropped`/`kept` so operator can see what got nuked. Fireflies side applies the same filter inline in `_step_extract_tasks` before rendering the `meeting_participants` block; logs `fireflies_participants_post_filter_applied`. Defense in depth — even if a downstream caller still references `row.participants` raw, `pick_meeting_owner_fallback` (FR-CR-05-142a) ALSO applies `_employee_forbids_topic` independently, so a forbidden teammate can never end up as the fallback owner. **Tests** (`test_team_members.py`, 3 new): `test_filter_participants_drops_drozdov_on_fundraising_topic` (operator's exact production notes + auto-title + transcript content combined → Дроздов dropped, Седов + Артем kept); `test_filter_participants_no_op_when_no_topic_keywords` (empty keywords → no filtering); `test_filter_participants_unknown_name_passes_through` (defensive — names not in `known_employees` aren't dropped). Suite: 1621 passed. **Operator effect:** next Fundraising-sync rerun should show `zoom_participants_post_filter_applied dropped=['Дима Дроздов']` in the trace, then `participants=['Дмитрий Седов', 'Артем Соколов', ...]` flows into both the task-extractor's `meeting_participants` block AND the short-summary Участники line. |
| FR-CR-05-144 | Direct Google Calendar API for meeting-title match (replaces Apps Script proxy of FR-CR-05-136). **Operator-pinned:** «давай пайплайн с календарем и firefiles … через google api client id 430890914351-…apps.googleusercontent.com, scope `calendar.readonly`», «новый dал», secret `GOCSPX-...`. Apps Script Web App was workspace-locked (302 to ServiceLogin) and operator chose direct OAuth instead. **Separate OAuth client:** new `GOOGLE_CALENDAR_CLIENT_ID` / `GOOGLE_CALENDAR_CLIENT_SECRET` settings (NOT the existing `GOOGLE_CLIENT_ID`/`SECRET` used for Sheets/Docs/Tasks) so Calendar consent is isolated from the existing one (revoke / re-consent doesn't disrupt sync). Refresh token stored in DB under `user_key="_calendar"` (vs `"_service_account"`). New scope `GOOGLE_SCOPES_CALENDAR = ["https://www.googleapis.com/auth/calendar.readonly"]`. **Service:** new `app/services/calendar_match.py::fetch_calendar_events_via_api(meeting_dt, *, window_minutes, credentials_factory, calendar_id="primary")` — calls `googleapiclient.discovery.build('calendar', 'v3', credentials=...).events().list(timeMin, timeMax, singleEvents=True, orderBy='startTime', maxResults=50)`, returns events in the same `{title, start, end, attendees, description}` shape as the Apps Script proxy so downstream LLM-pass + format code is unchanged. Handles all-day events (`date` vs `dateTime`). Failures NEVER raise — empty list on auth fail, network fail, `HttpError`, or `ImportError` (googleapiclient not installed). New `app/sync/factories.py::build_calendar_credentials_factory(settings)` returns a callable that loads + decrypts the Calendar refresh token from DB; the callable is passed into `match_and_format_title` (Calendar API auto-refreshes the access token via `googleapiclient`). **Match orchestrator:** `match_and_format_title` now accepts both backends — `api_credentials_factory` (preferred) and `apps_script_url`/`shared_token` (legacy fallback). When both are configured, API path wins. When neither is configured, returns None (no-op, leave title alone). **Pipeline wire-up:** `app/fireflies/pipeline.py::_step_match_calendar_title` checks `settings.google_calendar_client_id` first — if set, calls `build_calendar_credentials_factory(settings)` and forwards as `api_credentials_factory=`. Falls back to Apps Script when only that's configured. **Bootstrap CLI:** new `ops/bootstrap_calendar_oauth.py` — same shape as `bootstrap_google_oauth.py` (Sheets/Docs/Tasks) but uses `GOOGLE_SCOPES_CALENDAR` and saves under `user_key="_calendar"`. Operator runs once on a machine with browser access; refresh token is encrypted with the existing `SECRETS_ENCRYPTION_KEY` Fernet key. **Realtime polling:** Fireflies poll loop already runs every `FIREFLIES_POLL_INTERVAL_SECONDS=60` (FR-CR-05-58); calendar match step (FR-CR-05-136) is already wired into the per-recording pipeline; this commit just swaps the backend so polling now actually picks up titles via the Calendar API instead of failing on the locked Apps Script. **Tests** (`test_calendar_match.py`, 5 new): `test_fetch_calendar_events_via_api_returns_normalised_dicts` (shape parity + dateTime AND date both normalised + correct timeMin/timeMax/orderBy params); `test_fetch_calendar_events_via_api_no_credentials_returns_empty` (factory returns None → empty, no API call); `test_fetch_calendar_events_via_api_credentials_factory_raises` (factory raises → empty, never propagates); `test_match_and_format_title_prefers_api_path_when_factory_given` (API wins over Apps Script when both configured — Apps Script `urlopen` poison-trap fails the test if it runs); `test_match_and_format_title_no_op_when_neither_backend_configured`. Tests use a `_patched_googleapiclient` contextmanager that installs a stub `googleapiclient` package in `sys.modules` AND restores prior state on exit (no order-dependent test pollution). Suite: 1618 passed. **Operator runbook:** (1) set `GOOGLE_CALENDAR_CLIENT_ID` + `GOOGLE_CALENDAR_CLIENT_SECRET` + `CALENDAR_MATCH_ENABLED=true` in env. (2) `python -m ops.bootstrap_calendar_oauth` — runs the consent flow once, stores refresh token in DB. (3) restart listener — polling auto-picks-up Calendar API. The legacy `CALENDAR_APPS_SCRIPT_URL` is no longer required when the API client is configured. |
| FR-CR-05-143 | Artem-only Zoom recording filter (host OR participant) + targeted `--zoom-id` rerun. **Operator regression:** «мне надо проверять что там есть 1@thehumanoid.ai и только если да, то обрабатывать», «поэтому давай проверим на самой последней где есть и автоматом будем делать как только будет в новом email thehumanoid.ai (проверяем каждую минуту)». `/accounts/me/recordings` returns ALL cloud recordings under the Humanoid.AI Zoom account — Server-to-Server OAuth tokens are account-scoped, not user-scoped, so the listing leaks Viktor/Irina/Elena/jpog hosted recordings into Artem-targeted runs (operator confirmed via diagnostic: 4 of last 4 recordings had non-Artem `host_email`). The filter must keep meetings where Artem joined someone else's call too (host-match alone is too narrow). **Schema:** `ZoomRecordingMeta` gains `host_email: str | None` (default None). `ZoomClient.list_recordings` populates it from the API payload's `host_email` field. **Filter (host OR participant):** new `required_email=` kwarg on `list_recordings`; when truthy, a meeting is kept iff (a) `host_email == required_email` (case-insensitive — fast path, no extra API call) OR (b) the email appears in `/past_meetings/{uuid}/participants` (fallback when host doesn't match — extra API call). New `_fetch_meeting_participant_emails(uuid)` method handles UUID encoding (single-encode normally, double-encode when uuid starts with `/` or contains `//` per Zoom docs); returns lowercase emails set; never raises — empty set on any failure (effectively deny-by-default for that recording). Empty / None `required_email` preserves legacy behaviour (no filter). Logs `zoom_recordings_listed` with `required_email`, `kept`, `skipped_no_match`, `kept_via_participants`, `page_total`. **Settings:** new `ZOOM_REQUIRED_EMAIL` (default empty). When set (e.g., `1@thehumanoid.ai`), the migrator AND the realtime listener pass it into `list_recordings`. **Realtime polling integration:** `app/telegram_bot/listener.py::_maybe_poll_zoom` (already existed, every `ZOOM_POLL_INTERVAL_SECONDS=60` seconds) reads `settings.zoom_required_email` and forwards it as `required_email=` kwarg to `list_recordings` — operator's «проверяем каждую минуту» now lands only Artem-involved recordings. **CLI:** `ops/migrate_zoom.py` extended: (a) `--zoom-id <uuid>` for targeted rerun on one specific recording (bypasses the required-email filter — operator picked the uuid manually). (b) `--page-size <int>` (default 30) so operator can bump to 300 when host's recordings are sparse. (c) `--newest` is no longer the only valid mode — `--zoom-id` is an alternative; one of them is required. (d) `--days-back <int>` (default 30) — when `--zoom-id` is set, the lookup widens to this many days back (Zoom API's `/accounts/{id}/recordings` defaults to LAST 24 HOURS when `from`/`to` absent — operator confirmed via `page_total=4 (today only)`, so historical UUIDs need an explicit window). **Tests** (`test_zoom.py`): `::test_zoom_client_list_recordings_populates_host_email` (parses `host_email` from API payload onto the dataclass); `::test_zoom_client_list_recordings_keeps_host_match_no_participants_call` (host-match shortcuts the extra API call — cost saver); `::test_zoom_client_list_recordings_falls_back_to_participants_check` (host mismatch → fetch `/past_meetings/{uuid}/participants` → keep iff email present; verified via 3-recording fixture: viktor-hosted + Artem participant = keep, elena-hosted + Artem absent = drop, artem-hosted = keep without participants call); `::test_zoom_client_list_recordings_empty_required_email_disables_filter` (None / empty string preserves legacy behaviour, no participants calls); `::test_zoom_client_fetch_participant_emails_handles_failure` (deny-by-default when participants API returns empty). Plus stub-listener test updated to forward arbitrary kwargs to `list_recordings` so the realtime poll wiring runs cleanly. Suite: 1613 passed. **Operator effect:** `ZOOM_REQUIRED_EMAIL=1@thehumanoid.ai` in `/tmp/tg-listener.env` + listener container restart → polling every 60 s only ingests recordings where Artem hosts OR attends. `python -m ops.migrate_zoom --zoom-id 'HdyK6m9iQtKabZFpT6bN1Q==' --rerun` runs a single targeted rerun on Fundraising sync (filter bypassed). |
| FR-CR-05-142 | Never-null owner + notes-driven topic exclusion. **Operator regression (post FR-CR-05-141 zoom rerun on `HdyK6m9iQtKabZFpT6bN1Q==` / 01/05 - Fundraising sync):** «есть задачи без ответственных / такого быть не может! всегда ответственный должен быть, это сломалось - спека и тесты // и димы дроздорова не в участниках ни в задачах не должно быть в Fundrising». Two failures: (a) tasks 74-77 from the run had `owner_user_id IS NULL` because the LLM emitted `owner=null` and the pipeline preserved it (FR-CR-05-134 anti-admin-fallback was too strict — it conflated «don't fallback to AI Lead» with «leave null»); (b) Дима Дроздов appeared in participants AND task assignments (Sanders Capital, TWG, DBS, Lunate) for a fundraising meeting where his notes already say «не вести fundraising-задачи». **142a — Always pick an owner:** TASK_EXTRACTION_SYSTEM Rule 5 rewritten as «ALWAYS PICK AN OWNER» with explicit five-step cascade (a→b→c→d→e): (a) Rule 7 named-assignee → (b) NEW self-named-in-task-body «Список <Имя>» / «Задачи <Имя>» → (c) Rule 8 present-teammate domain match → (d) meeting PRINCIPAL among participants (notes/role mark CEO/founder/принципал) → (e) first non-forbidden participant. Rule 6 retained as «NEVER PICK THE ADMIN / AI LEAD ROW» but the prior «null > admin» framing dropped (cascade now ALWAYS lands on a non-admin teammate). TASK_VERIFICATION_SYSTEM mirrors. **Pipeline-side never-null safety net:** new `app/services/team_members.py::pick_meeting_owner_fallback(known_employees, participants_real_names, topic_keywords)` runs three passes — principal-not-forbidden → any-not-forbidden → first-present-unconditional — filtering admin uids via `TELEGRAM_ADMIN_USER_IDS`. Returns `None` only when there are no present participants at all. Helper `infer_topic_keywords_from_text` derives the topic bucket (`fundraising`/`ir`/`investor`/`research`) from meeting title + task title for the «не вести X-задачи» check. Both `app/zoom/pipeline.py::_step_extract_tasks` and `app/fireflies/pipeline.py::_step_extract_tasks` invoke the fallback whenever `owner_uid` resolves to None, log `task_owner_resolved` with `resolution=fallback_principal` / `fallback_no_participants`, and trace the same to per-recording JSONL. **142b — Notes-driven exclusion in participants:** `app/services/zoom_participants.py::extract_zoom_participants_via_llm` accepts new `meeting_title` kwarg, prepends `meeting_title: <title>` block to the user prompt. PARTICIPANTS_EXTRACT_SYSTEM AMBIGUOUS FIRST-NAME RULE extended with a SECOND EXCEPTION: when the meeting topic is X (inferred from `meeting_title` / transcript) AND a candidate's notes say «не вести X-задачи» / «не вести X» / «do not assign X», EXCLUDE that candidate even on a bare-first-name match. Two worked examples pinned in the prompt: A (notes don't disambiguate, both Димы included) vs B (notes EXCLUDE Дроздов on a fundraising meeting → only Седов). TASK_EXTRACTION_SYSTEM Rule 8 also gets a NOTES-FORBIDS-DOMAIN clause for the per-task path with the Sanders Capital worked example. **Tests** (additions to `test_fireflies.py`, `test_counterparties.py`, `test_team_members.py`): `test_pipeline_assigns_principal_when_llm_returns_null_owner` (replaces FR-CR-05-134's `test_pipeline_leaves_owner_null_when_llm_declined_to_assign` — now asserts owner is non-null and equals the principal participant uid, NEVER admin); `test_task_extraction_prompt_pins_never_null_owner_cascade` (FR id, «ALWAYS PICK AN OWNER», cascade examples); `test_task_extraction_prompt_pins_notes_forbids_domain_rule` (FR-CR-05-142b literal, «не вести fundraising», Sanders Capital example, in both extractor + verifier prompts AND PARTICIPANTS_EXTRACT_SYSTEM); `test_extract_zoom_participants_passes_meeting_title_to_llm_for_topic_filter` (kwarg threading + user prompt content); 6 helper tests in `test_team_members.py` covering principal-pick, admin-skip-via-env, notes-forbids-topic-skip, no-participants → None, last-resort-first-participant, `infer_topic_keywords_from_text`. Existing `test_task_extraction_prompt_forbids_admin_default_owner` updated for the new «NEVER PICK THE ADMIN» framing (drops obsolete «null is strictly better» language). Suite: 1607 passed. |
| FR-CR-05-141 | Slack mirror multi-chunk delivery — operator-pinned «можешь в слак отправлять в зум саммери все сообщение целиком до 35 тыс символов, а что уже не вмещается то след сообщением». **Service:** `app/services/slack_mirror.py::_split_for_slack(text, *, limit=35_000)` — paragraph-boundary first, then single-newline, then hard-cut for pathological single lines. `post_meeting_summary_to_slack` return shape changed: `dict | None` → `list[dict[str, Any]]` (one element per successfully-posted chunk). Pipeline call site (`app/zoom/pipeline.py::_send_short_summary`) updated to count `chunks_posted` from the list, log per-chunk via `slack_mirror_chunk_posted`, surface aggregate in `zoom_slack_mirror_posted` trace + log. **Tests** (`test_slack_mirror.py`): existing tests updated for the new list-return shape; new `_split_for_slack_*` tests cover short-body single-chunk, paragraph boundary, long single-paragraph fallback to line-split; `test_post_meeting_summary_chunks_long_body_sequentially` end-to-end pins multi-call delivery with each chunk ≤ 35 000 chars + same channel + ordered. Suite: 1621 passed. |
| FR-CR-05-140 | Ambiguous first-name participants — operator regression: «у тебя в участниках Дмитрий Дроздов, а нужен Седов». LLM picked one of two same-first-name teammates («Дима Дроздов» vs «Дмитрий Седов») when transcript only said «Дима» without a surname; downstream task-routing then assigned all «Дима»-tasks to the wrong one. **Fix in `PARTICIPANTS_EXTRACT_SYSTEM`:** new AMBIGUOUS FIRST-NAME RULE — when transcript uses bare first name AND `team_members` has TWO+ teammates with that first name, include ALL of them in the participants list. Downstream task-routing layer (Rule 8 of TASK_EXTRACTION_SYSTEM, FR-CR-05-139) reads notes per-task and disambiguates with much more context than this single pass has. ONLY exception: transcript explicitly uses a surname («Седов посмотрит …») — then include only the named one. Worked example pinned in the prompt. |
| FR-CR-05-139 | Participants-aware owner routing — disambiguate identical first names via the meeting attendees list. **Operator regression:** «но у тебя Дима Седов / Дроздов опять поехал, надо с учетом контекста вычленять кто был на встрече и уже когда задачи назначаешь то сопоставлять роль и notes». Two teammates share a first name («Дима Дроздов» — Аналитик, and «Дмитрий Седов» — Финансовый Советник Артема). Without an explicit attendee list the LLM was routing fundraising tasks to Седов via role-match even when only Дроздов was on the call. Operator clarified: «там в notes типа надо понять, что доступен был только Дроздов тк по его вопросам речь шла» — present-on-the-call beats role-match. **Prompt rule 8 (TASK_EXTRACTION_SYSTEM + TASK_VERIFICATION_SYSTEM):** «PARTICIPANTS BEAT ROLE-MATCH». Decision tree: (a) explicit named delegation → Rule 7 wins, with first-name disambiguation; (b) topic-routing → prefer participants over absent teammates with closer role-match; (c) DO NOT pick an absent teammate just because their role/notes is a closer match — leave owner null OR route to a present teammate. Worked example pinned in the prompt: «Дима Дроздов» vs «Дмитрий Седов» case end-to-end. **User-prompt block:** both Fireflies and Zoom builders now render a prominent `meeting_participants` list (REAL NAMES of who was on the call) above the transcript / detailed-summary so the LLM can apply Rule 8 directly. **Zoom early extraction:** new `_ensure_team_participants(row, known_employees)` helper on `ZoomPipeline` calls `extract_zoom_participants_via_llm` once per recording (cached on `row.__dict__["_zm_team_participants"]`); reused by `_step_extract_tasks` (NEW — was previously absent for Zoom) AND by `_step_short_summary`. Fireflies side uses `row.participants` (Fireflies API populates reliably). **Tests:** `test_fireflies.py::test_task_extraction_prompt_pins_disambiguate_first_names_via_participants` — pins «PARTICIPANTS BEAT» rule + FR id + worked example + the explicit «DO NOT pick an absent teammate» instruction in both extractor and verifier prompts. Suite: 1617 passed. |
| FR-CR-05-138 | Batch multi-select enrollment widget — UX redesign of FR-CR-05-133. **Operator-pinned:** «давай сделаем так: что не распознал, выводи одним сообщением после задач мол вот эти сущности не распознаны, и кнопки к виджету сделай мульти выбор циферный который тыкаешь (2 ряда по 5 и далее переход на след) — в общем в тексте описываешь какие сущности новые нашел и не распознал и оттыкиваешь на кнопки с их номерами делаешь мультивыбор → далее, и он по каждой сначала корректное название поставить / подтвердить, далее тебя начнет спрашивать информацию и можно ввести текстом или голосом». **Schema:** alembic `0026_counterparty_prompt_batches` adds `counterparty_prompt_batches(id, source_kind, source_id, chat_id, user_id BIGINT, multiselect_message_id BIGINT, entity_count, status, current_index, current_step, created_at, updated_at, completed_at)` with `UNIQUE(source_kind, source_id, user_id)` so reruns don't double-batch the same operator. Extends `counterparty_prompts` with `batch_id` FK (ON DELETE SET NULL), `index_in_batch INT` (1-based position in the numpad), `selected BOOL`, `canonical_name_corrected VARCHAR(512)` (operator's typed/voiced replacement for the surface form). **State machine** (3 stages, all English): `pending_selection ─[Next]→ processing(current_index=1, step=confirm_name) ─[text/voice OR Keep]→ processing(idx=1, step=context) ─[text/voice OR Skip]→ processing(idx=2, step=confirm_name) … → completed`. Skip during confirm-name drops the entity; Skip during context keeps the hub but no satellite. Unselected entities flip to `declined`. **Widgets:** `batch_select_keyboard(prompts, batch_id, selected_count)` — 2-row × 5-button numpad (5/row overflow-friendly) with «Next →  (N selected)» footer; selected indices shown with leading ✅ in both label and message body. `batch_confirm_name_keyboard(batch_id, mention_text)` — `[✅ Keep "..."] [⏭ Skip]` (mention trimmed to 30 chars to fit Telegram's 64-byte button cap). `batch_context_keyboard(batch_id)` — single `[⏭ Skip]`. New action constants `ACTION_BATCH_TOGGLE` / `_NEXT` / `_KEEP_NAME` / `_SKIP_ENTITY` reuse the existing `<verb>:<int>` callback_data format. **Service:** `app/services/counterparty_enrollment_batch.py` — `post_enrollment_batch` (dedupe by mention_normalised, one batch per recipient, idempotent on rerun); `handle_toggle` (flips `selected`, redraws); `handle_next` (zero-selected → terminal recap; otherwise marks unselected as declined and starts processing); `handle_keep_current_name` (uses surface form as canonical); `handle_skip_current` (drops or keeps without satellite based on step); `receive_text_for_current` (routes text/voice to confirm-name OR context based on `current_step`); `find_active_batch_for_user` (listener helper for routing replies without `reply_to`). All paths trace via `enrollment_batch_*` events. **Listener wiring:** `_dispatch_action` adds 4 branches; `_maybe_route_to_active_batch` runs AFTER PendingRegistry but BEFORE the regular ingest path — when the user has exactly one batch in `processing` state, the next text/voice (transcribed via Whisper) becomes input for the current step. **Pipeline:** `_step_enroll_unresolved` (Fireflies + Zoom) now calls `post_enrollment_batch` instead of the legacy `post_enrollment_prompts`, passing the meeting title for the widget header. **Tests** (`test_counterparty_enrollment_batch.py`, 13 tests): batch creation idempotency, phonetic-form dedup; toggle flip+redraw; toggle rejects non-recipient; Next-with-zero-selected terminates batch with all DECLINED; Next-with-selected starts processing at index=1 step=confirm_name; Keep advances to context with `canonical_name_corrected = mention_text`; text reply during confirm-name overrides name; text reply during context creates `Counterparty` + `telegram_enrollment` satellite + advances to next entity; Skip-confirm drops; Skip-context keeps hub no satellite; `find_active_batch_for_user` returns processing batch only. Plus integration test in `test_counterparty_enrollment.py` updated for the new return shape (batches_created, not posted-prompts). Suite: 1616 passed. **Backwards compat:** legacy v1 module (`counterparty_enrollment.py`) + statuses (`pending_yesno`, `awaiting_context`) preserved for old DB rows; only the call site in the pipeline is switched to v2. |
| FR-CR-05-137 | Slack mirror for Zoom meeting short-summary DMs. **Operator-pinned:** «и потом еще в слак надо отдавать зум встречи, в Artem AI отдаем как только в телеграм приходят, сразу же туда». **Trigger:** Zoom pipeline's `_send_short_summary` posts the same body to admin Telegram users; immediately afterward the new mirror code posts the same body to a configured Slack channel (Artem AI's bot DM `D0AUXKND35Y` per operator's setup). **Service:** `app/services/slack_mirror.py::post_meeting_summary_to_slack(slack_token, channel_id, body)` — instantiates `slack_sdk.WebClient` and calls `chat_postMessage` with `unfurl_links=False, unfurl_media=False`. Returns the response dict on success / `None` on disabled-by-config / `None` on `SlackApiError` / `None` on any unexpected exception (including transport errors when constructing the client). Failures NEVER cascade. **HTML→Slack-mrkdwn conversion:** `_to_slack_mrkdwn(body)` replaces the title-as-hyperlink contract `<a href="<doc_url>"><title></a>` (FR-CR-05-127) with Slack's `<<doc_url>|<title>>` form, decodes `&amp;` / `&lt;` / `&gt;` / `&quot;` / `&#39;`. Non-anchor lines pass through verbatim. **Settings:** `SLACK_MEETING_CHANNEL_ID` (default empty = mirror disabled). The existing `SLACK_BOT_TOKEN` is reused. **Pipeline integration:** `app/zoom/pipeline.py::_send_short_summary` post-action — after Telegram sent count is computed, posts the same body to Slack inside a try/except, traces `zoom_slack_mirror_posted` event to per-recording JSONL (FR-CR-05-128). **Tests** (`test_slack_mirror.py`, 9 tests): conversion of anchor + HTML entities + plain-pass-through + empty body; no-op on missing token / channel / blank body; chat_postMessage delegation with the right channel + converted text + unfurl flags off; `SlackApiError` returns None; unexpected exception during WebClient init returns None. Suite: 1603 passed. |
| FR-CR-05-136 | Calendar-match for meeting titles via Apps Script proxy. **Operator-pinned:** «когда спарсили fireflies файл, то далее посмотрели примерный диапазон по времени в гугл календаре (плюс минус полчаса), подтянули названия и участников встреч в этом промежутке, далее с ллм прогнали какой из заголовков встречи более соответствует повестке из fireflies и далее обновили название встречи на формат «30/04 - US Innovative Technology / TWG Globa»». **Why Apps Script:** operator-pinned «через Apps Script потому что я подключу потом другой токен где у меня не будет возможности сделать сервис аккаунт потому что там надо биллинг подключить — это сложно». Apps Script Web App runs as the operator's Google user, no GCP project / billing required for Calendar read. **Architecture (split):** Apps Script side (`docs/apps_script/calendar_proxy.gs`) exposes a `doGet(e)` Web App endpoint with a shared-token guard in URL query, returns `{events: [{title, start, end, attendees, description}]}` for the requested ISO date range. Python side (`app/services/calendar_match.py`) HTTPs the URL, runs an LLM-pass to pick the best match, formats the canonical title. **Service:** `fetch_calendar_events_around(meeting_dt, *, window_minutes, apps_script_url, shared_token)` — `urllib` GET, never raises, returns `[]` on any failure (HTTP error, JSON parse, network, Apps Script `{error: ...}` body). `match_calendar_event_to_meeting_via_llm(*, agenda, events, llm_backend, model, reasoning_effort)` — returns the picked event dict or `None`. JSON mode (`response_format={"type":"json_object"}`); the LLM emits `{"picked_index": <int|null>, "reason": <str>}`; out-of-range indices dropped. `format_canonical_title(meeting_dt, picked_title)` — `DD/MM - <picked_title>` (operator format). `match_and_format_title(...)` — end-to-end orchestrator with trace events. **Settings:** `CALENDAR_MATCH_ENABLED` (default `false`, opt-in); `CALENDAR_MATCH_WINDOW_MINUTES` (default `30`); `CALENDAR_APPS_SCRIPT_URL`; `CALENDAR_APPS_SCRIPT_SHARED_TOKEN`. **Pipeline integration:** `app/fireflies/pipeline.py::_step_match_calendar_title` runs AFTER `_step_detailed_summary` (we have the agenda) and BEFORE `_step_match_counterparties` (so the canonical title flows into the doc + short-summary header + per-task DM cards). Updates `MeetingRecording.title` in DB. Failures NEVER cascade — the step bracket catches every exception and the original title survives. Trace events `calendar_match_events_fetched`, `calendar_match_llm_picked`, `calendar_match_title_updated`. **Apps Script source + deploy guide:** `docs/apps_script/calendar_proxy.gs` (shared-token-guarded `doGet` returning JSON) + `docs/apps_script_deployment.md` step-by-step (create project, set Script Property, authorise CalendarApp, deploy as Web App «Anyone with link» + «Execute as me», wire env vars). **Tests** (`test_calendar_match.py`, 14 tests): title formatter (DD/MM padding, whitespace strip, empty input); HTTP proxy URL shape (token + ISO from/to in query string); HTTP error fallback to empty list; Apps Script error body fallback; LLM picks indexed event; LLM null = no rewrite; LLM out-of-range index dropped; LLM invalid JSON dropped; empty agenda / empty events return `None`; end-to-end on-match returns canonical title; end-to-end no-events returns `None`; end-to-end LLM-decline returns `None`. Suite: 1603 passed. |
| FR-CR-05-133 | In-Telegram enrollment widget for unresolved counterparty mentions. **Operator-pinned:** «если ты детектишь сущности, но их нет в справочнике, то ты по каждой сущности спроси нужно ли отслеживать информацию по ним и узнай может какой контекст чтобы юзер ввел текстом или голосом — то есть виджет с кнопками да/нет и когда да нажимаешь, то виджет исчезает, появляется новый мол введи информацию, ты вводишь текст или голос или кнопка пропустить — все по англ, спека + тесты». **Trigger:** after `_step_match_counterparties` finishes Pass 2, every mention with `directory_id is None` flows into a new `_step_enroll_unresolved` (Fireflies + Zoom mirrors) which posts ONE widget per (mention × admin recipient) to each admin's Telegram DM. **Schema:** alembic `0025_counterparty_prompts` — `counterparty_prompts(id, source_kind, source_id, mention_text, mention_normalised, chat_id BIGINT, user_id BIGINT, yesno_message_id BIGINT, context_message_id BIGINT, status, context_text TEXT, created_counterparty_id FK→counterparties.id ON DELETE SET NULL, created_at, updated_at, responded_at)` with `UNIQUE(source_kind, source_id, mention_normalised, user_id)` so pipeline reruns NEVER double-post the same widget to the same operator. Indexes on `mention_normalised` and `status` for the listener's lookups. **State machine** (all Telegram-facing copy English, operator-pinned): `pending_yesno ─[Yes]→ awaiting_context ─[text/voice]→ completed_added`; `─[No]→ declined`; `─[Skip]→ completed_skipped`. **Stage 1 widget:** «🔍 I picked up «<mention>» in a meeting transcript but it isn't in the counterparties directory.\n\nTrack this entity? [Yes] [No]». **Stage 2 widget (after Yes):** «Got it — adding «<mention>» to the directory.\n\nSend any context (text or voice), or press Skip to add it without notes. [Skip]». **Terminals:** «✅ Added «<mention>» to the directory with context.» (with notes) / «✅ Added «<mention>» to the directory (no context).» (Skip) / «❌ Won't track «<mention>».» (No). **Service:** `app/services/counterparty_enrollment.py::post_enrollment_prompts` (idempotent dedupe by `mention_normalised`; per-recipient rows; persist `yesno_message_id` from sender response; `EnrollmentPostResult(posted, skipped_existing, failed)`); `handle_yes` (rejects clicks from non-recipient admins; idempotent on double-click; edits stage-1 → stage-2 in place; captures `context_message_id`); `handle_no` (terminal, drops keyboard, NO Counterparty write); `handle_skip` (creates Counterparty hub via `_ensure_counterparty(name=mention)` find-or-create on `name_normalised`, NO satellite); `complete_with_context` (creates hub + `CounterpartyAttribute(source="telegram_enrollment", attributes={notes, via_recording_kind, via_recording_id})`; updates satellite in place if it already exists from a prior enrollment for the same hub; rejects empty-after-strip context). All paths trace via `trace_event(source, recording_id, event="counterparty_enrollment_*")`. **Keyboards:** `app/telegram_bot/keyboards.py` adds `ACTION_ENROLL_YES/NO/SKIP` constants + `enrollment_yesno_keyboard(prompt_id)` (two-button row, English «Yes» / «No») + `enrollment_skip_keyboard(prompt_id)` (single «Skip» button). **Listener wiring:** `app/telegram_bot/listener.py::_dispatch_action` adds three branches calling `_handle_enroll_yes/no/skip`. The Yes branch additionally registers a `PendingQuestion(action="enroll_context", task_id=prompt_id, …)` keyed by the stage-2 message id so the next text/voice reply from this user in this chat hits a new branch in `_handle_pending_reply` (also added) which calls `complete_with_context(reply_text=...)`. Voice replies flow through the existing `_maybe_transcribe_voice` (Whisper) before hitting the enrollment service. **Pipeline integration:** `_step_match_counterparties` (both Fireflies and Zoom) stashes the unresolved mentions on `row.__dict__["_fr_unresolved_mentions"]` / `_zm_unresolved_mentions`. `_step_enroll_unresolved` (new) reads that list, fetches admin recipients via `app.telegram_bot.handlers.admin_user_ids()`, calls `post_enrollment_prompts`. Failures here MUST NOT break the rest of the pipeline (wrapped in the same `_trace_step` + try/except as `_step_match_counterparties`). **Tests** (`test_counterparty_enrollment.py`, 16 tests): `test_enrollment_yesno_keyboard_uses_english_labels_and_action_constants` (English-only labels, callback_data round-trips through `parse_callback_data`); `::test_enrollment_skip_keyboard_single_button`; `::test_post_enrollment_prompts_creates_one_widget_per_mention_per_recipient` (3 unique mentions × 2 recipients = 6 widgets; «Тезер»/«тезер» dedupe to one); `::test_post_enrollment_prompts_idempotent_on_rerun` (UNIQUE constraint on rerun); `::test_post_enrollment_prompts_skips_when_no_recipients`; `::test_post_enrollment_prompts_records_send_failure_without_crashing`; `::test_handle_yes_transitions_to_awaiting_context_and_edits_widget`; `::test_handle_yes_rejects_unrelated_admin_clicks`; `::test_handle_yes_idempotent_on_double_click`; `::test_handle_no_sets_declined_and_does_not_create_counterparty`; `::test_handle_skip_creates_hub_without_satellite`; `::test_handle_skip_reuses_existing_hub_when_same_normalised_name` (operator manually added to Sheet between Yes and Skip → no duplicate hub); `::test_complete_with_context_creates_hub_and_satellite_with_notes`; `::test_complete_with_context_strips_whitespace_and_skips_empty_reply`; `::test_complete_with_context_replays_overwrites_satellite`; `::test_fireflies_pipeline_posts_enrollment_widgets_for_unresolved_mentions` (end-to-end: Pass 2 returns 1 resolved + 2 null → 2 widgets posted, 0 for the resolved one). Suite: 1577 passed. |
| FR-CR-05-132 | Drop `counterparties.type` — operator-pinned «давай справочник почистим, сделаем дистинкт только по именам без привязки к типу, тип вообще снеси». **Schema:** alembic `0024_counterparties_drop_type` collapses any `(name_normalised)` duplicate hubs (lowest id wins; repoints `counterparty_attrs.counterparty_id` and `counterparty_mentions.counterparty_id` to the surviving hub via window-function UPDATEs, then drops collisions on `(counterparty_id, source)` and `(source_kind, source_id, counterparty_id)` to keep their UNIQUE constraints intact, then deletes the loser hubs); swaps `UNIQUE(name_normalised, type)` → `UNIQUE(name_normalised)`; drops the `type` column. **Model:** `Counterparty.type` removed; `__table_args__` updated. **Sync:** `app/sync/counterparties.py::CounterpartiesSheetSync._read_status_outreach` no longer surfaces a `type` field — the sheet's column-A category label lives in the satellite's `attributes` JSON under its header name. `_read_name_first_tab` returns `{name, attributes, source}` (was `{name, type, attributes, source}`); `pull()` constructs `Counterparty(name=, name_normalised=)` (no `type=`). Dedupe loop unchanged (already `name_normalised`-keyed since FR-CR-05-126). **Matcher:** `_render_directory` drops the `type` column from the rendered table (now `id | name`); `ORDER BY` drops `Counterparty.type`; trace shortlist payloads no longer carry `type`; the three system prompts (`COUNTERPARTY_MATCH_SYSTEM`, `COUNTERPARTY_RESOLVE_SYSTEM`, `CANONICALIZE_TASKS_SYSTEM`) updated to describe `id | name`. **Pipelines:** Fireflies + Zoom both drop `Counterparty.type` from `ORDER BY` + end-of-pipeline `pipeline_summary` payload + `_build_counterparties_section_for_doc` (no « — <type>» suffix). **Tests:** all 25+ `Counterparty(...)` test instantiations stripped of `type=`; assertions on hub `.type` rewritten to read from satellite (`attributes` for column-A categories, `source` for tab labels); doc-rendering assertion updated; `test_pull_loads_outreach_tabs_with_tab_name_as_type` renamed `..._with_tab_name_as_source`. **Operator effect:** post-deploy DB collapsed `Tether`×3 (Financial/VC + Outreach + Rejections) → 1 hub, `Jabal`×2 → 1 hub; 593 hubs == 593 distinct_norms confirmed by SQL. Suite: 1561 passed. |
| FR-CR-05-128 | Counterparty directory dedupe — broaden `normalise_name`. **Operator regression:** `python -m ops.pull_counterparties` produced 593 hubs from 677 input rows (84 dedupes) but a manual scan via fuzzy detection script surfaced **90 more exact-fold clusters** (same name, different `type`) because `normalise_name` didn't strip parenthetical notes the operator uses for contact / source labels («Sequoia Capital (Лучиана)» vs «Sequoia Capital (Alfred Lin)», «MGX Fund (via Guy Hamelin)» vs «MGX Fund», «BBB (British Business Bank) (via Brent)» vs «BBB») and didn't translit Cyrillic→Latin (so «Тенсент» and «Tencent» stayed on separate hubs). **Fix:** `app/sync/counterparties.py::normalise_name` extended with two new steps before the legal-form strip — (a) `re.sub(r"\s*[\(\[][^)\]]*[\)\]]\s*", " ", ...)` strips both `(...)` and `[...]` content; (b) Cyrillic→Latin translit table maps every letter so cross-script duplicates collapse to one canonical key. Suffix-strip regex updated to drop the post-translit forms (`pao`/`oao`/`ao`/`ooo` instead of Cyrillic `пао`). After re-pull on the operator's directory, the 90 exact-fold clusters collapse to single hubs (each carrying multiple satellites from the originating sheets — proper data-vault shape). **Tests:** `test_counterparties.py::test_normalise_name_strips_accents_and_legal_forms` extended — pins parens-strip («Sequoia Capital (Лучиана)» → «sequoia capital»), translit (Cyrillic «Тенсент» → «tensent»), nested parens stripped to bare name. Suite: 1554 passed. Residual ~18 near-duplicate clusters (typos like «Mirae Asset» vs «Mirae Assets», «Walton enterprise» vs «Walton Enterprises») require an LLM-pass cleanup CLI — separate FR. |
| FR-CR-05-128 (rolling) | One-message overview guarantee + per-recording trace JSONL files. **Operator-pinned, repeatedly:** «To-Do всё ещё отдельным сообщением — ну сколько можно блять это прогонять, один раз уже зафиксируй навсегда и тестами и спекой покрой». **Root cause:** `_split_for_telegram` falls back to greedy paragraph-split when overview > limit; with 22+ tasks emitting verbose `<topic> - <action with details>` descriptions (FR-CR-05-120), the body hits ~5-7 KB and the greedy packer cuts between «Суть» and «To-Do» — breaking the operator's one-message contract. **Fix:** `_build_todo_section` accepts `compact: bool = False` — verbose mode keeps full descriptions (FR-CR-05-120 default); compact mode emits «N) Title (Owner)» only. Both `_step_short_summary` (Fireflies + Zoom) build verbose first; if the resulting body > `_SHORT_SUMMARY_ONE_MESSAGE_LIMIT = 4000` chars, rebuild in compact mode so the body fits in ONE Telegram DM. Full descriptions still ship via the Doc + per-task DM cards. Per-message split limit bumped to 4096 (Telegram hard cap). **Per-recording trace files:** `app/services/trace_log.py::trace_event(source, recording_id, event, **fields)` appends one JSONL line to `<MEETING_TRACE_DIR>/<source>-<recording-id>.jsonl` (default `/app/traces/...`) with a UTC ISO timestamp + event name + structured fields. Operator can `cat traces/zoom-<id>.jsonl \| jq` after a run to walk through every step / LLM call / Telegram send chronologically. Recording-id sanitiser strips `/` and `=` from Zoom UUIDs. Wired into: `_trace_step` (every step bracket fires `step_started` / `step_done` / `step_failed` to the trace file too), `whisper_bias_prompt_built` (prompt_chars + 240-char preview), `task_extraction_llm_returned` (raw_count + raw_titles + raw_owners), `task_owner_resolved` (per-task routing decisions), `task_verification_done` (verifier deltas), `pipeline_summary` (end-of-run snapshot), `counterparty_match_*` (call_started + llm_returned + done — threaded `trace_source` + `trace_recording_id` from both meeting pipelines). Failures swallowed: tracing must NEVER break the pipeline. **Catalog:** `docs/TRACES.md` — every event listed with fields + grep recipes for the 6 most common debug scenarios (walk one recording, latest summaries, Whisper biasing fired, counterparty matcher result, raw extraction output, owner routing). PR contract: new event = catalog entry. **Tests:** `test_fireflies.py::test_short_summary_compact_todo_when_overview_overflows` (22 tasks × 250-char descriptions → body > 4000 → compact rebuild → fits ≤4096 + all 22 titles present + verbose body absent + EXACTLY ONE summary DM); `::test_short_summary_keeps_verbose_todo_when_fits` (3 short tasks → verbose preserved); `::test_trace_event_writes_jsonl_per_recording` (separate file per `(source, recording_id)`, ISO-UTC timestamps, structured fields); `::test_trace_event_safe_against_zoom_id_slashes` (base64 sanitiser). Suite: 1553 passed. |
| FR-CR-05-127 | Title-as-hyperlink in TG short summary + Whisper biasing for proper-noun fidelity. **Operator-pinned (two regressions in one round):** (a) «'📄 Подробный отчёт:' — не выводи, просто гиперссылкой к названию» — the trailer line at the bottom is replaced by an HTML `<a href>` wrapping the «DD/MM - <Topic>» header. (b) «Whisper транскрипция максимально подробная, не пропускает 'tether'» — Whisper was mishearing brand names (Tether→teaser, Schaeffler→шафлера) because the call had no biasing context. **Title-as-hyperlink:** `app/fireflies/pipeline.py::_wrap_short_summary_with_doc_link(body, doc_url)` splits the body at the first `\n`, wraps line 1 in `<a href="<doc_url>">…</a>`, HTML-escapes the rest of the body so Telegram `parse_mode=HTML` (sender default) accepts free-text containing `&` / `<` / `>`. Both `_step_short_summary` (Fireflies + Zoom) replace the old `body.rstrip() + "\n\n📄 Подробный отчёт: " + url` trailer with the wrap call. `SHORT_SUMMARY_SYSTEM` prompt updated: drops «No emojis except optional `📄 Подробный отчёт: <url>` line», pins «caller wraps header in `<a href>` after you finish», forbids the LLM emitting any «Подробный отчёт» / URL trailer. **Whisper biasing:** `app/services/transcription.py::transcribe_bytes` accepts `prompt: str | None = None` (kwarg, optional) — forwarded to OpenAI's `audio.transcriptions.create` only when truthy so the Slack voice-message path is unchanged. `::build_whisper_bias_prompt(session, *, meeting_title=None, participants=None, max_chars=800)` packs the operator's canonical name registries (priority: meeting_title → participants → `team_members.real_name` where `active=True` → `counterparties.name`) as a comma-separated string, deduped case-insensitively, capped at `max_chars` to stay inside Whisper's ~224-token `prompt` limit. Empty registries + no metadata → `None`. Both `_step_transcribe` (Fireflies + Zoom) accept `session` and call the builder before chunked Whisper calls; the same `prompt` is reused for every chunk so concatenated transcripts stay consistent. Failures (DB exceptions, etc.) log + degrade to `prompt=None`. **Tests:** `test_fireflies.py::test_wrap_short_summary_with_doc_link_wraps_first_line_html` (first-line wrap, HTML-escape rest, no legacy trailer, empty / no-URL pass-through); `::test_short_summary_prompt_no_longer_mentions_old_doc_trailer` (prompt regression); `::test_build_whisper_bias_prompt_packs_team_and_counterparties` (priority + active-only filter + dedup); `::test_build_whisper_bias_prompt_returns_none_when_empty`; `::test_build_whisper_bias_prompt_caps_at_max_chars` (highest-priority items survive truncation); `::test_transcribe_bytes_passes_prompt_to_openai` (kwarg forwarded; absent when None — back-compat); `::test_fireflies_pipeline_passes_whisper_bias_prompt` (end-to-end: TeamMember + Counterparty rows surface into the Whisper call). End-to-end fireflies / zoom pipeline tests updated: `assert "📄 Подробный отчёт:" not in row.short_summary` + `assert "<a href=" in row.short_summary`. Suite: 1549 passed. |
| FR-CR-05-126 | Per-LLM-call transparency + fuzzy prefilter for counterparty matcher. **Operator regression:** transcript said «teaser» (Whisper misheard «Tether»), match returned 0 because prompt didn't push hard enough on phonetic / Cyrillic-Latin variants AND the LLM had to scan all 600+ directory rows. **Fuzzy prefilter:** `app/services/counterparty_match.py::_shortlist_directory_for_transcript` tokenises both transcript and directory names (NFKD + ASCII-fold + lowercase), keeps directory rows whose name-tokens hit a transcript-token via substring or `difflib.SequenceMatcher.ratio() ≥ 0.78`. «teaser» surfaces «Tether» (1-char swap). Caps at 200 rows; falls through to full directory when shortlist is empty. **Stronger prompt:** `COUNTERPARTY_MATCH_SYSTEM` now opens with «WHISPER MISHEARS THINGS — THIS IS THE MAIN JOB» banner and pins worked examples («teaser→Tether», «АДНОК/Адног→ADNOC», «Голдман Сакс→Goldman Sachs»), plus anti-examples («Felix» person ≠ «Felix Capital» fund; «teaser deck» industry term ≠ Tether company). **Per-LLM-call transparency:** every round-trip in the meeting pipelines emits a `*_llm_returned` log with the raw model output (titles + owners + matched ids) BEFORE filtering / persisting. Counterparty match logs `counterparty_match_call_started` (model, reasoning_effort, directory_size, shortlist_size, shortlist_sample, transcript_preview, prompt_chars), `counterparty_match_llm_returned` (raw_ids_count, valid_ids_count, invalid_ids), `counterparty_match_done` (matched_names list). Task extraction logs `*_task_extraction_llm_returned` (raw_count, raw_titles[:25], raw_owners[:25]). Verifier `*_task_verification_done` extended with new_titles[:25] / new_owners[:25]. **Tests:** `test_counterparties.py::test_shortlist_catches_whisper_misheard_tokens` (teaser→Tether, ADNOC direct, Goldman direct); `::test_shortlist_falls_back_to_full_directory_when_empty`; `::test_counterparty_match_prompt_pins_phonetic_and_cyrillic_examples`. Existing dedup-and-orders test updated for shortlist behaviour (unrelated entries are correctly filtered out). Suite: 1539 passed. |
| FR-CR-05-125 | LLM matching of counterparty mentions in meeting transcripts to the canonical directory (FR-CR-05-124 phase 2). **Schema:** alembic `0023_counterparty_mentions` — junction `counterparty_mentions(id, counterparty_id, source_kind, source_id, context, created_at)` with `UNIQUE(source_kind, source_id, counterparty_id)` and index on `(source_kind, source_id)`. `source_kind` discriminator carries «fireflies» or «zoom»; `source_id` is the platform's recording UUID. Matches one Counterparty hub to N recordings without two nullable FKs. **Matcher service:** `app/services/counterparty_match.py::match_counterparties_in_transcript(session, transcript, llm_backend, model, reasoning_effort)` — single LLM call with the full directory rendered as `id \| type \| name` + the transcript. Returns Counterparty rows in LLM-emitted order, dedupes, drops invalid ids. Empty directory → no LLM call → `[]` (pipeline runs fine, just without the 🔗 block). LLM exception → logged + `[]`. **Prompt:** `COUNTERPARTY_MATCH_SYSTEM` instructs the model to fuzzy-match speech-recognition variants («Адног» / «АДНОК» / «Adn-OC» → «ADNOC») while filtering coincidental name matches (a person named «Felix» ≠ «Felix Capital» unless context confirms it). Returns ids in transcript-appearance order; never invents ids. **Pipeline integration:** new `_step_match_counterparties` in both `app/fireflies/pipeline.py` and `app/zoom/pipeline.py` runs between detailed_summary and extract_tasks (FR-CR-05-119 order: detailed → match → tasks → verify → doc → short → cards). Replaces existing mentions for the recording's `(source_kind, source_id)` before inserting new ones — idempotent on `--rerun`. **Output integration:** `_build_counterparties_section_for_doc` renders «🔗 КОНТРАГЕНТЫ» header + bullet list with type for the Google Doc body (after detailed_summary, before tasks). `_build_counterparties_section_for_short_summary` renders «🔗 Контрагенты: name1, name2» one-liner for the TG short summary (after To-Do, before doc-link trailer). Both helpers query `counterparty_mentions` JOIN `counterparties`; empty result → empty string (sections drop entirely). **Tests:** `test_counterparties.py::test_match_counterparties_in_transcript_dedupes_and_orders` (LLM ids preserved in order, dedupe, invalid-id drop, prompt carries the directory); `::test_match_counterparties_returns_empty_on_llm_failure`; `::test_match_counterparties_skips_when_directory_empty` (no LLM call); `::test_build_counterparties_section_renders_doc_and_short_summary`. |
| FR-CR-05-124 | Counterparties hub + satellite directory pulled from Google Sheets (data-vault style). **Schema:** alembic `0022_counterparties` — hub `counterparties(id, name, type, name_normalised, …)` with `UNIQUE(name_normalised, type)` + index on `name_normalised`; satellite `counterparty_attrs(counterparty_id, source, attributes JSONB, captured_at)` with `ON DELETE CASCADE` to the hub and `UNIQUE(counterparty_id, source)`. **Sync:** `app/sync/counterparties.py::CounterpartiesSheetSync.pull(session)` — wipe-and-replace from up to three sheets: (a) «Status outreach» single tab where column A=type, column B=name, every column captured into JSONB; (b) «Outreach / Rejections / Looking for intros» tabs where column A=name, tab name=type; (c) «Investor Targets / rejected» tabs same name-first pattern. Generic `name_first_tabs=[(sheet_id, tab), …]` lets the operator add new sources without code. Empty input → no-op (protects directory from transient failures). Same canonical name across multiple tabs → one hub + multiple satellites; same name on same source → first wins. **Settings:** `COUNTERPARTIES_STATUS_SHEET_ID`, `COUNTERPARTIES_OUTREACH_SHEET_ID`, `COUNTERPARTIES_TARGETS_SHEET_ID` + matching tab-name vars; `COUNTERPARTIES_POLL_INTERVAL_SECONDS=300`. Feature opt-in (factory returns None when no sheet IDs set). **Listener integration:** `app/telegram_bot/listener.py::_maybe_run_counterparties_pull` runs on every tick, throttled by interval, swallows pull errors. Wired from `ops/telegram_listener.py` when any of the three sheet IDs is set. **CLI:** `python -m ops.pull_counterparties` for one-shot full sync; `--dry-run` reads without touching DB. **Normalise:** `normalise_name` does NFKD + accent-strip + lower + collapse-whitespace + drop common legal-form suffixes («Inc.», «LLC», «Ltd», «ПАО», «АО», «Société Générale» → «societe generale»). **Tests:** `test_counterparties.py` — `test_normalise_name_strips_accents_and_legal_forms`; `::test_pull_loads_status_outreach_with_jsonb_attributes`; `::test_pull_loads_outreach_tabs_with_tab_name_as_type`; `::test_pull_wipes_existing_rows_before_reloading`; `::test_pull_dedupes_same_name_in_status_outreach`; `::test_pull_empty_input_does_not_wipe_directory`; `::test_pull_loads_targets_sheet_alongside_outreach` (third sheet pattern); `::test_factory_returns_none_when_no_sheet_ids`; `::test_settings_outreach_tab_names_default_split_correctly`. **Phase 2 (separate FR-CR-05-125):** LLM matching step in Fireflies/Zoom pipelines that extracts company mentions from transcripts, fuzzy-matches against the directory via `name_normalised`, and surfaces canonical names + ids on the doc / short summary. |
| FR-CR-05-123 | Owner-assignment context contract: ALL three task-routing surfaces (TG ingest, Fireflies pipeline, Zoom pipeline) feed the LLM the SAME full team_members context — `slack_user_id \| display_name \| real_name \| role \| notes` — and apply the SAME routing rules (rule 6 anti-admin-default, rule 7 named-assignee). The TG path uses a separate `OWNER_SYSTEM_PROMPT` call (FR-CR-04-04); the Fireflies / Zoom paths combine extract+route into `TASK_EXTRACTION_SYSTEM` (one LLM call per meeting, ~20 tasks). **Tests:** `test_fireflies.py::test_owner_assignment_full_team_context_consistent_across_all_paths` — verifies the rendered employee table contains both `role` and `notes` columns for the meeting path (`_render_known_employees_table`) AND the TG path (`build_owner_user_prompt`); pins rule 7 / rule 6 across `TASK_EXTRACTION_SYSTEM` + `TASK_VERIFICATION_SYSTEM` + `OWNER_SYSTEM_PROMPT`. `::test_as_known_employees_returns_role_and_notes` — `app/services/team_members.py::as_known_employees` returns rows with non-empty role + notes for both `prefer_telegram=True` (meetings) and `prefer_telegram=False` (Slack/TG default); the SAME source data flows to all three pipelines. **Operator handle:** when an owner-routing regression like «Алине поручено → admin» appears, the fix is in the team sheet — fill the matching row's `role` + `notes`. The pipelines auto-pull every 60 sec via `app/sync/team_sheet.py::TeamSheetSync.pull` (FR-CR-05-23). |
| FR-CR-05-122 | Per-step trace logs so the operator can walk through a single recording's run by `grep <source>_step_(started|done|failed)` over the listener output. **Helper:** `app/fireflies/pipeline.py::_trace_step(source, step, **ctx)` — context manager that emits `<source>_step_started step=<step> <ctx>`, runs the body, emits `<source>_step_done step=<step> duration_ms=N <ctx>` (or `<source>_step_failed` + re-raise on exception). Both Fireflies and Zoom `process_one` wrap every step (download, transcribe, detailed_summary, extract_tasks, verify_tasks, doc_export, short_summary, send_short_summary, post_task_cards) in this helper, threading `fireflies_id` / `zoom_id` as the contextual key. **Test:** `test_zoom.py::test_zoom_pipeline_emits_trace_lines_for_every_step` runs the full pipeline end-to-end with stubbed external services, captures structlog events via monkeypatched `log.info` / `log.warning`, asserts that every step name appears in BOTH `*_step_started` and `*_step_done` events, that `_done` events carry an `int duration_ms`, and that no `_step_failed` events fire on the happy path. Operator trace pattern: `sudo docker logs slack-task-tg-listener \| grep -E '_step_(started\|done\|failed)' \| less` shows the full chronology of every recording's pipeline. |
| FR-CR-05-121 | Two-pass task extraction: a verifier LLM call re-reads the transcript + the already-extracted task list and adds anything the first pass missed. **Prompt:** `test_fireflies.py::test_task_verification_prompt_pins_second_pass_contract` pins `TASK_VERIFICATION_SYSTEM` — «SECOND-PASS verifier» framing, allows empty `{"tasks": []}` as a valid response, forbids duplicating already-extracted tasks, reuses the FR-CR-05-120 `<тема> - <action>` description format + rule 6 (anti-admin-default) + rule 7 (named-assignee). **Pipeline integration:** `_step_verify_tasks` runs after `_step_extract_tasks` and before `_step_doc_export` in both `app/fireflies/pipeline.py::process_one` and `app/zoom/pipeline.py::process_one`. Logs `*_task_verification_done` with `existing_count` + `newly_added` for triage. New rows get `reason="*_verified"` on their `TaskStatusHistory`, get scheduled for Sheets sync, and get DM-card-posted by the same `_step_post_task_cards` after the short summary. **End-to-end:** `test_zoom.py::test_verifier_pass_adds_missed_tasks_in_zoom_pipeline` — first pass returns 1 task, verifier returns 1 different task, DB holds 2; the test's verifier-aware fake LLM keys off the «SECOND-PASS verifier» substring in `system_prompt` (existing fakes were updated to return `{"tasks": []}` for the verifier so they don't double-count). |
| FR-CR-05-120 | Operator-pinned reasoning-model task extraction + new short-summary header format. **Thinking model:** `test_intent_pipeline.py::test_default_models_use_gpt_5_5_everywhere` updated — `fireflies_tasks_model` defaults to `gpt-5.5-thinking` (was `gpt-5.5`); operators without access override via `FIREFLIES_TASKS_MODEL=gpt-5.5`. `test_fireflies.py::test_task_extraction_prompt_pins_thinking_guidance` pins the «THINK CAREFULLY» block: read the entire transcript first, expect 8-25 tasks per 30-min business meeting, walk the `known_employees` table item-by-item for owner selection (rule 7 wins on named-assignee matches). **«<topic> - <action>» description format:** `::test_task_extraction_prompt_pins_topic_action_description_format` pins the operator-mandated description shape (≤300 chars, comma-separated multi-clause OK) with worked examples (Schaeffler / Draper / QIA / Варанты). The short-summary To-Do uses these descriptions verbatim. **New header «DD/MM - <Topic>»:** `::test_short_summary_prompt_uses_new_dd_mm_header_format` pins the new flat header in the canonical example («30/04 - ADNOC»), drops the old «<Тема> — DD.MM.YYYY \| NN мин» format and the two-line «Их сторона / Наша сторона» split (single «Участники: …» line for the short summary). **Verbatim To-Do builder:** `::test_build_todo_section_renders_tasks_verbagebatim_with_owner` (renamed) — uses `Task.description` as is (LLM already wrote it in the operator format), hard-cap 350 chars; empty `owner_display_name` drops the parens entirely (no «(не назначен)» noise). |
| FR-CR-05-119 | Operator-pinned single-source-of-truth for action items: tasks live as Task rows; the short-summary «To-Do» block (TG) is one-sentence-per-task; the Google Doc carries the FULL multi-sentence task list with owner / due / priority; LLM-generated «СЛЕДУЮЩИЕ ШАГИ» / «To-Do» blocks are dropped. **Pipeline order:** `detailed → tasks → doc → short` (both Fireflies and Zoom). Tasks must exist before doc-export so the archived report carries the verbose list, and before short-summary so the compressed To-Do block uses the actual Task rows. **Doc full task list:** `test_fireflies.py::test_build_full_tasks_section_for_doc_renders_verbose_with_meta` — `_build_full_tasks_section_for_doc(session, source_kind, source_conversation_id)` returns «📌 ЗАДАЧИ\\n\\n1. {description verbatim}\\n   Ответственный: X · Срок: 15.05.2026 18:00 · Приоритет: high\\n\\n2. …». Default-medium priority and date-only due render cleanly. **Short summary one-sentence compression:** `::test_first_sentence_compresses_multi_sentence_description` — `_first_sentence(text, limit=240)` returns first sentence (regex on `[.!?]\\s|$`, min-30-char gate so «И. Иванов» abbrev doesn't cut), falls back to char-cap with «…»; `::test_build_todo_section_renders_tasks_one_sentence_with_owner` extends the builder test with multi-sentence input → verifies only first sentence appears in the To-Do line. **Telegram chunk-split:** `::test_split_for_telegram_chunks_at_paragraph_boundaries` — `_split_for_telegram(text, limit=3800)` chunks at `\\n\\n` boundaries (each chunk starts on a fresh section), hard-splits at word boundary for any single paragraph longer than the limit; both `_step_send_short_summary` (Fireflies) and `_send_short_summary` (Zoom) iterate chunks and DM each separately, only count «delivered» when ALL chunks land. **LLM-emitted To-Do strip:** `::test_strip_llm_todo_block_removes_emitted_section`. **Detailed summary prompt drops next-steps:** `::test_detailed_summary_prompt_drops_next_steps_section`. **Short summary prompt forbids LLM emitting To-Do:** `::test_short_summary_prompt_forbids_llm_emitting_todo`. **Named-assignee rule (rule 7):** `::test_task_extraction_prompt_named_assignee_overrides_admin_default` — pins the «NAMED ASSIGNEE OVERRIDES EVERYTHING» language, with the «Алине поручено» / «Дима, нужно» worked regression examples + ban on substituting a different teammate. **Test infra:** `test_pipeline_posts_tg_card_per_extracted_task` filter tightened — task cards filtered by `👤` marker so the new short-summary To-Do (which also lists task descriptions) doesn't double-count as cards. |
| FR-CR-05-118 | Zoom listener auto-polling + enum migration + Fireflies-parity for task creation. **Listener poll:** `test_zoom.py::test_zoom_listener_poll_off_by_default` (no API call until `wire_zoom(enabled=True)`); `::test_zoom_listener_poll_pulls_when_enabled` (after wire + cutoff override → `process_one` called for each fresh recording); `::test_zoom_listener_poll_throttled_within_interval` (3 immediate ticks → 1 API call); `::test_zoom_listener_poll_swallows_pipeline_errors` (one bad recording doesn't abort the batch); `::test_zoom_listener_poll_skips_recordings_before_startup` (FR-CR-05-51 cutoff symmetric with Fireflies — old meetings dropped); `::test_settings_zoom_polling_defaults` (realtime OFF by default; 60s interval; 10-item batch; Fireflies poll defaulted to 60s for parity). Manual: `app/telegram_bot/listener.py::_maybe_poll_zoom` mirrors `_maybe_poll_fireflies` (cutoff, throttle, error-swallow); `ops/telegram_listener.py` wires `ZoomPipeline` when `ZOOM_ACCOUNT_ID/CLIENT_ID/SECRET` set; `.env` activates with `ZOOM_REALTIME_ENABLED=true`. **Heartbeat log:** `_maybe_poll_{fireflies,zoom}` fire `listener_*_poll_done` with `seen` / `processed` / `skipped` / `skipped_old` / `tasks_created` / `errors` whenever the API returned any rows — operator can tell «no new meetings, all skipped» from broken polling. **Enum migration:** alembic `0021_task_source_kind_zoom` runs `ALTER TYPE task_source_kind ADD VALUE 'zoom'` (the 0020 migration created the `zoom_recordings` table but forgot the enum extension; Postgres rejected `INSERT … source_kind='zoom'` with «invalid input value» and rolled back the whole pipeline transaction). `test_task_source_kind.py::test_postgres_enum_extension_pinned_in_migrations` scans every alembic file and asserts each `TaskSourceKind` value appears in some `ADD VALUE 'X'` clause OR the original `Enum(...)` inline list — future enum additions without the matching migration fail at unit-test, not deploy. **Zoom task creation parity:** `::test_zoom_pipeline_runs_every_step_and_creates_zoom_source_tasks` asserts `t.source_conversation_id == row.zoom_id` (JOIN key), `t.source_message_ts == row.zoom_id`, `t.due_time == 18:00`, that admin DM stream contains BOTH short summary AND ≥1 task card (pre-fix only summary). `app/zoom/pipeline.py::_step_extract_tasks` mirrors Fireflies path: full Task() with join keys + due_time, `session.flush()` between Task and `TaskStatusHistory(reason='zoom_extracted')`, `schedule_sync_task` for Sheets/Tasks API push, `post_initial_card` per task (FR-CR-05-58 parity), `owner_display_name` resolution. **--rerun parity:** `ops/migrate_zoom.py --rerun` resets the same step-flag set as `migrate_fireflies --rerun`. |
| FR-CR-05-117 | Operator-mandated short-summary format + meeting-quality polish. **Auto-stamp title fix:** `test_fireflies.py::test_looks_like_auto_stamp_title_detects_fireflies_defaults` («Apr 30, 03:32 PM», «May 5 at 5pm», «Dec 1», empty/None flagged; real titles like «ADNOC partnership call» pass through). Manual: `pipeline.py::_step_detailed_summary` calls `_derive_topic_title` (LLM, ≤60 chars Russian) when `_looks_like_auto_stamp_title(row.title)`. **Operator-pinned format:** `::test_short_summary_prompt_pins_operator_format` pins the canonical ADNOC layout in `SHORT_SUMMARY_SYSTEM` (verbatim example, `<Тема> — DD.MM.YYYY \| NN мин` header, «Их сторона» / «Наша сторона» split, «Суть», «To-Do», anti-auto-stamp). **Doc-link trailer:** `::test_pipeline_process_one_runs_every_step` extended — asserts `📄 Подробный отчёт: <url>` appended to `row.short_summary` after LLM body; `test_zoom.py::test_zoom_pipeline_runs_every_step_and_creates_zoom_source_tasks` mirrors. Append is deterministic (caller, not LLM) so URL can't be truncated/hallucinated. **UID-suffix strip:** `::test_strip_uid_suffixes_removes_employee_uids_only` (LLM accidentally copied «Валентина (462156243)» into description; post-processor drops `(<token>)` only when token matches a real `slack_user_id` — preserves «(Q2)», «(2025)», «($300k)»). `::test_task_extraction_prompt_forbids_uid_in_description` pins the prompt rule. **Markdown strip:** `::test_strip_markdown_emphasis_removes_paired_markers` (`**bold**` / `__bold__` / `*italic*` / `` `code` `` paired markers stripped, «3 * 5» math survives); `::test_detailed_summary_prompt_forbids_markdown` pins «PLAIN TEXT ONLY. NO MARKDOWN» rule in `DETAILED_SUMMARY_SYSTEM`. **Anti-admin-default:** `::test_task_extraction_prompt_forbids_admin_default_owner` — rule #6 in `TASK_EXTRACTION_SYSTEM` pins «Null is STRICTLY BETTER than defaulting to admin / AI Lead», admin row is context only. Manual: `_step_extract_tasks` logs `fireflies_task_owner_resolved` per task (`llm_owner` / `final_owner` / `resolution`). **Initial card header dropped:** `test_telegram_cards.py::test_post_initial_card_sends_one_dm_per_recipient` extended — asserts no «✨ New task from this message» banner in any DM card text. **Zoom audio extension sniffing:** `::test_sniff_audio_extension_detects_real_container` (M4A / MP4 / MP3 / WAV / Ogg / FLAC magic-byte detection; unknown bytes → None); `::test_split_audio_chunker_preserves_input_container` (chunker outputs `.m4a` for `.m4a` input — Whisper «Invalid file format» fix; «Exactly one MP3 audio stream» fix). Manual: `pipeline.py::_step_download_audio` (Zoom) writes to `.bin`, sniffs, renames to real ext; sanitises base64 zoom_id (`/` → `_`, `=` stripped) so it doesn't create subdirs. **Rerun CLI:** `ops/migrate_fireflies.py --rerun` and `ops/migrate_zoom.py --rerun` reset every step flag + step output on the matched rows (audio_downloaded, transcribed, detailed_summarised, short_summary_sent, doc_exported, tasks_extracted, transcript_text, detailed_summary, short_summary, google_doc_id, google_doc_url, tasks_extracted_count, last_error, processed_at). Existing Tasks NOT deleted. |
