...
| # | Баг | Статус |
|---|---|---|
| B1 | **Бессодержательные саммари уходят в Slack и на вебхук** (заголовок + «Участники:» без сути). Корень — Whisper-галлюцинация / тонкий VTT. | ✅ `summary_has_body` на вебхуке. **FR-CR-05-157e (2026-06-04):** единый гейт `is_contentless_meeting` в `_step_short_summary` (Zoom+FF): при пустой встрече `return False` ДО `row.short_summary` → НЕ постится НИ В ОДИН канал (TG+Slack+webhook). Закрыл регресс «Запись без содержимого» (0 задач, ушло и в Slack, и в n8n 15К payload). Сигналы: no-content фраза ИЛИ derived-title «без содержимого / no content». Тест `test_contentless_meeting.py`. |
