...
| FR-CR-05-206 | **Стратегический экспорт — добавить ОСТАЛЬНЫЕ задачи из встреч/чатов (`tasks`: zoom/ff/slack)** (operator-pinned 2026-05-27: «давай добавляем в эту таблицу остальные» — после FR-CR-05-205 в листе `1h1wCHmr` были только telegram-драфты, ~901 стратегическая задача из встреч отсутствовала). Стратегические задачи из Zoom/Fireflies/Slack живут в таблице `tasks` (direction в `task.extra["direction"]`), а НЕ в `action_drafts` (там `task_id` всегда NULL). **Fix**: `ops/sheet_sync_export_tasks.py` после цикла по `action_drafts` добавляет второй проход по `tasks`: `select(Task).where(deleted_at IS NULL).where(source_kind IN (zoom, fireflies, slack)).where(created_at >= since)`; фильтр `extra["direction"] in DIRECTIONS_IMPORTANT` + непустой title. Мапит в те же 16 колонок: **Status** = реальный статус задачи (`STATUS_DISPLAY_BY_KEY`, фолбэк на `--status`), **Источник** = `Zoom/Fireflies/Slack` (из `source_kind`), **Ссылка** = `google_doc_url` отчёта (join `zoom_recordings`/`meeting_recordings` по `source_conversation_id`; фолбэк `source_permalink`) через новый helper `_source_and_link_for_task(session, task)` (защитный, ошибка БД → фолбэк). Telegram-драфты и meeting-tasks не пересекаются (в `tasks` нет telegram-стратегических) → слияние дизъюнктно, дедуп не нужен. **Slack-gap + доклассификация (operator 2026-05-27 «доклассифицировать + починить апстрим»):** живые slack-задачи попадали в `tasks` БЕЗ `direction` (None) → не проходили стратегический фильтр (в листе слака не было). Причина: `create_task_from_draft` строила `extra` только с `owner_assumed`, теряя `payload["direction"]` (классифицированный на ингесте, FR-CR-05-200). **Апстрим-фикс:** `create_task_from_draft` теперь копирует `payload["direction"]` → `extra["direction"]` (единый chokepoint всех draft→Task путей; meeting-tasks уже несут direction из пайплайна FR-CR-05-163). **Бэкфилл в экспорте:** для драфтов/задач без direction экспорт доклассифицирует gpt-4o-mini (`_classifier_backend` + `_classify_missing` → `classify_one_direction`; флаг включён по умолчанию, `--no-classify` оставляет чисто read-only, `--classify-model` override), пишет direction обратно (`payload`/`extra`) и **коммитит ТОЛЬКО эти изменения** (dry-run — никогда не пишет; иначе `session.rollback()`). Так slack подтягивается сейчас и экспорт самозалечивается на будущих прогонах. | `test_sheet_export_source_link.py::test_fr_cr_05_206_zoom_task_source_and_doc_report` + `…slack_task_uses_permalink` + `…fireflies_task_prefers_doc_report` + `…task_no_recording_falls_back_to_permalink` + `…classify_missing_returns_direction` + `…classifier_backend_none_without_key` + `test_task_direction_carryover.py::test_fr_cr_05_206_direction_carries_into_task_extra` + `…no_direction_leaves_extra_unset` · → ARCH §3,§4 |
