...
def _build_responder_callback(
    *,
    ...
    if not api_key:
        return None
    # FR-CB2-3.27 — hard SDK timeout. Anthropic's MCP tool
    # resolution can hang several minutes when one MCP server is
    # slow. 60 sec covers legit fast paths; longer = retry.
    anthropic_client = Anthropic(api_key=api_key, timeout=60.0)
