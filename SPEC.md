...
| FR-CR-05-128 | Operator-pinned approval flow for meeting-extracted tasks: meeting tasks now ship through the SAME ActionDraft → ✅/✏️/❌ confirm-widget contract that TG-ingested tasks use. **Operator regression:** «апрув задач оператором в тг боте после того как ты эти задачи составил, я должен отжать, а они уже как будто отжаты — посмотри как реализовано с тг-потоком». Pre-128 the Fireflies/Zoom pipelines created `Task(status=todo)` rows immediately + scheduled Sheets sync + posted finalised task cards (`post_initial_card`); the operator never had a chance to reject / edit a hallucinated task before it landed in the working list. **Pipeline refactor:** `_step_extract_tasks` (Fireflies + Zoom) now creates `ActionDraft(state=proposed)` rows instead of `Task` rows. Each meeting persists ONE `ContextSnapshot` + ONE `IntentInference` per pass («fireflies_extract», «fireflies_verify», «zoom_extract», «zoom_verify») via shared helpers `app/fireflies/pipeline.py::_create_meeting_inference` + `::_create_meeting_draft`; drafts hang off the inference. Draft `payload` carries the canonical task fields (title/description/owner_user_id/owner_display_name/priority/due_date/due_time); `payload._pending` carries source-routing fields (`source_kind="fireflies"|"zoom"`, `conversation_id=fireflies_id|zoom_id`, `message_ts`, `permalink`, `fallback_author`) so `handle_confirm_draft` → `create_task_from_draft` materialises the eventual `Task` with the correct `source_kind` + join keys. `_step_verify_tasks` reads drafts (NOT tasks) for the «already-extracted» dedup block + appends new drafts under a separate inference. **Widget posting:** `_step_post_task_cards` now calls `post_draft_confirmation` per draft (operator-pinned: same widget shape as TG-ingest) instead of `post_initial_card`. The 3-button keyboard (`accept`/`edit`/`ignore`) is the existing TG-flow keyboard; no new callback handlers — the existing `app/telegram_bot/handlers.py::handle_confirm_draft` already supports `_pending.source_kind` discrimination and creates the `Task` with `source_kind=fireflies|zoom` on ✅. ❌ → existing soft-delete path (`state=ignored`); the draft stays in DB for audit but disappears from doc + To-Do renders. **Read paths widened:** `_build_todo_section` + `_build_full_tasks_section_for_doc` now source from `_meeting_drafts(session, source_kind, conversation_id)` (filter on JSON `payload._pending.{source_kind,conversation_id}`, exclude ignored/expired/failed) instead of querying `tasks` directly; `_draft_render_view(draft)` projects the draft.payload into the same field set the old Task-based renderers used. To-Do block in the short summary therefore shows the proposed list verbatim before any approval, matching what's in the per-task widgets the operator is about to triage. **No Sheets sync until approval:** `schedule_sync_task` is no longer called during the meeting pipeline; it fires lazily inside `handle_confirm_draft` after `create_task_from_draft`, so the directory only contains operator-approved rows. **Idempotency on `--rerun`:** `_wipe_pending_meeting_drafts(session, source_kind, conversation_id)` deletes only `state=proposed/edited` rows for the meeting before `_step_extract_tasks` re-creates them, so a second pipeline run doesn't pile up duplicate widgets. CONFIRMED drafts are sacred (operator's ✅ already produced a Task; re-run must NOT undo that), and IGNORED drafts stay too (audit trail of what was rejected). Both pipelines call the wipe at the top of `_step_extract_tasks`; `--rerun` just toggles `row.tasks_extracted=False` and the wipe handles the rest. **Tests:** `test_fireflies.py::test_meeting_extract_creates_proposed_drafts_not_tasks` (no Task created, exactly one ActionDraft with full payload + _pending; `schedule_sync_task` NOT called); `::test_meeting_post_task_cards_emits_confirm_widgets` (widget posted with the standard 3-button accept/edit/ignore keyboard); `::test_meeting_draft_confirm_creates_task_with_source_fields` (existing handler turns a fireflies-draft into a `Task(source_kind=fireflies)` with `source_conversation_id` + `source_permalink` propagated from `_pending`; draft.state → confirmed, draft.task_id wired); `::test_rerun_wipes_pending_drafts_keeps_confirmed_and_ignored` (only proposed/edited deleted; confirmed + ignored + other meetings untouched); `::test_meeting_extract_is_idempotent_on_rerun` (end-to-end: extract twice → exactly one proposed draft per task). Existing fireflies/zoom end-to-end tests rewritten: `tests` queries swapped to `ActionDraft` queries; soft-delete coverage uses `state=ignored` instead of `Task.deleted_at`. Test helper `_add_meeting_draft` builds drafts via the same `_create_meeting_inference` + `_create_meeting_draft` helpers the pipelines use. Suite: 1554 passed. |
